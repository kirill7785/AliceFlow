						...
						if (fabs(sumP) < RealZERO) {
							...
							if (bpositive_connections) {
								...
								if (fabs(Amat.aij[is0]) > maxelem_threshold*theta) {
									...
									P[icount1].aij = static_cast<real_mix_precision>(fabs(Amat.aij[is0]) / sumP);
									icount1++;
	...
	integer &number_of_F_nodes_with_one_single_strong_C_neighborF)
{
	...
	// Прямая интерполяция с элементам непрямой. Экспериментальный вариант. 
	// Непрямая интерполяция применяется только для F узлов которые
	...
	doublerealT* &threshold_quick_only_negative)
{
						...
						if (itypemodifyinterpol > 0) {
							...
							if (itypemodifyinterpol == 1) {
								...
								if (ifrom_re_operation_protection < inumberadaptpass) {
									...
									integer id_size_873 = static_cast<integer>(n_a[ilevel - 1] / 100);
	...
	if (!byes_add) {
		...
		// с параметром 0.4 четвертая интерполяция обгоняет более сильную шестую.
		doublerealT magic = static_cast <doublerealT>(0.4); // 0.4 optimum
							...
							if (Amat.j[is0] != Amat.i[ii1]) {
								if (this_is_C_node[Amat.j[is0]]  ) {
									...
									if (bmodify_interpol4_amg && ((icsos == 1) && (icsosF == 0))) {
										...
										if (is0_candidate != -1) {
											P[icount1].j = static_cast<integer_mix_precision>(i8);
											P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat.j[is0_candidate]]);
											P[icount1].aij = static_cast<real_mix_precision>(fabs(Amat.aij[is0_candidate]) / (sumP + sum1));
											icount1++;
											if (icount1 >= nsizePR*n) {
												printf("memory error!!!\n");
												printf("not enough memory for the interpolation operator.\n");
										...
										if (fabs(Amat.aij[is0]) > maxelem_threshold_theta) {
											P[icount1].j = static_cast<integer_mix_precision>(i8);
											P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat.j[is0]]);
											P[icount1].aij = static_cast<real_mix_precision>(fabs(Amat.aij[is0]) / (sumP + sum1));
											icount1++;
											if (icount1 >= nsizePR*n) {
												printf("memory error!!!\n");
												printf("not enough memory for the interpolation operator.\n");
	...
	doublerealT* &threshold_quick_only_negative)
{
	...
	if (byes_add == false) {
		...
		// magic=0.4 is optimum.11 jan 2016
		doublerealT magic = static_cast<doublerealT>(0.4); // 0.4
			...
			for (integer i8 = 1; i8 <= n_a[ilevel - 1]; ++i8) if (this_is_F_node[i8]  ) {
				...
				if ((false) && (icsos == 1)) {
					...
					if (((icsos == 1) || (icsos == 2) || (icsos == 3)) && (icsosF != 0)) {
													...
													if (Amat.j[is0_loc] != Amat.i[ii1_loc]) {
														if (this_is_C_node[Amat.j[is0_loc]]  ) {
															...
															if (fabs(Amat.aij[is0_loc]) > maxelem_threshold_loc*magic) {
																...
																P[icount1].aij = static_cast<real_mix_precision>(mult1*fabs(Amat.aij[is0_loc]) / sumP_loc1);
						...
						for (integer i_95 = icount1_mem; i_95 < icount1; ++i_95) {
							P[i_95].aij = static_cast<real_mix_precision>(P[i_95].aij / sum_ok);
						}
...
double theta(integer ilevel)
{
	...
	*/
	if (my_amg_manager.bthreshold_auto) {
		// Автоматическое задание порога threshold.
		...
		return my_amg_manager.theta; // Константа заданная пользователем в меню.
	}
}
...
#else
							printf("memory very large ilength_n=%d n_a[ilevel - 1]=%d\n", ilength_n, n_a[ilevel - 1]);
#endif
							system("PAUSE");
						}
													...
													if (this_is_C_node[Amat.j[is0_loc]]  ) {
														for (i_97 = 0; i_97 < ilength_n; ++i_97) {
															if (Amat.j[is0_loc] == jposition_in_P[i_97]) {
																...
																P[icount1_frozen + i_97].aij += static_cast<real_mix_precision>((my_mult * fabs(Amat.aij[is0_loc])) / (sumP * sum23));
																break;
															}
														}
													}
