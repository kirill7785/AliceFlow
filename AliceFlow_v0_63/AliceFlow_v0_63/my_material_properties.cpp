...
void my_air_properties(doublereal TiP, doublereal PiP, 
	...
	*/
	...
	rho = 1.1614f;
	cp= static_cast<float>(1000.0*(1.0005+1.1904e-4*(TiP))); // J/(kg*GC)
	lam= static_cast<float>(2.44e-2*exp(0.82*log(TK/273.15))); // W/(m*K)
	mu= static_cast<float>(1.717e-5*exp(0.683*log(TK/273.15))); // Pa*s
	//printf("mu=%e, TK=%e\n",mu,TK); // debug Ok
	//system("pause");
	// в maple по способу наименьших квадратов получена следующая зависимость:
    // Исправлено 27.07.2016
	beta = static_cast<float>(0.001*(-0.06810259 + 0.00013411*TiP - 8.287412214e-8*TiP*TiP + 1022.18 / (273.15 + TiP)));
...
void my_water_properties(doublereal TiP,  
	...
	*/
	...
	// В приближении Буссинеска опорное значение плотности константа,
	// зависимость плотности от температуры учитывается через коэффициент линейного температурного расширения.
	rho = static_cast<float>(995.7 / (0.984 + 0.483e-3*(20.0))); // kg/m!3
	cp= static_cast<float>((4194-1.15*TiP+1.5e-2*TiP*TiP)); // J/(kg*GC)
	lam= static_cast<float>(0.553*(1.0+0.003*TiP)); // W/(m*K)
	mu= static_cast<float>((1.78e-6/(1.0+0.0337*TiP+0.000221*TiP*TiP))*rho); // Pa*s
	// в maple по способу наименьших квадратов получена следующая зависимость:
	beta= static_cast<float>(0.02289718770+0.142992772827544502e-6*TiP*TiP-0.67623235e-4*TiP-(6.27314791869)/(TK));
...
//const int MY_AUSN = 101;    // Припой золото 80% станум 20%
const int MY_ALUMINA = 101;   // поликор Alumina, Al2O3 -ref. MatWeb[2] 99.9% Al2O3, polycrystalline aluminum oxide.
const int MY_SI = 102;        // Кремний
const int MY_GAAS = 103;      // Арсенид Галия
const int MY_GAN = 104;       // Нитрид Галия
const int MY_SIC4H = 105;     // Карбид кремния SiC4H
const int MY_SAPPHIRE = 106;  // Сапфир
const int MY_DIAMOND = 107;   // Алмаз
const int MY_MD40 = 108;      // Псевдосплав МД40
const int MY_AU = 109;        // Золото
const int MY_SIO2 = 110;      // оксид кремния
const int MY_COPPER = 111;    // Медь
const int MY_KOVAR = 112;     // Ковар
const int MY_BRASS = 113;     // Латунь ЛС-59-1-Л
const int MY_DURALUMIN = 114; // Дюралюминий Д16
const int MY_ALUMINIUM_NITRIDE = 115; // Нитрид аллюминия (кристаллический).
const int MY_GLUE_ECHES = 116;  // Клей ЭЧЭС
...
void my_si_properties(doublereal TiP,
						...
						float &lam) {
       // Silicon
       float TK= static_cast<float>(TiP+273.15);
	   rho=2330; // kg/m^3
	   //cp=711; // J/(kg*K)
	   ...
	   if (TK < 800.0) {
		   cp= static_cast<float>(83.23167276-11801.68987/TK+0.3123808545e-5*TK*TK*TK+3.672005743*TK-0.5805489801e-2*TK*TK);
	   }
	   else {
	   ...
	   lam= static_cast<float>(148*exp(-1.35*log(TK/300))); // 120 W/(m*K)
} // Si
...
void my_alumina_properties(doublereal TiP,
	...
	if (TK < 800.0) {
		cp = static_cast<float>(0.00000505903*TK*TK*TK - 0.01042874335*TK*TK + 7.612830818*TK + (50982.822 / TK) - 898.2525);
	}
	else {
		// ограничитель сверху на ту область где аппроксимация по способу наименьших квадратов не работает.
		cp = 1172;
	...
	// 600 18.9
	// 800 13.0
	lam = static_cast<float>(47.84832382*exp(-1.328556144*log(TK / 300.0))); // 46
...
void my_gaas_properties(doublereal TiP,
	   ...
	   {
		   cp = static_cast<float>(4.059e-8*TK*TK*TK - 1.296e-4*TK*TK + 1.854e-1*TK + 279.0);
	   }
	   ...
	   lam= static_cast<float>(54*exp(-1.25*log(TK/300))); // 54 W/(m*K)
...
void my_gan_properties(doublereal TiP,
	   ...
	   if ((TK > 200) && (TK < 1300)) {
		   cp = static_cast<float>(591.807 + 0.064971*TK - 2.6e7 / (TK*TK) + 2.94e9 / (TK*TK*TK));
	   }
	   // Функция изменения теплопроводности от температуры взята из
	   // книги Р. Квэя Электроника нитрида галия.
	   //lam=130; // данные В.Д. Красильникова
	   lam= static_cast<float>(130*exp(-0.43*log(TK/300))); // 130 W/(m*K)
} // GaN
...
void my_sic4h_properties(doublereal TiP,
	   ...
	   if ((TK>=77.0)&&(TK<=773.2)) {
		   cp= static_cast<float>(-600.9595790625+6.41266663224237*TK+16052.9931969468/TK-0.00900588312450910*TK*TK+0.459922567674634e-5*TK*TK*TK);
	   ...
	   //lam=370; // V-легир. SiC
	   lam= static_cast<float>(370*exp(-1.411*log(TK/300))); // 370 W/(m*K)
} // SiC4H
...
void my_sapphire_properties(doublereal TiP,
	   ...
	   if (TK<774) {
		   cp= static_cast<float>(-703.3920492+19101.78790/TK+0.5623375490e-5*TK*TK*TK+7.499240665*TK-0.01095728614*TK*TK);
	   }
	   else cp=1168;
	  ...
	  // lam=23; // Данные В.Д. Красильникова
	   ...
	   lam= static_cast<float>(42*exp((-1.134821)*log(TK/298))); // 42 W/(m*K) при 298К
...
void my_diamond_properties(doublereal TiP,
	   ...
	   if (TK < 774) {
		   cp= static_cast<float>(-696.4289+32660.38060/TK-0.2774926361e-5*TK*TK*TK+3.566362474*TK+0.1285766705e-2*TK*TK);
	   }
	   else cp=1590;
	   ...
	   lam= static_cast<float>(2300*exp(-1.85*log(TK/300))); // 130 W/(m*K)
...
void my_md40_properties(doublereal TiP,
	   ...
	   if ((TK > 173) && (TK < 573))
	   {
		   cp = static_cast<float>(1.66e-6*TK*TK*TK - 2.263e-3*TK*TK + 1.095*TK + 138.5);
	   }
	   else if (TK <= 173) cp = 268.77f;
	   else cp = 335.111f;
	   lam=210.0f; // W/(m*K)
	   if ((TK >= 273) && (TK < 1600)) {
		   lam = static_cast<float>(7.15e-9*TK*TK*TK - 2.18e-6*TK*TK - 6.03e-2*TK + 271.0);
	   }
...
void my_au_properties(doublereal TiP,
	   ...
	   if ((TK>77.0)&&(TK<773.0)) {
		   cp= static_cast<float>(140.2979168-3345.749125/TK+0.3399388027e-7*TK*TK*TK+0.3605589168e-2*TK-0.2418961279e-4*TK*TK);
	   }
	   ...
	   if (TK<974) {
		   lam= static_cast<float>(327.5268055+146.5916497/TK+0.5614880962e-7*TK*TK*TK-0.008865308782*TK-0.1043207287e-3*TK*TK);
	   }
	   else lam=272;
...
void my_sio2_properties(doublereal TiP,
	   ...
	   if ((TK >= 273.0) && (TK <= 773))
	   {
		   cp = static_cast<float>(-1.6667e-7*TK*TK*TK - 9.635e-4*TK*TK + 1.975*TK + 236.02);
	   }
...
void my_copper_properties(doublereal TiP,
	   ...
	   if ((TK>77.0)&&(TK<773.0)) {
		   cp= static_cast<float>(506.2148755-22501.73085/TK-0.3085157062e-6*TK*TK*TK-0.2854473337*TK+0.5289207596e-3*TK*TK);
	   }
	   else if (TK<77.0) cp=195.0f;
	   else cp=430.0f;
	   ...
	   if ((TK>173.2) && (TK<973.2)) {
		   lam= static_cast<float>(318.4875333+12313.69842/TK+0.1748333335e-6*TK*TK*TK+0.2380247005*TK-0.3905912003e-3*TK*TK);
	   }
	   else if (TK<=173.2) lam=420.0f;
	   else lam=354.0f;
} // Copper
...
void my_kovar_properties(doublereal TiP,
	   ...
	   if ((TK>273) && (TK < 703))
	   {
		   cp = static_cast<float>(0.486*TK + 306.6);
	   }
	   else if (TK < 273) cp = 439.6f;
	   else cp = 648.954f;
	   //lam=19; // W/(m*K) 
	   // Функциональная зависимость теплопроводности от температуры
	   // получена по следующим точкам с помощью МНК.
	   ...
	   if ((TK>273.0) && (TK<973.0)) {
		   lam= static_cast<float>(44.13357753-3650.246187/TK-0.4572641192e-7*TK*TK*TK-0.8854732756e-1*TK+0.1132267331e-3*TK*TK);
	   } else if (TK<273.0) lam=14.1f;
	   else lam=19.3f;
} // Kovar
...
void my_brass_properties(doublereal TiP,
	   ...
	   if ((TK>=173.2)&&(TK<=673.2)) {
		   lam= static_cast<float>(-591.598556543738+2.90903837477216*TK+52133.0458047673/TK-0.00454066869215317*TK*TK+0.249629141483149e-5*TK*TK*TK);
...
void my_duralumin_properties(doublereal TiP,
	   ...
	   if ((TK>273.0) && (TK < 673.0)) {
		   cp = static_cast<float>(3.33e-7*TK*TK*TK - 2.62e-3*TK*TK + 3.3*TK + 38.0);
	   }
	   else if (TK <= 273.0) cp = 750.13f;
	   else cp = 1172.0f;
	   lam=130.0f; // W/(m*K)
	   if ((TK>150.0) && (TK < 573)) {
		   lam = static_cast<float>(0.171*TK + 66.1);
	   }
...
void my_aluminium_nitride_properties(doublereal TiP,
	...
	// По данным программы SYMMIC.
	lam = static_cast<float>(319 * exp((-1.57)*log(TK / 300))); // 319 W/(m*K) при 300К
} // aluminium nitride
...
void update_temp_properties1(TEMPER &t, FLOW* &f, BLOCK* b, int lb, 
	...
	for (integer iP = iadd; iP<iadd+t.maxelm; iP++) {
		...
		doublereal Temperature_in_cell = 0.125*(temperature[nvtx_global[0][iP] - 1] +
												temperature[nvtx_global[1][iP] - 1] +
												temperature[nvtx_global[2][iP] - 1] + 
												temperature[nvtx_global[3][iP] - 1] + 
												temperature[nvtx_global[4][iP] - 1] +
												temperature[nvtx_global[5][iP] - 1] + 
												temperature[nvtx_global[6][iP] - 1] +
												temperature[nvtx_global[7][iP] - 1]);
		...
		if (matlist[b[ib].imatid].blibmat == 1) {
			...
			if (b[ib].itype == PHYSICS_TYPE_IN_BODY::SOLID) {
				...
				// проверка на допустимость температур.
				//diagnostic_critical_temperature(t.potent[iP - iadd], f, t, b, lb);
				diagnostic_critical_temperature(Temperature_in_cell, f, t, b, lb);

				

				// Не забываем ортотропность коэффициента теплопроводности.
				t.prop[MULT_LAM_X][iP - iadd] = matlist[b[ib].imatid].orthotropy_multiplyer_x;
				t.prop[MULT_LAM_Y][iP - iadd] = matlist[b[ib].imatid].orthotropy_multiplyer_y;
				t.prop[MULT_LAM_Z][iP - iadd] = matlist[b[ib].imatid].orthotropy_multiplyer_z;
...
float return_dynamic_viscosity(TPROP* matlist, integer imatid, 
	...
	case 1: // power-law fluid
		...
		if ((!bfirst_start)&&(SInvariantStrainRateTensor>0.0)) {
		   mu=static_cast<float>(fmax(matlist[imatid].mumin,fmin(2.0*matlist[imatid].Amu*exp((matlist[imatid].degreennmu-1.0)*log(SInvariantStrainRateTensor)),matlist[imatid].mumax)));
		}
		else {
	...
	case 2: // закон Кессона Caisson
		...
		if ((!bfirst_start)&&(SInvariantStrainRateTensor>0.0)) {
			if (matlist[imatid].Amu>=0.0) {
				mu= static_cast<float>(fmax(matlist[imatid].mumin,fmin(((sqrt(matlist[imatid].Amu/SInvariantStrainRateTensor))+matlist[imatid].Bmu)*((sqrt(matlist[imatid].Amu/SInvariantStrainRateTensor))+matlist[imatid].Bmu),matlist[imatid].mumax)));
			}
			else {
	...
	case 3: // закон Прандтля
		...
		if ((!bfirst_start)&&(SInvariantStrainRateTensor>0.0)) {
			if ( (fabs(matlist[imatid].Amu)>1e-30) && (fabs(matlist[imatid].Bmu)>1e-30) && ((SInvariantStrainRateTensor/matlist[imatid].Bmu>-1.0)&&(SInvariantStrainRateTensor/matlist[imatid].Bmu<1.0))) {
				mu= static_cast<float>(fmax(matlist[imatid].mumin,fmin(matlist[imatid].mumax,(matlist[imatid].Amu/SInvariantStrainRateTensor)*asin(SInvariantStrainRateTensor/matlist[imatid].Bmu))));
			}
			else {
	...
	case 4: // Carreau fluid
		mu= static_cast<float>(fmax(matlist[imatid].mumin,fmin(matlist[imatid].mumax,matlist[imatid].Amu+(matlist[imatid].Bmu-matlist[imatid].Amu)*exp(0.5*(matlist[imatid].degreennmu-1.0)*log(1.0+(matlist[imatid].Cmu*SInvariantStrainRateTensor)*(matlist[imatid].Cmu*SInvariantStrainRateTensor))))));
		break;
	case 5: // Пауэлл-Эйринг
		...
		if ((!bfirst_start)&&(SInvariantStrainRateTensor>0.0)) {
			...
			mu= static_cast<float>(fmax(matlist[imatid].mumin,fmin(matlist[imatid].mumax,matlist[imatid].Amu+(matlist[imatid].Bmu/SInvariantStrainRateTensor)*log(matlist[imatid].Cmu*SInvariantStrainRateTensor+sqrt(1.0+(matlist[imatid].Cmu*SInvariantStrainRateTensor)*(matlist[imatid].Cmu*SInvariantStrainRateTensor))))));
		} else 
		{
	...
	case 6: // Williamson non-Newtonian fluid
		...
		mu= static_cast<float>(fmax(matlist[imatid].mumin,fmin(matlist[imatid].mumax,matlist[imatid].Amu/(matlist[imatid].Bmu+SInvariantStrainRateTensor)+matlist[imatid].Cmu)));
		break;
...
void gran_prop_flow(TEMPER &t, FLOW* &f, BLOCK* b, integer lb, integer iflow,
	   ...
	   if (f[iflow].neighbors_for_the_internal_node[G][1] != nullptr) {
		   ...
		   if (iG >= f[iflow].maxelm) {
			   // это граничный узел
			   rho = 1.1614f; mu = static_cast < float>(1.84e-5); beta_t = 0.003331f; // инициализация default dry air 300K 1atm properties
			   if (matlist[b[ib].imatid].blibmat == 1) {
				    float  cp, lam;
				   cp = 1005.0f; lam = 0.025f;
				   // библиотечный находящийся внутри программы AliceFlow материал
