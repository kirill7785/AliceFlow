...
void my_elmatr_quad_PAm3(int iP, equation3D** &sl, equation3D_bon** &slb,  
	...
	*/
	...
	if (ISezai) {
	    ...
	    // Вообще говоря поле плотности также должно быть с предыдущей итерации.
		// Надо бы домножить на RCh.29.01.2021
	    Fe_sum += RCh * (1.0-alpha[VELOCITY_X_COMPONENT])*(mfcurrentretune[E_SIDE] - rhoe * SpeedCorOlde*dSqe - rhoe2 * SpeedCorOlde2*dSqe2 - rhoe3 * SpeedCorOlde3*dSqe3 - rhoe4 * SpeedCorOlde4*dSqe4);
	    Fn_sum += RCh * (1.0-alpha[VELOCITY_Y_COMPONENT])*(mfcurrentretune[N_SIDE] - rhon * SpeedCorOldn*dSqn - rhon2 * SpeedCorOldn2*dSqn2 - rhon3 * SpeedCorOldn3*dSqn3 - rhon4 * SpeedCorOldn4*dSqn4);
	    Ft_sum += RCh * (1.0-alpha[VELOCITY_Z_COMPONENT])*(mfcurrentretune[T_SIDE] - rhot * SpeedCorOldt*dSqt - rhot2 * SpeedCorOldt2*dSqt2 - rhot3 * SpeedCorOldt3*dSqt3 - rhot4 * SpeedCorOldt4*dSqt4);
	    Fw_sum += RCh * (1.0-alpha[VELOCITY_X_COMPONENT])*(mfcurrentretune[W_SIDE] - rhow * SpeedCorOldw*dSqw - rhow2 * SpeedCorOldw2*dSqw2 - rhow3 * SpeedCorOldw3*dSqw3 - rhow4 * SpeedCorOldw4*dSqw4);
	    Fs_sum += RCh * (1.0-alpha[VELOCITY_Y_COMPONENT])*(mfcurrentretune[S_SIDE] - rhos * SpeedCorOlds*dSqs - rhos2 * SpeedCorOlds2*dSqs2 - rhos3 * SpeedCorOlds3*dSqs3 - rhos4 * SpeedCorOlds4*dSqs4);
	    Fb_sum += RCh * (1.0-alpha[VELOCITY_Z_COMPONENT])*(mfcurrentretune[B_SIDE] - rhob * SpeedCorOldb*dSqb - rhob2 * SpeedCorOldb2*dSqb2 - rhob3 * SpeedCorOldb3*dSqb3 - rhob4 * SpeedCorOldb4*dSqb4);
