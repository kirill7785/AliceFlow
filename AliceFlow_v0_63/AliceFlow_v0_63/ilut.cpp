...
#include "iluk.cpp" // ILUK decomposition
	...
	if (lfil < 0) {
	   ...
	   for (ii=1; ii<=n; ++ii) {
		   ...
		   if (tnorm < 1e-30) {
			   ...
			   tnorm/= static_cast<doublereal>(j2-j1+1);
...
#pragma omp parallel shared(nd) private(i__, i__2,k)
					{
...
#pragma omp parallel shared(nd) private(i__, i__1,k)
					{
#pragma omp parallel sections 
						{
#pragma omp section
