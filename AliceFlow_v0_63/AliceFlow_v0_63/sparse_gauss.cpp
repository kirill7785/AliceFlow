...
void addValueIRow(IRow *xO, integer num, doublereal value)
{
	...
	// поиск ячейки с индексом num
	//i=search_i(xO->elm, xO->n, num);
	i = search_i_and_add(xO->elm, xO->n, num, value);// modify уже сделан
...
doublereal getValueIRow(IRow *xO, integer num)
{
   ...
   if (i!=-1) return static_cast<doublereal>(get_val(xO->elm, xO->n, num));
   return 0.0;
} // getValueIRow
...
void convertIMatrixtoCSIR_ILU_ITL(IMatrix *xO, doublereal* &U_val, integer* &U_ind, integer* &U_ptr, doublereal* &L_val, integer* &L_ind, integer* &L_ptr) {
	...
	if (ik < nz) {
		U_val[ik] = xO->dd[n - 1];
		U_ind[ik] = n - 1;
		U_ptr[n - 1] = (ik < U_ptr[n - 1] ? ik : U_ptr[n - 1]);
	}
	else {
	...
	if ((L_val != nullptr) && (L_ind != nullptr) && (L_ptr != nullptr)) {
		...
		if (ik < nz) {
			L_val[ik] = xO->dd[n - 1];
			L_ind[ik] = n - 1;
			L_ptr[n - 1] = (ik < U_ptr[n - 1] ? ik : U_ptr[n - 1]);
			ik++;
		}
		else {
