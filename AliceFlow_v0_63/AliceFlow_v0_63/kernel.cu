...
// 1. n шаговый метод c оптимальным параметром релаксации tau(i), i=1..n  нам каждом шаге. 
...
// Один раз вычисляется на каждом уровне и запоминается.
...
// На каждом уровне многосеточной иерархии свои верхняя и нижняя граница спектра. Уточнение нижней границы спектра 2 раза за V цикл (по разу на нисходящей и восходящей ветви V цикла).
// 5. Использование итерационного процесса Чебышева в качестве сглаживателя в алгебраическом многосеточном методе.
// 
...
int main_body(BLOCK*& b, int& lb, char ch_EXPORT_ALICE_ONLY = 'y') {
	...
	if (bCAD) {
		...
		bconstruction_union_now = new bool[lu + 1];
