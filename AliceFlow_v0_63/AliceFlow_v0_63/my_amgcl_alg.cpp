	...
	bool bprint_message)
{
	...
	const bool bprint_log = bprint_message;
...
*/
	...
	for (integer i = 0; i < maxelm; ++i) {
		...
		if (sl[i].iP == -1) {
			printf("bad diagonal in string %lld iP=%d\n", i, sl[i].iP);
			system("PAUSE");
		}
	...
	if (my_amg_manager.amgcl_selector == 0) {
					...
					for (ptrdiff_t j = row_jumper[i], e = row_jumper[i + 1]; j < e; ++j) {
						if (col_buffer[j] == i) {
							if (elements[j] <= 0.0) {
								...
								std::cout << elements[j] << " " << i << " " << n << " rhs=" << rhs[j] << std::endl;
								system("pause");
							}
		...
		if (my_amg_manager.amgcl_smoother == 2) {
			...
			*/
			...
			prm.solver.M = my_amg_manager.m_restart;
