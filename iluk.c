...
// —держит медленный линейный поиск и из-за этого непригодна.
/* ----------------------------------------------------------------------- */
...
/*     determine smallest column index */
	...
	i__2 = lenl;
	// ƒь€вольски медленный поиск минимума. Ёто линейный поиск.
	//printf("jj=%d\n",jj);// jj==1 далеко не всегда.
	// Ёто означает что нужно поддерживать удаление элемента по ключу.
	//getchar();
	for (j = jj + 1; j <= i__2; ++j) {
	    if (jw[j] < jrow) {
		jrow = jw[j];
...
// …. —аада на два потока. Ёто дало ускорение одной итерации SIMPLE алгоритма 
// на 18.4%. 
/* ----------------------------------------------------------------------- */
...
/* L1: */
	...
	if (bparallelizm_old) {
		//ILUK 839 +2
		//doublereal *w_dubl=new doublereal[n+2]; // +2 запас по пам€ти.
...
#pragma omp section
						{
							...
							jlu[nd.b0.iright_start + 1] = ju02start; // об€зательно важно.
...
#pragma omp critical
				...
				if (ierr_messag == 0) {
					...
					for (integer iscan_par = nd.b0.iright_start; iscan_par <= nd.b0.iright_finish; iscan_par++) {
						jw[iscan_par + 1] = jw_dubl[iscan_par + 1]; // модификаци€ рабочего массива.
						ju[iscan_par + 1] = ju[iscan_par + 1] - ju02start + ju0;
						jlu[iscan_par + 1] -= ju02start - ju0;
					}
					jlu[nd.b0.iright_finish + 2] = ju[nd.b0.iright_finish + 1]; // ¬от так наверно совсем правильно.
...
//	for (integer i54=0; i54<n+2; i54++) {
	...
	getchar();*//*
						...
						//ju[nd.b0.iright_finish+1]=0; // ¬от обрыв соответствующий даным из правилього файла.
