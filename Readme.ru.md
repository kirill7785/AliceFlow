...
5. Введите терминале g++ AliceFlow_v0_48.cpp 2> gcc_log.txt В результате появится файл a.exe.
6. Переименуйте a.exe в AliceFlow_v0_48.exe. 
7. Создайте на рабочем столе папку Alice_EXE. Поместите в папку Alice_EXE программу интерфейс AliceMesh_v0_45.exe написанную на Delphi 
...
8. На сайте GitHub в папке AliceEXE лежат примеры для программы AliceMesh_v0_45.exe. Поместите их в папку AliceEXE на рабочем столе.
9. Можно пользоваться. Запустите AliceMesh_v0_45.exe прочитайте один из примеров File->Read. Запустите пример Solve->Run.
Программа интерфейс AliceMesh_v0_45.exe автоматически вызовет программу решатель AliceFlow_v0_48.exe.
10. Установите программу tecplot360 или бесплатный аналог - программу ParaView. Откройте в tecplot360(или ParaView) файл с расширением .PLT
...
11. Для тех кто хочет использовать параллельную версию программы запустите в терминале mingw-get (Для этого нужно подключение к сети интернет
на вашем компьютере) и выберите в нём пакеты  mingw32-pthreads-w32.
12. После установки находясь в папке C:\src выполните g++ AliceFlow_v0_48.cpp -fopenmp 2> gcc_log.txt для создания параллельной версии программы.
