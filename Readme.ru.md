# AliceFlow_v0.49 - Программа для тепловых расчётов в 3D.

Программа Alice_Flow_v0.49 предназначена для расчёта поля температур в трёхмерных твердотельных моделях. В ряде случаев учитывается конвективный перенос теплоносителя. Учитываются нелинейности  различной природы. Поддерживается расчёт тепловой переходной характеристики. Для ускорения вычислений используется алгебраический многосеточный метод. Для сокращения числа ячеек сетки и ускорения нестационарных  расчётов реализованы Адаптивные Локально Измельчённые Сетки (АЛИС).
...
Рекомендуется использовать простые прямоугольные расчётные области.
...
* 3D cfd Полунеявная процедура для связывающих уравнений неразрывности и скорость-давление  (SIMPLE [1972]). Стационарный решатель.(Неастационарный гидродинамический решатель Future jan 2021).
* Поправка С.М. Рхи и У.Л. Чоу [1984] См. статью Ибрагима Сезая.
...
* Схемы высокой разрешающей способности для аппроксимации конвекции на структурированной неравномерной сетке: WACEB, SMARTER, SUPER-C и др. (А также для АЛИС сетки Future jan 2021)
...
модель зазора (зазоров) с теплообменом излучением между границами зазора. СЛАУ решается двумя алгебраическими многосеточными методами на выбор: amg1r5 или РУМБА.v.0.14.(Future jan 2021).
* Графический визуализатор на FWGL OpenGL на языке с++ встроенный в код программы. Визуализация результатов расчёта в 3D, анимация результатов нестационарного CFD расчёта. (Future jan 2021).
