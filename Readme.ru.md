...
Программа AliceFlow.v.0.48 решает систему уравнений в частных производных и для расчета пользователю необходимо задать свойства материалов (постоянные или зависящие от температуры):
...
загрузить из текстового файла load.txt, предварительно рассчитав его на основе встроенного SIMPLE алгоритма.
Поддерживаются также граничные условия Ньютна-Рихмана (обычно если поле скорости не задано) или Стефана-Больцмана (для теплообмена излучением).
