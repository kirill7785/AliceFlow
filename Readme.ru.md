...
* 3D cfd Полунеявная процедура для связывающих уравнений неразрывности и скорость-давление  (SIMPLE [1972]). Стационарный или неастационарный гидродинамический решатель доступны.
...
* Модель ламинарно-турбулентного перехода Ментора и Латгрии [2009] gamma-ReTheta-SST model. (Transition SST model 4 eqn). (Находится на этапе написания 23.01.2021).
...
* Графический визуализатор на FWGL OpenGL на языке с++ встроенный в код программы. Визуализация результатов расчёта в 3D, анимация результатов нестационарного CFD расчёта. 
...
(описание устарело. Проект собирается в Microsoft Visual Studio 2019. Библиотека FWGL OpenGL должна быть прописана в зависимостях.).
