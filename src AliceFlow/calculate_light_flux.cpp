...
void calculate_light_flux2(doublereal* & myF, TEMPER& t,
	BLOCK*& b, integer &lb) 
{
	doublereal* distance = new doublereal[static_cast<integer>(t.maxelm) + static_cast<integer>(t.maxbound)];
	for (integer i = 0; i < static_cast<integer>(t.maxelm); ++i) {
		myF[i] = 0.0;
	}
	for (integer ibid = 0; ibid < lb; ibid++) {
		...
		if (b[ibid].arr_Sc[0] > 1.0e-30) {
			...
			printf("x0=%e [m], y0=%e [m], z0=%e [m], maxelm=%d\n",x0,y0,z0, t.maxelm);
			for (integer i = 0; i < static_cast<integer>(t.maxelm); ++i) {
				TOCHKA p1;
...
void ray_tracing(TOCHKA p3, TOCHKA p0, TOCHKA p1, integer i0, integer i1, TEMPER& t, BLOCK*& b, doublereal radius2) {
	...
	if (visit_ray_traser != nullptr) delete[] visit_ray_traser;
	visit_ray_traser = nullptr;
...
void calculate_light_flux(doublereal*& myF, TEMPER& t,
	...
	size_cell = new doublereal[static_cast<integer>(t.maxelm)];

	stack_ray_trayser = new integer[static_cast<integer>(t.maxelm) + 2];
	doublereal* distance = new doublereal[static_cast<integer>(t.maxelm) + static_cast<integer>(t.maxbound)];
