...
#ifndef PAMENDMENTv_0_07_CPP
#define PAMENDMENTv_0_07_CPP 1
...
// 29.01.2021
doublereal interpolation_na_face(doublereal VG, doublereal VP, doublereal fgplus) {

	// Неработоспособность вариантов возникала при apvel вычисленных с учетом реверсированного знака.
	// Если apvel сумма внедиагональных коэффициентов то работают все формулы. 
	// Правильные формулы дают лучшую сходимость.
	// Никакой релаксации tau к своему среднему значению после сборки не применяется. 

	// Теоретически верный вариант на основе среднего геометрического, но дающий расходимость змеевика.
	return (VG * VP / (fgplus * VP + (1.0 - fgplus) * VG));// заниженное линейное

	// Неверное соотношение долго использованное в коде и дающее сходимость для zmeevik
	// 29.01.2021 Действительно zmeevik работает с этой формулой.
	//return (VG * VP / ( (1.0-fgplus) * VP + fgplus * VG));// завышенное линейное.

	// линейная
	// для плотности можно линейную схему.
	// Гораздо хуже сходится даже с apvel = sum anb 
	// не рекомендуется к использованию.
	//return VP * (1.0 - fgplus) + fgplus * VG;

	// обратная линейная Неверно. Хорошо работает.
	//return VP * fgplus + (1.0 - fgplus) * VG;
}


// 29.01.2021
doublereal interpolation_na_face_tau(doublereal VG, doublereal VP, doublereal fgplus, double Fg, integer igran) {
	...
	// Теоретически верный вариант на основе среднего геометрического, но дающий расходимость змеевика.
	// 26.04.2022 Верно порядок меняется при взвешенно обратных величинах. Сверено со статьёй I. Sezai.
	return (VG * VP / (fgplus * VP + (1.0 - fgplus) * VG));// заниженное линейное
	...
	// Это схема против потока. НЕ Работает.29.01.2021
	/*
	if ((igran == E_SIDE)||(igran == N_SIDE)||(igran == T_SIDE)) {
		if (Fg > 0.0) {
			return VP;
		}
		else {
	...
	if ((igran == W_SIDE) || (igran == S_SIDE) || (igran == B_SIDE)) {
		if (Fg > 0.0) {
			return VG;
		}
		else {
	        ...
	        switch (border_neighbor[inumber].Norm) {
		        case E_SIDE:
					...
					*/
					...
					//system("pause"); // debug внутренняя нормаль
...
void my_elmatr_quad_PAm_bon3(equation3D_bon** &slb, equation3D** sl, 
							...
							integer ls, integer lw, WALL* w, bool bDirichlet, 
							int*** neighbors_for_the_internal_node, doublereal RCh,
							bool &breversedflow, doublereal** tau, int &inumber_PAM_Dirichlet_bc) {
	...
	if ((border_neighbor[inumber].MCB >= ls) && (border_neighbor[inumber].MCB<(ls + lw)) && 
		(w[border_neighbor[inumber].MCB - ls].bpressure || w[border_neighbor[inumber].MCB - ls].bopening)) {
			...
			if (!bFReeStyle) {
			   ...
			   // doublereal kineticenergy=0.0; // кинетическая энергия потока: 0.5*rho*Vel!2.
			    ...
			    if (bDirichlet) {
					...
					// Внимание нулевая поправка давления на выходе при любых условиях.
					inumber_PAM_Dirichlet_bc++;
					slb[PAM][inumber].aw = 1.0;
					slb[PAM][inumber].ai = 0.0;
					slb[PAM][inumber].b = 0.0;// Нулевая поправка давления на выходе и точка.
					slb[PAM][inumber].iI = -1; // не присутствует в матрице
					slb[PAM][inumber].iW = border_neighbor[inumber].iB;
			    	...
			    	switch (ioutflowcondition) {
				     case PRESSUREOUTLET: // поправка давления фиксированна и равна нулю:
		                              ...
		                              slb[PAM][inumber].iI=-1; // не присутствует в матрице
		                              slb[PAM][inumber].iW=border_neighbor[inumber].iB;
									 // printf("%d\n",slb[PAM][inumber].iW); system("pause");// больше либо равно maxelm
					                  break;
			      	 case BERNULLI: // Рассматривается выходная граница потока и условие для поправки давления для неё.
					      // Если жидкость вытекает через выходную границу то используется постоянное давление rCe на выходной границе.
					      // Т.е. при наличии вытекания жидкости из расчётной области через выходную границу используется стандартное 
					      ...
					      if (rsign>=0.0) {
		                     ...
		                     slb[PAM][inumber].iI=-1; // не присутствует в матрице
		                     slb[PAM][inumber].iW=border_neighbor[inumber].iB; 
							 //std::cout << "pokidaeet b=" << slb[PAM][inumber].b << std::endl;
						  }
						  else {
		                      ...
		                      slb[PAM][inumber].iI=-1; // не присутствует в матрице
		                      slb[PAM][inumber].iW=border_neighbor[inumber].iB; 
							  //std::cout << "pronikaet b=" << slb[PAM][inumber].b << std::endl;
						  }
						  //system("pause");
					      break;
				     default: // поправка давления фиксированна и равна нулю:
					 ...
					 if (w[border_neighbor[inumber].MCB - ls].bopening) {
						 ...
						 case BERNULLI:
							 ...
							 if (rsign >= 0.0) {
								 ...
								 for (int i_7 = 0; i_7 < lw; i_7++) {
									 if (w[i_7].Vx * w[i_7].Vx + w[i_7].Vy * w[i_7].Vy + w[i_7].Vz * w[i_7].Vz > 1.0e-30) {
										 // Задача с вынужденной конвекцией.
										 bOk = false;
									 }
								 }
	     ...
	     else if ((!bDirichlet) /*&& !((inumber==(maxbound-1)) && bPfix && (!bFReeStyle))*/ &&
		 !((border_neighbor[inumber].MCB>=ls) && (border_neighbor[inumber].MCB<(ls+lw)) &&
	    ...
	    {  
			...
			*/
			...
			if (1||b_on_adaptive_local_refinement_mesh) {
				// Самое простое условие однородное условие Неймана.
	...
	*/
	...
	if (iG > -1) {
		...
		else
		{
			...
			doublereal ug_sign = 0.0;
			if (!bG) {
				ug_sign = fgplus * VG + (1.0 - fgplus) * VP;
			}
			else {
				ug_sign = VG;
			}
			doublereal taug = interpolation_na_face_tau(tau[VGid][iG], tau[VGid][iP],fgplus,ug_sign,G);//29.01.2021
			//28.07.2016
			if (bG) {
				taug = tau[VGid][iG];
			}
			...
			if (!bG) {
				...
				if (bRhieChowi) {
					...
					// taug уже содержит в себе alpha_u
	                    ...
	                    BOUND* border_neighbor, integer *ilevel_alice, int* ptr)
{
	...
	if (ISezai) {
		...
		//std::cout << "return mass flux:\n";
		//getchar();

		//debug
		// if ((iW >= maxelm) && (pa[iP].x < -0.0356)) {
			//if (fabs(1.0 - Fw / (rP * dy * dz)) > 1.0e-3) 
			//{
				//std::cout << "return mass flux:\n";
				//std::cout << mfcurrentretune[W_SIDE] / (rP * dy * dz) << std::endl;
				//std::cout << Fw_sum / (rP * dy * dz) << std::endl;
				//std::cout << mfold[W_SIDE] / (rP * dy * dz) << std::endl;
				//getchar();
			//}
		//}
	                    ...
	                    BOUND* border_neighbor, integer *ilevel_alice, int* ptr)
{
	...
	if (iE > -1) {
		if (!bE) {
			rhoe = interpolation_na_face(rE, rP, feplus);
			//rhoe = rE * rP / (feplus * rP + (1.0 - feplus) * rE);
		}
		else rhoe = rE; // проверено !
	}
	if (iW > -1) {
		if (!bW) {
			rhow = interpolation_na_face(rW, rP, fwplus);
	...
	if (iN > -1) {
		if (!bN) {
			rhon = interpolation_na_face(rN, rP, fnplus);
	...
	if (iS > -1) {
		if (!bS) {
			rhos = interpolation_na_face(rS, rP, fsplus);
	...
	if (iT > -1) {
		if (!bT) {
			rhot = interpolation_na_face(rT, rP, ftplus);
			//rhot = rT * rP / (ftplus * rP + (1.0 - ftplus) * rT);
		}
		else rhot = rT;
	}
	if (iB > -1) {
		if (!bB) {
			rhob = interpolation_na_face(rB, rP, fbplus);
			//rhob = rB * rP / (fbplus * rP + (1.0 - fbplus) * rB);
		}
		else rhob = rB;
	}
	...
	if (b_on_adaptive_local_refinement_mesh) {
		...
		if (iE2 > -1) {
			if (!bE2) {
				rhoe2 = interpolation_na_face(rE2, rP, feplus2);
				//rhoe2 = rE2 * rP / (feplus2 * rP + (1.0 - feplus2) * rE2);
			}
			else rhoe2 = rE2; // проверено !
		}
		if (iW2 > -1) {
			if (!bW2) {
				rhow2 = interpolation_na_face(rW2, rP, fwplus2);
				//rhow2 = rW2 * rP / (fwplus2 * rP + (1.0 - fwplus2) * rW2);
			}
			else rhow2 = rW2;
		}
		if (iN2 > -1) {
			if (!bN2) {
				rhon2 = interpolation_na_face(rN2, rP, fnplus2);
				//rhon2 = rN2 * rP / (fnplus2 * rP + (1.0 - fnplus2) * rN2);
			}
			else rhon2 = rN2;
		}
		if (iS2 > -1) {
			if (!bS2) {
				rhos2 = interpolation_na_face(rS2, rP, fsplus2);
				//rhos2 = rS2 * rP / (fsplus2 * rP + (1.0 - fsplus2) * rS2);
			}
			else rhos2 = rS2;
		}
		if (iT2 > -1) {
			if (!bT2) {
				rhot2 = interpolation_na_face(rT2, rP, ftplus2);
				//rhot2 = rT2 * rP / (ftplus2 * rP + (1.0 - ftplus2) * rT2);
			}
			else rhot2 = rT2;
		}
		if (iB2 > -1) {
			if (!bB2) {
				rhob2 = interpolation_na_face(rB2, rP, fbplus2);
				//rhob2 = rB2 * rP / (fbplus2 * rP + (1.0 - fbplus2) * rB2);
			}
			else rhob2 = rB2;
		}
		...
		if (iE3 > -1) {
			if (!bE3) {
				rhoe3 = interpolation_na_face(rE3, rP, feplus3);
				//rhoe3 = rE3 * rP / (feplus3 * rP + (1.0 - feplus3) * rE3);
			}
			else rhoe3 = rE3;
		}
		if (iW3 > -1) {
			if (!bW3) {
				rhow3 = interpolation_na_face(rW3, rP, fwplus3);
				//rhow3 = rW3 * rP / (fwplus3 * rP + (1.0 - fwplus3) * rW3);
			}
			else rhow3 = rW3;
		}
		if (iN3 > -1) {
			if (!bN3) {
				rhon3 = interpolation_na_face(rN3, rP, fnplus3);
				//rhon3 = rN3 * rP / (fnplus3 * rP + (1.0 - fnplus3) * rN3);
			}
			else rhon3 = rN3;
		}
		if (iS3 > -1) {
			if (!bS3) {
				rhos3 = interpolation_na_face(rS3, rP, fsplus3);
				//rhos3 = rS3 * rP / (fsplus3 * rP + (1.0 - fsplus3) * rS3);
			}
			else rhos3 = rS3;
		}
		if (iT3 > -1) {
			if (!bT3) {
				rhot3 = interpolation_na_face(rT3, rP, ftplus3);
				//rhot3 = rT3 * rP / (ftplus3 * rP + (1.0 - ftplus3) * rT3);
			}
			else rhot3 = rT3;
		}
		if (iB3 > -1) {
			if (!bB3) {
				rhob3 = interpolation_na_face(rB3, rP, fbplus3);
				//rhob3 = rB3 * rP / (fbplus3 * rP + (1.0 - fbplus3) * rB3);
			}
			else rhob3 = rB3;
		}
		...
		if (iE4 > -1) {
			if (!bE4) {
				rhoe4 = interpolation_na_face(rE4, rP, feplus4);
				//rhoe4 = rE4 * rP / (feplus4 * rP + (1.0 - feplus4) * rE4);
			}
			else rhoe4 = rE4;
		}
		if (iW4 > -1) {
			if (!bW4) {
				rhow4 = interpolation_na_face(rW4, rP, fwplus4);
				//rhow4 = rW4 * rP / (fwplus4 * rP + (1.0 - fwplus4) * rW4);
			}
			else rhow4 = rW4;
		}
		if (iN4 > -1) {
			if (!bN4) {
				rhon4 = interpolation_na_face(rN4, rP, fnplus4);
				//rhon4 = rN4 * rP / (fnplus4 * rP + (1.0 - fnplus4) * rN4);
			}
			else rhon4 = rN4;
		}
		if (iS4 > -1) {
			if (!bS4) {
				rhos4 = interpolation_na_face(rS4, rP, fsplus4);
				//rhos4 = rS4 * rP / (fsplus4 * rP + (1.0 - fsplus4) * rS4);
			}
			else rhos4 = rS4;
		}
		if (iT4 > -1) {
			if (!bT4) {
				rhot4 = interpolation_na_face(rT4, rP, ftplus4);
				//rhot4 = rT4 * rP / (ftplus4 * rP + (1.0 - ftplus4) * rT4);
			}
			else rhot4 = rT4;
		}
		if (iB4 > -1) {
			if (!bB4) {
				rhob4 = interpolation_na_face(rB4, rP, fbplus4);
				//rhob4 = rB4 * rP / (fbplus4 * rP + (1.0 - fbplus4) * rB4);
			}
			else rhob4 = rB4;
		}
	...
	if (ISezai) {
	    ...
	    // С включённой поправкой (дополнительная нижняя релаксация) из статьи I. Sezai. !!!
	   // mfold - значение массового потока с предыдущей итерации.
		// Надо бы домножить на RCh.29.01.2021
        mfcurrentretune[E_SIDE]=Fe_sum + RCh * (1.0-alpha[VELOCITY_X_COMPONENT])*(mfold[E_SIDE]-rhoe*SpeedCorOlde*dSqe - rhoe2 * SpeedCorOlde2*dSqe2 - rhoe3 * SpeedCorOlde3*dSqe3 - rhoe4 * SpeedCorOlde4*dSqe4);
	    mfcurrentretune[N_SIDE]=Fn_sum + RCh * (1.0-alpha[VELOCITY_Y_COMPONENT])*(mfold[N_SIDE]-rhon*SpeedCorOldn*dSqn - rhon2 * SpeedCorOldn2*dSqn2 - rhon3 * SpeedCorOldn3*dSqn3 - rhon4 * SpeedCorOldn4*dSqn4);
	    mfcurrentretune[T_SIDE]=Ft_sum + RCh * (1.0-alpha[VELOCITY_Z_COMPONENT])*(mfold[T_SIDE]-rhot*SpeedCorOldt*dSqt - rhot2 * SpeedCorOldt2*dSqt2 - rhot3 * SpeedCorOldt3*dSqt3 - rhot4 * SpeedCorOldt4*dSqt4);
	    mfcurrentretune[W_SIDE]=Fw_sum + RCh * (1.0-alpha[VELOCITY_X_COMPONENT])*(mfold[W_SIDE]-rhow*SpeedCorOldw*dSqw - rhow2 * SpeedCorOldw2*dSqw2 - rhow3 * SpeedCorOldw3*dSqw3 - rhow4 * SpeedCorOldw4*dSqw4);
	    mfcurrentretune[S_SIDE]=Fs_sum + RCh * (1.0-alpha[VELOCITY_Y_COMPONENT])*(mfold[S_SIDE]-rhos*SpeedCorOlds*dSqs - rhos2 * SpeedCorOlds2*dSqs2 - rhos3 * SpeedCorOlds3*dSqs3 - rhos4 * SpeedCorOlds4*dSqs4);
	    mfcurrentretune[B_SIDE]=Fb_sum + RCh * (1.0-alpha[VELOCITY_Z_COMPONENT])*(mfold[B_SIDE]-rhob*SpeedCorOldb*dSqb - rhob2 * SpeedCorOldb2*dSqb2 - rhob3 * SpeedCorOldb3*dSqb3 - rhob4 * SpeedCorOldb4*dSqb4);
	}
...
// поправки давления
void my_elmatr_quad_PAm(int iP, equation3D** &sl, equation3D_bon** &slb,  
						doublereal** potent, TOCHKA* pa, float** prop, float** prop_b,
						int** nvtx, int*** neighbors_for_the_internal_node, int maxelm, doublereal **diag_coef,
...
// реализовано 22 июня 2012 года.
void my_elmatr_quad_PAm2(int iP, equation3D** &sl, equation3D_bon** &slb,  
						doublereal** potent, TOCHKA* pa, float** prop, float** prop_b,
						int** nvtx, int*** neighbors_for_the_internal_node, int maxelm,
						doublereal* alpha, doublereal dbeta, doublereal** &rhie_chow, doublereal RCh,
						bool btimedepend, doublereal dtimestep, doublereal* mfoldtimestep,
...
void my_elmatr_quad_PAm3(int iP, equation3D** &sl, equation3D_bon** &slb,  
						doublereal** potent, TOCHKA* pa, float** prop, float** prop_b,
						int** nvtx, int*** neighbors_for_the_internal_node, int maxelm,
						doublereal* alpha, doublereal dbeta, doublereal** &rhie_chow, doublereal RCh,
						bool btimedepend, doublereal dtimestep, doublereal* mfoldtimestep,
	                    ...
	                    BOUND* border_neighbor, integer *ilevel_alice, int* ptr, int maxbound,
	                    doublereal** sumanb, integer inum_iter_loc) {

	// inum_iter_loc - Номер глобальной итерации SIMPLE алгоритма.

	if (iP > maxelm + maxbound) {
		printf("my_elmatr_quad_PAm3 iP=%d maxelm=%d maxbound=%d\n", iP, maxelm, maxbound);
		system("pause");
	}
	...
	*/
	...
	int iE4 = -1, iN4 = -1, iT4 = -1, iW4 = -1, iS4 = -1, iB4 = -1; // номера соседних контрольных объёмов
										  ...
										  // -1 если не используется и [0..maxelm+maxbound-1] если используется.
	if (b_on_adaptive_local_refinement_mesh) {
		iE2 = neighbors_for_the_internal_node[E_SIDE][1][iP]; iN2 = neighbors_for_the_internal_node[N_SIDE][1][iP];
		iT2 = neighbors_for_the_internal_node[T_SIDE][1][iP];
		iW2 = neighbors_for_the_internal_node[W_SIDE][1][iP]; iS2 = neighbors_for_the_internal_node[S_SIDE][1][iP];
		iB2 = neighbors_for_the_internal_node[B_SIDE][1][iP];
		iE3 = neighbors_for_the_internal_node[E_SIDE][2][iP]; iN3 = neighbors_for_the_internal_node[N_SIDE][2][iP];
		iT3 = neighbors_for_the_internal_node[T_SIDE][2][iP];
		iW3 = neighbors_for_the_internal_node[W_SIDE][2][iP]; iS3 = neighbors_for_the_internal_node[S_SIDE][2][iP]; 
		iB3 = neighbors_for_the_internal_node[B_SIDE][2][iP];
		iE4 = neighbors_for_the_internal_node[E_SIDE][3][iP]; iN4 = neighbors_for_the_internal_node[N_SIDE][3][iP]; 
		iT4 = neighbors_for_the_internal_node[T_SIDE][3][iP];
		iW4 = neighbors_for_the_internal_node[W_SIDE][3][iP]; iS4 = neighbors_for_the_internal_node[S_SIDE][3][iP]; 
	...
	*/
	...
	if (iE > -1) {
		if (!bE) {
		ae = interpolation_na_face(sumanb[VX][iE],sumanb[VX][iP], feplus);
		//ae = sumanb[VX][iE] * sumanb[VX][iP] / (feplus * sumanb[VX][iP] + (1.0 - feplus) * sumanb[VX][iE]);
		}
		else ae = sumanb[VX][iE]; // проверено !
	}
	if (iW > -1) {
		if (!bW) {
		aw = interpolation_na_face(sumanb[VX][iW],sumanb[VX][iP], fwplus);
		//aw = sumanb[VX][iW] * sumanb[VX][iP] / (fwplus * sumanb[VX][iP] + (1.0 - fwplus) * sumanb[VX][iW]);
		}
		else aw = sumanb[VX][iW]; // проверено !
	}
	if (iN > -1) {
		if (!bN) {
		an = interpolation_na_face(sumanb[VY][iN],sumanb[VY][iP], fnplus);
		//an = sumanb[VY][iN] * sumanb[VY][iP] / (fnplus * sumanb[VY][iP] + (1.0 - fnplus) * sumanb[VY][iN]);
		}
		else an = sumanb[VY][iN]; // проверено !
	}
	if (iS > -1) {
		if (!bS) {
		as = interpolation_na_face(sumanb[VY][iS],sumanb[VY][iP], fsplus);
		//as = sumanb[VY][iS] * sumanb[VY][iP] / (fsplus * sumanb[VY][iP] + (1.0 - fsplus) * sumanb[VY][iS]);
		}
		else as = sumanb[VY][iS]; // проверено !
	}
	if (iT > -1) {
		if (!bT) {
		at = interpolation_na_face(sumanb[VZ][iT],sumanb[VZ][iP], ftplus);
		//at = sumanb[VZ][iT] * sumanb[VZ][iP] / (ftplus * sumanb[VZ][iP] + (1.0 - ftplus) * sumanb[VZ][iT]); 
		}
		else at = sumanb[VZ][iT]; // проверено !
	}
	if (iB > -1) {
		if (!bB) {
		ab = interpolation_na_face(sumanb[VZ][iB],sumanb[VZ][iP], fbplus);
		//ab = sumanb[VZ][iB] * sumanb[VZ][iP] / (fbplus * sumanb[VZ][iP] + (1.0 - fbplus) * sumanb[VZ][iB]);
		}
		else ab = sumanb[VZ][iB]; // проверено !
	}

	doublereal ae2, aw2, as2, an2, at2, ab2;
	if (iE2 > -1) {
		if (!bE2) {
		ae2 = interpolation_na_face(sumanb[VX][iE2],sumanb[VX][iP], feplus2);
		//ae2 = sumanb[VX][iE2] * sumanb[VX][iP] / (feplus2 * sumanb[VX][iP] + (1.0 - feplus2) * sumanb[VX][iE2]); 
		}
		else ae2 = sumanb[VX][iE2]; // проверено !
	}
	if (iW2 > -1) {
		if (!bW2) {
		aw2 = interpolation_na_face(sumanb[VX][iW2],sumanb[VX][iP], fwplus2);
		//aw2 = sumanb[VX][iW2] * sumanb[VX][iP] / (fwplus2 * sumanb[VX][iP] + (1.0 - fwplus2) * sumanb[VX][iW2]); 
		}
		else aw2 = sumanb[VX][iW2]; // проверено !
	}
	if (iN2 > -1) {
		if (!bN2) {
		an2 = interpolation_na_face(sumanb[VY][iN2],sumanb[VY][iP], fnplus2);
		//an2 = sumanb[VY][iN2] * sumanb[VY][iP] / (fnplus2 * sumanb[VY][iP] + (1.0 - fnplus2) * sumanb[VY][iN2]); 
		}
		else an2 = sumanb[VY][iN2]; // проверено !
	}
	if (iS2 > -1) {
		if (!bS2) {
		as2 = interpolation_na_face(sumanb[VY][iS2],sumanb[VY][iP], fsplus2);
		//as2 = sumanb[VY][iS2] * sumanb[VY][iP] / (fsplus2 * sumanb[VY][iP] + (1.0 - fsplus2) * sumanb[VY][iS2]); 
		}
		else as2 = sumanb[VY][iS2]; // проверено !
	}
	if (iT2 > -1) {
		if (!bT2) {
		at2 = interpolation_na_face(sumanb[VZ][iT2],sumanb[VZ][iP], ftplus2);
		//at2 = sumanb[VZ][iT2] * sumanb[VZ][iP] / (ftplus2 * sumanb[VZ][iP] + (1.0 - ftplus2) * sumanb[VZ][iT2]);
		}
		else at2 = sumanb[VZ][iT2]; // проверено !
	}
	if (iB2 > -1) {
		if (!bB2) {
		ab2 = interpolation_na_face(sumanb[VZ][iB2],sumanb[VZ][iP], fbplus2);
		//ab2 = sumanb[VZ][iB2] * sumanb[VZ][iP] / (fbplus2 * sumanb[VZ][iP] + (1.0 - fbplus2) * sumanb[VZ][iB2]); 
		}
		else ab2 = sumanb[VZ][iB2]; // проверено !
	}

	doublereal ae3, aw3, as3, an3, at3, ab3;
	if (iE3 > -1) {
		if (!bE3) {
		ae3 = interpolation_na_face(sumanb[VX][iE3],sumanb[VX][iP], feplus3);
		//ae3 = sumanb[VX][iE3] * sumanb[VX][iP] / (feplus3 * sumanb[VX][iP] + (1.0 - feplus3) * sumanb[VX][iE3]); 
		}
		else ae3 = sumanb[VX][iE3]; // проверено !
	}
	if (iW3 > -1) {
		if (!bW3){
		aw3 = interpolation_na_face(sumanb[VX][iW3],sumanb[VX][iP], fwplus3);
		//aw3 = sumanb[VX][iW3] * sumanb[VX][iP] / (fwplus3 * sumanb[VX][iP] + (1.0 - fwplus3) * sumanb[VX][iW3]);
		}
		else aw3 = sumanb[VX][iW3]; // проверено !
	}
	if (iN3 > -1) {
		if (!bN3) {
		an3 = interpolation_na_face(sumanb[VY][iN3],sumanb[VY][iP], fnplus3);
		//an3 = sumanb[VY][iN3] * sumanb[VY][iP] / (fnplus3 * sumanb[VY][iP] + (1.0 - fnplus3) * sumanb[VY][iN3]); 
		}
		else an3 = sumanb[VY][iN3]; // проверено !
	}
	if (iS3 > -1) {
		if (!bS3) {
		as3 = interpolation_na_face(sumanb[VY][iS3],sumanb[VY][iP], fsplus3);
		//as3 = sumanb[VY][iS3] * sumanb[VY][iP] / (fsplus3 * sumanb[VY][iP] + (1.0 - fsplus3) * sumanb[VY][iS3]);
		}
		else as3 = sumanb[VY][iS3]; // проверено !
	}
	if (iT3 > -1) {
		if (!bT3) {
		at3 = interpolation_na_face(sumanb[VZ][iT3],sumanb[VZ][iP], ftplus3);
		//at3 = sumanb[VZ][iT3] * sumanb[VZ][iP] / (ftplus3 * sumanb[VZ][iP] + (1.0 - ftplus3) * sumanb[VZ][iT3]); 
		}
		else at3 = sumanb[VZ][iT3]; // проверено !
	}
	if (iB3 > -1) {
		if (!bB3) {
		ab3 = interpolation_na_face(sumanb[VZ][iB3],sumanb[VZ][iP], fbplus3);
		//ab3 = sumanb[VZ][iB3] * sumanb[VZ][iP] / (fbplus3 * sumanb[VZ][iP] + (1.0 - fbplus3) * sumanb[VZ][iB3]); 
		}
		else ab3 = sumanb[VZ][iB3]; // проверено !
	}

	doublereal ae4, aw4, as4, an4, at4, ab4;
	if (iE4 > -1) {
		if (!bE4) {
		ae4 = interpolation_na_face(sumanb[VX][iE4],sumanb[VX][iP], feplus4);
		//ae4 = sumanb[VX][iE4] * sumanb[VX][iP] / (feplus4 * sumanb[VX][iP] + (1.0 - feplus4) * sumanb[VX][iE4]); 
		}
		else ae4 = sumanb[VX][iE4]; // проверено !
	}
	if (iW4 > -1) {
		if (!bW4) {
		aw4 = interpolation_na_face(sumanb[VX][iW4],sumanb[VX][iP], fwplus4);
		//aw4 = sumanb[VX][iW4] * sumanb[VX][iP] / (fwplus4 * sumanb[VX][iP] + (1.0 - fwplus4) * sumanb[VX][iW4]);
		}
		else aw4 = sumanb[VX][iW4]; // проверено !
	}
	if (iN4 > -1) {
		if (!bN4) {
		an4 = interpolation_na_face(sumanb[VY][iN4],sumanb[VY][iP], fnplus4);
		//an4 = sumanb[VY][iN4] * sumanb[VY][iP] / (fnplus4 * sumanb[VY][iP] + (1.0 - fnplus4) * sumanb[VY][iN4]); 
		}
		else an4 = sumanb[VY][iN4]; // проверено !
	}
	if (iS4 > -1) {
		if (!bS4) {
		as4 = interpolation_na_face(sumanb[VY][iS4],sumanb[VY][iP], fsplus4);
		//as4 = sumanb[VY][iS4] * sumanb[VY][iP] / (fsplus4 * sumanb[VY][iP] + (1.0 - fsplus4) * sumanb[VY][iS4]);
		}
		else as4 = sumanb[VY][iS4]; // проверено !
	}
	if (iT4 > -1) {
		if (!bT4) {
		at4 = interpolation_na_face(sumanb[VZ][iT4],sumanb[VZ][iP], ftplus4);
		//at4 = sumanb[VZ][iT4] * sumanb[VZ][iP] / (ftplus4 * sumanb[VZ][iP] + (1.0 - ftplus4) * sumanb[VZ][iT4]); 
		}
		else at4 = sumanb[VZ][iT4]; // проверено !
	}
	if (iB4 > -1) {
		if (!bB4) {
		ab4 = interpolation_na_face(sumanb[VZ][iB4],sumanb[VZ][iP], fbplus4);
		//ab4 = sumanb[VZ][iB4] * sumanb[VZ][iP] / (fbplus4 * sumanb[VZ][iP] + (1.0 - fbplus4) * sumanb[VZ][iB4]);
		}
		else ab4 = sumanb[VZ][iB4]; // проверено !
	}
	*/
	...
	*/
	...
	if (ISezai) {
	    ...
	    // Вообще говоря поле плотности также должно быть с предыдущей итерации.
		// Надо бы домножить на RCh.29.01.2021 ????
		// Дивергенция скорости должна быть дивергенцией скорости, зачём её домножать на RCh=0.1 непонятно это испортит баланс.
		doublereal RCh_mult = 1.0; // RCh
	    Fe_sum += RCh_mult * (1.0-alpha[VELOCITY_X_COMPONENT])*(mfcurrentretune[E_SIDE] - rhoe * SpeedCorOlde*dSqe - rhoe2 * SpeedCorOlde2*dSqe2 - rhoe3 * SpeedCorOlde3*dSqe3 - rhoe4 * SpeedCorOlde4*dSqe4);
	    Fn_sum += RCh_mult * (1.0-alpha[VELOCITY_Y_COMPONENT])*(mfcurrentretune[N_SIDE] - rhon * SpeedCorOldn*dSqn - rhon2 * SpeedCorOldn2*dSqn2 - rhon3 * SpeedCorOldn3*dSqn3 - rhon4 * SpeedCorOldn4*dSqn4);
	    Ft_sum += RCh_mult * (1.0-alpha[VELOCITY_Z_COMPONENT])*(mfcurrentretune[T_SIDE] - rhot * SpeedCorOldt*dSqt - rhot2 * SpeedCorOldt2*dSqt2 - rhot3 * SpeedCorOldt3*dSqt3 - rhot4 * SpeedCorOldt4*dSqt4);
	    Fw_sum += RCh_mult * (1.0-alpha[VELOCITY_X_COMPONENT])*(mfcurrentretune[W_SIDE] - rhow * SpeedCorOldw*dSqw - rhow2 * SpeedCorOldw2*dSqw2 - rhow3 * SpeedCorOldw3*dSqw3 - rhow4 * SpeedCorOldw4*dSqw4);
	    Fs_sum += RCh_mult * (1.0-alpha[VELOCITY_Y_COMPONENT])*(mfcurrentretune[S_SIDE] - rhos * SpeedCorOlds*dSqs - rhos2 * SpeedCorOlds2*dSqs2 - rhos3 * SpeedCorOlds3*dSqs3 - rhos4 * SpeedCorOlds4*dSqs4);
	    Fb_sum += RCh_mult * (1.0-alpha[VELOCITY_Z_COMPONENT])*(mfcurrentretune[B_SIDE] - rhob * SpeedCorOldb*dSqb - rhob2 * SpeedCorOldb2*dSqb2 - rhob3 * SpeedCorOldb3*dSqb3 - rhob4 * SpeedCorOldb4*dSqb4);
	...
	// В статье Ибрагима Сезая (ap)e находится средним геометрическим, значит
	// и псевдовремя tau на грани должно находится средним геометрическим. 29.01.2021
	doublereal taue = 1.0, tauw = 1.0, taun = 1.0, taus = 1.0, taut = 1.0, taub = 1.0;
	if (iE > -1) {
		if (!bE) {
			taue = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iE], tau[VELOCITY_X_COMPONENT][iP], feplus, Fe_sum, E_SIDE);
			//taue = tau[VELOCITY_X_COMPONENT][iE] * tau[VELOCITY_X_COMPONENT][iP] / (feplus * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - feplus) * tau[VELOCITY_X_COMPONENT][iE]);
		}
		else taue = tau[VELOCITY_X_COMPONENT][iE]; // проверено !
	}
	if (iW > -1) {
		if (!bW) {
			tauw = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iW], tau[VELOCITY_X_COMPONENT][iP], fwplus, Fw_sum, W_SIDE);
			//tauw = tau[VELOCITY_X_COMPONENT][iW] * tau[VELOCITY_X_COMPONENT][iP] / (fwplus * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - fwplus) * tau[VELOCITY_X_COMPONENT][iW]);
		}
		else tauw = tau[VELOCITY_X_COMPONENT][iW];
	}
	if (iN > -1) {
		if (!bN) {
			taun = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iN], tau[VELOCITY_Y_COMPONENT][iP], fnplus, Fn_sum, N_SIDE);
			//taun = tau[VELOCITY_Y_COMPONENT][iN] * tau[VELOCITY_Y_COMPONENT][iP] / (fnplus * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fnplus) * tau[VELOCITY_Y_COMPONENT][iN]);
		}
		else taun = tau[VELOCITY_Y_COMPONENT][iN];
	}
	if (iS > -1) {
		if (!bS) {
			taus = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iS], tau[VELOCITY_Y_COMPONENT][iP], fsplus, Fs_sum, S_SIDE);
			//taus = tau[VELOCITY_Y_COMPONENT][iS] * tau[VELOCITY_Y_COMPONENT][iP] / (fsplus * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fsplus) * tau[VELOCITY_Y_COMPONENT][iS]);
		}
		else taus = tau[VELOCITY_Y_COMPONENT][iS];
	}
	if (iT > -1) {
		if (!bT) {
			taut = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iT], tau[VELOCITY_Z_COMPONENT][iP], ftplus, Ft_sum, T_SIDE);
			//taut = tau[VELOCITY_Z_COMPONENT][iT] * tau[VELOCITY_Z_COMPONENT][iP] / (ftplus * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - ftplus) * tau[VELOCITY_Z_COMPONENT][iT]);
		}
		else taut = tau[VELOCITY_Z_COMPONENT][iT];
	}
	if (iB > -1) {
		if (!bB) {
			taub = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iB], tau[VELOCITY_Z_COMPONENT][iP], fbplus, Fb_sum, B_SIDE);
			//taub = tau[VELOCITY_Z_COMPONENT][iB] * tau[VELOCITY_Z_COMPONENT][iP] / (fbplus * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - fbplus) * tau[VELOCITY_Z_COMPONENT][iB]);
		}
		else taub = tau[VELOCITY_Z_COMPONENT][iB];
	}
	...
	if (b_on_adaptive_local_refinement_mesh) {
		...
		if (iE2 > -1) {
			if (!bE2) {
				taue2 = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iE2], tau[VELOCITY_X_COMPONENT][iP], feplus2, Fe_sum , E_SIDE);
				//taue2 = tau[VELOCITY_X_COMPONENT][iE2] * tau[VELOCITY_X_COMPONENT][iP] / (feplus2 * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - feplus2) * tau[VELOCITY_X_COMPONENT][iE2]);
			}
			else taue2 = tau[VELOCITY_X_COMPONENT][iE2]; // проверено !
		}
		if (iW2 > -1) {
			if (!bW2) {
				tauw2 = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iW2], tau[VELOCITY_X_COMPONENT][iP], fwplus2, Fw_sum, W_SIDE);
				//	tauw2 = tau[VELOCITY_X_COMPONENT][iW2] * tau[VELOCITY_X_COMPONENT][iP] / (fwplus2 * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - fwplus2) * tau[VELOCITY_X_COMPONENT][iW2]);
			}
			else tauw2 = tau[VELOCITY_X_COMPONENT][iW2];
		}
		if (iN2 > -1) {
			if (!bN2) {
				taun2 = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iN2], tau[VELOCITY_Y_COMPONENT][iP], fnplus2, Fn_sum, N_SIDE);
				//taun2 = tau[VELOCITY_Y_COMPONENT][iN2] * tau[VELOCITY_Y_COMPONENT][iP] / (fnplus2 * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fnplus2) * tau[VELOCITY_Y_COMPONENT][iN2]);
			}
			else taun2 = tau[VELOCITY_Y_COMPONENT][iN2];
		}
		if (iS2 > -1) {
			if (!bS2) {
				taus2 = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iS2], tau[VELOCITY_Y_COMPONENT][iP], fsplus2, Fs_sum, S_SIDE);
				//taus2 = tau[VELOCITY_Y_COMPONENT][iS2] * tau[VELOCITY_Y_COMPONENT][iP] / (fsplus2 * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fsplus2) * tau[VELOCITY_Y_COMPONENT][iS2]);
			}
			else taus2 = tau[VELOCITY_Y_COMPONENT][iS2];
		}
		if (iT2 > -1) {
			if (!bT2) {
				taut2 = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iT2], tau[VELOCITY_Z_COMPONENT][iP], ftplus2, Ft_sum, T_SIDE);
				//taut2 = tau[VELOCITY_Z_COMPONENT][iT2] * tau[VELOCITY_Z_COMPONENT][iP] / (ftplus2 * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - ftplus2) * tau[VELOCITY_Z_COMPONENT][iT2]);
			}
			else taut2 = tau[VELOCITY_Z_COMPONENT][iT2];
		}
		if (iB2 > -1) {
			if (!bB2) {
				taub2 = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iB2], tau[VELOCITY_Z_COMPONENT][iP], fbplus2, Fb_sum, B_SIDE);
				//taub2 = tau[VELOCITY_Z_COMPONENT][iB2] * tau[VELOCITY_Z_COMPONENT][iP] / (fbplus2 * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - fbplus2) * tau[VELOCITY_Z_COMPONENT][iB2]);
			}
			else taub2 = tau[VELOCITY_Z_COMPONENT][iB2];
		}


		if (iE3 > -1) {
			if (!bE3) {
				taue3 = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iE3], tau[VELOCITY_X_COMPONENT][iP], feplus3, Fe_sum , E_SIDE);
				//taue3 = tau[VELOCITY_X_COMPONENT][iE3] * tau[VELOCITY_X_COMPONENT][iP] / (feplus3 * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - feplus3) * tau[VELOCITY_X_COMPONENT][iE3]);
			}
			else taue3 = tau[VELOCITY_X_COMPONENT][iE3]; // проверено !
		}
		if (iW3 > -1) {
			if (!bW3) {
				tauw3 = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iW3], tau[VELOCITY_X_COMPONENT][iP], fwplus3, Fw_sum, W_SIDE);
				//tauw3 = tau[VELOCITY_X_COMPONENT][iW3] * tau[VELOCITY_X_COMPONENT][iP] / (fwplus3 * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - fwplus3) * tau[VELOCITY_X_COMPONENT][iW3]);
			}
			else tauw3 = tau[VELOCITY_X_COMPONENT][iW3];
		}
		if (iN3 > -1) {
			if (!bN3) {
				taun3 = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iN3], tau[VELOCITY_Y_COMPONENT][iP], fnplus3, Fn_sum, N_SIDE);
				//taun3 = tau[VELOCITY_Y_COMPONENT][iN3] * tau[VELOCITY_Y_COMPONENT][iP] / (fnplus3 * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fnplus3) * tau[VELOCITY_Y_COMPONENT][iN3]);
			}
			else taun3 = tau[VELOCITY_Y_COMPONENT][iN3];
		}
		if (iS3 > -1) {
			if (!bS3) {
				taus3 = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iS3], tau[VELOCITY_Y_COMPONENT][iP], fsplus3, Fs_sum, S_SIDE);
				//taus3 = tau[VELOCITY_Y_COMPONENT][iS3] * tau[VELOCITY_Y_COMPONENT][iP] / (fsplus3 * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fsplus3) * tau[VELOCITY_Y_COMPONENT][iS3]);
			}
			else taus3 = tau[VELOCITY_Y_COMPONENT][iS3];
		}
		if (iT3 > -1) {
			if (!bT3) {
				taut3 = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iT3], tau[VELOCITY_Z_COMPONENT][iP], ftplus3, Ft_sum, T_SIDE);
				//taut3 = tau[VELOCITY_Z_COMPONENT][iT3] * tau[VELOCITY_Z_COMPONENT][iP] / (ftplus3 * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - ftplus3) * tau[VELOCITY_Z_COMPONENT][iT3]);
			}
			else taut3 = tau[VELOCITY_Z_COMPONENT][iT3];
		}
		if (iB3 > -1) {
			if (!bB3) {
				taub3 = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iB3], tau[VELOCITY_Z_COMPONENT][iP], fbplus3, Fb_sum, B_SIDE);
				//taub3 = tau[VELOCITY_Z_COMPONENT][iB3] * tau[VELOCITY_Z_COMPONENT][iP] / (fbplus3 * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - fbplus3) * tau[VELOCITY_Z_COMPONENT][iB3]);
			}
			else taub3 = tau[VELOCITY_Z_COMPONENT][iB3];
		}


		if (iE4 > -1) {
			if (!bE4) {
				taue4 = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iE4], tau[VELOCITY_X_COMPONENT][iP], feplus4, Fe_sum, E_SIDE);
				//taue4 = tau[VELOCITY_X_COMPONENT][iE4] * tau[VELOCITY_X_COMPONENT][iP] / (feplus4 * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - feplus4) * tau[VELOCITY_X_COMPONENT][iE4]);
			}
			else taue4 = tau[VELOCITY_X_COMPONENT][iE4]; // проверено !
		}
		if (iW4 > -1) {
			if (!bW4) {
				tauw4 = interpolation_na_face_tau(tau[VELOCITY_X_COMPONENT][iW4], tau[VELOCITY_X_COMPONENT][iP], fwplus4, Fw_sum, W_SIDE);
				//tauw4 = tau[VELOCITY_X_COMPONENT][iW4] * tau[VELOCITY_X_COMPONENT][iP] / (fwplus4 * tau[VELOCITY_X_COMPONENT][iP] + (1.0 - fwplus4) * tau[VELOCITY_X_COMPONENT][iW4]);
			}
			else tauw4 = tau[VELOCITY_X_COMPONENT][iW4];
		}
		if (iN4 > -1) {
			if (!bN4) {
				taun4 = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iN4], tau[VELOCITY_Y_COMPONENT][iP], fnplus4, Fn_sum, N_SIDE);
				//taun4 = tau[VELOCITY_Y_COMPONENT][iN4] * tau[VELOCITY_Y_COMPONENT][iP] / (fnplus4 * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fnplus4) * tau[VELOCITY_Y_COMPONENT][iN4]);
			}
			else taun4 = tau[VELOCITY_Y_COMPONENT][iN4];
		}
		if (iS4 > -1) {
			if (!bS4) {
				taus4 = interpolation_na_face_tau(tau[VELOCITY_Y_COMPONENT][iS4], tau[VELOCITY_Y_COMPONENT][iP], fsplus4, Fs_sum, S_SIDE);
				//taus4 = tau[VELOCITY_Y_COMPONENT][iS4] * tau[VELOCITY_Y_COMPONENT][iP] / (fsplus4 * tau[VELOCITY_Y_COMPONENT][iP] + (1.0 - fsplus4) * tau[VELOCITY_Y_COMPONENT][iS4]);
			}
			else taus4 = tau[VELOCITY_Y_COMPONENT][iS4];
		}
		if (iT4 > -1) {
			if (!bT4) {
				taut4 = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iT4], tau[VELOCITY_Z_COMPONENT][iP], ftplus4, Ft_sum, T_SIDE);
				//taut4 = tau[VELOCITY_Z_COMPONENT][iT4] * tau[VELOCITY_Z_COMPONENT][iP] / (ftplus4 * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - ftplus4) * tau[VELOCITY_Z_COMPONENT][iT4]);
			}
			else taut4 = tau[VELOCITY_Z_COMPONENT][iT4];
		}
		if (iB4 > -1) {
			if (!bB4) {
				taub4 = interpolation_na_face_tau(tau[VELOCITY_Z_COMPONENT][iB4], tau[VELOCITY_Z_COMPONENT][iP], fbplus4, Fb_sum,B_SIDE);
				//taub4 = tau[VELOCITY_Z_COMPONENT][iB4] * tau[VELOCITY_Z_COMPONENT][iP] / (fbplus4 * tau[VELOCITY_Z_COMPONENT][iP] + (1.0 - fbplus4) * tau[VELOCITY_Z_COMPONENT][iB4]);
			}
			else taub4 = tau[VELOCITY_Z_COMPONENT][iB4];
		}
	}
	...
	*/
	...
	//sl[PAM][iP].b=RelaxationPAMTerm[iP]+0.25*(sl[PAM][iP].b - RelaxationPAMTerm[iP]);

	//RelaxationPAMTerm[iP] = sl[PAM][iP].b;

	// 03.01.2021 Если нулевой поток то явный дисбаланс и мы  препятствуем этому явному дисбалансу т.к. будет нефизично скорее всего..
	//sl[PAM][iP].b=  ( ( (fabs(Fw_sum)>1.0e-30)&&(fabs(Fs_sum) > 1.0e-30)||(inum_iter_loc>5) ? Fw_sum - Fe_sum : 0.1 * Fw_sum - Fe_sum)
		//+((fabs(Fs_sum) > 1.0e-30) && (fabs(Fn_sum) > 1.0e-30) || (inum_iter_loc > 5) ? Fs_sum - Fn_sum : 0.1* Fs_sum - Fn_sum) +
		//((fabs(Fb_sum) > 1.0e-30) && (fabs(Ft_sum) > 1.0e-30) || (inum_iter_loc > 5) ? Fb_sum -Ft_sum : 0.1* Fb_sum - Ft_sum) + baddDFLUX2);
	
	// И слева и справа узлы строго внутренние. Граничный источник обрабатывается особо и может привести к слишком большому перепаду давления.
	// Так неработает.
	//sl[PAM][iP].b = ((((iW<maxelm)&&(iE<maxelm)&&(iW>-1)&&(iE>-1)) ?  (Fw_sum - Fe_sum) : 0.0) + (((iN < maxelm) && (iS < maxelm)&&(iN>-1)&&(iS>-1)) ? (Fs_sum - Fn_sum) : 0.0) + (((iT < maxelm) && (iB < maxelm)&&(iT>-1)&&(iB>-1)) ? (Fb_sum - Ft_sum) : 0.0) + baddDFLUX2);
...
//02.01.2018 Вроде Ok.
	...
	*/
	...
	if (fabs(sl[PAM][iP].ap) < 1.0e-300) {		
		printf("Error sl[PAM][%d].ap=%e < 1.0e-300 PAM in function my_elmatr_quad_PAm3\n", iP, sl[PAM][iP].ap);
		printf("sl[PAM][%d].ae=%e \n", iP, sl[PAM][iP].ae);
		printf("sl[PAM][%d].aw=%e \n", iP, sl[PAM][iP].aw);
		printf("sl[PAM][%d].an=%e \n", iP, sl[PAM][iP].an);
		printf("sl[PAM][%d].as=%e \n", iP, sl[PAM][iP].as);
		printf("sl[PAM][%d].at=%e \n", iP, sl[PAM][iP].at);
		printf("sl[PAM][%d].ab=%e \n", iP, sl[PAM][iP].ab);
		if (b_on_adaptive_local_refinement_mesh) {
			printf("sl[PAM][%d].ae2=%e \n", iP, sl[PAM][iP].ae2);
			printf("sl[PAM][%d].aw2=%e \n", iP, sl[PAM][iP].aw2);
			printf("sl[PAM][%d].an2=%e \n", iP, sl[PAM][iP].an2);
			printf("sl[PAM][%d].as2=%e \n", iP, sl[PAM][iP].as2);
			printf("sl[PAM][%d].at2=%e \n", iP, sl[PAM][iP].at2);
			printf("sl[PAM][%d].ab2=%e \n", iP, sl[PAM][iP].ab2);
			printf("sl[PAM][%d].ae3=%e \n", iP, sl[PAM][iP].ae3);
			printf("sl[PAM][%d].aw3=%e \n", iP, sl[PAM][iP].aw3);
			printf("sl[PAM][%d].an3=%e \n", iP, sl[PAM][iP].an3);
			printf("sl[PAM][%d].as3=%e \n", iP, sl[PAM][iP].as3);
			printf("sl[PAM][%d].at3=%e \n", iP, sl[PAM][iP].at3);
			printf("sl[PAM][%d].ab3=%e \n", iP, sl[PAM][iP].ab3);
			printf("sl[PAM][%d].ae4=%e \n", iP, sl[PAM][iP].ae4);
			printf("sl[PAM][%d].aw4=%e \n", iP, sl[PAM][iP].aw4);
			printf("sl[PAM][%d].an4=%e \n", iP, sl[PAM][iP].an4);
			printf("sl[PAM][%d].as4=%e \n", iP, sl[PAM][iP].as4);
			printf("sl[PAM][%d].at4=%e \n", iP, sl[PAM][iP].at4);
			printf("sl[PAM][%d].ab4=%e \n", iP, sl[PAM][iP].ab4);
		}
		if (sl[PAM][iP].b != sl[PAM][iP].b) {
