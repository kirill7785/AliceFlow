...
// 27.04.2022 Исправлена формула для коэффициента диффузии на основании среднего гармонического.
...
void my_elmatr_quad_SpallartAllmares3D(
	...
	*/
	...
	GP = ((prop[MU_DYNAMIC_VISCOSITY][iP])+ fmax(0.0, rP*potent[NUSHA][iP]))/ eqin.fluidinfo[0].sigma_nu; // в центре внутреннего КО.
	if (iE > -1) {
	...
	if (sl[NUSHA_SL][iP].b != sl[NUSHA_SL][iP].b) {
		printf("exptsr+attrs error NAN or INF in control volume %d NUSHA\n", iP);
		system("pause");
	}
	...
	if (sl[NUSHA_SL][iP].b != sl[NUSHA_SL][iP].b) {
		printf("dSc*dx*dy*dz error NUSHA NAN or INF in control volume %d\n", iP);
		system("pause");
	}
