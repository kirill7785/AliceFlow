...
void green_gaussO1(integer iP, doublereal** &potent, int** &nvtx, TOCHKA* &pa,
	...
	if (bRCh) {
		...
		// Обратная пропорциональность с весом соответствует статье I.Sezai.
		if (iE > -1) {
		...
		if (b_on_adaptive_local_refinement_mesh) {
			...
			if (iE2 > -1) {
				rhoe2 = rE2 * rP / ((1.0 - feplus2) * rE2 + feplus2 * rP); // проверено.
			}
			if (iW2 > -1) {
			...
			if (iE3 > -1) {
				rhoe3 = rE3 * rP / ((1.0 - feplus3) * rE3 + feplus3 * rP); // проверено.
			}
			if (iW3 > -1) {
			...
			if (iE4 > -1) {
				rhoe4 = rE4 * rP / ((1.0 - feplus4) * rE4 + feplus4 * rP); // проверено.
			}
			if (iW4 > -1) {
