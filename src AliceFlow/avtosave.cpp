...
void avtosave(FLOW* &f, TEMPER &t, int flow_interior, int* &inumber_iteration_SIMPLE, doublereal* &continity_start) {
	...
	if (((err) !=0)||(fp_avtosave == NULL)) {
       ...
       exit(0);
		...
		if ((isize0)%20 != 0) fprintf(fp_avtosave,"\n");

		integer j=0;
		for (j=0; j<flow_interior; ++j) {


			const integer isizeF = static_cast<integer>(f[j].maxelm) + static_cast<integer>(f[j].maxbound);
		    ...
		    if ((isizeF)%20 != 0) fprintf(fp_avtosave,"\n");
			// VY
            for (i=0; i<(isizeF); ++i) {
...
void avtoreadvalue(FLOW* &f, TEMPER &t, int flow_interior,
	...
	else if (fp_avtosave!=NULL)
	{
		...
		if (din != (isize0)) bcontinue=false; // проверка на совпадение числа КО в задаче теплопроводности
...
#endif
#endif
			const integer isizeF = static_cast<integer>(f[i].maxelm) + static_cast<integer>(f[i].maxbound);
			
			if (din != (isizeF)) bcontinue=false; // проверка на количество КО в FLUID зоне номер i.
		}
		...
		if (!bcontinue) {
			...
			for (i=0; i<flow_interior; ++i) {
				...
				for (j=0; j< static_cast<integer>(f[i].maxelm); ++j) {
				...
				for (integer j_1 = 0; j_1 < static_cast<integer>(f[i].maxelm); ++j_1) {
		        ...
		        for (integer j_1=0; j_1<(isizeF); ++j_1) {
					...
					//if ((f[i].iflowregime == VISCOSITY_MODEL::RANS_MENTER_SST) || (f[i].iflowregime == VISCOSITY_MODEL::RANS_LANGTRY_MENTOR_SST)) {
						//diag_add_only_Turb = -(2.0 / 3.0) * f[i].prop[RHO][j_1] * f[i].potent[TURBULENT_KINETIK_ENERGY][j_1]/(f[i].prop[MU_DYNAMIC_VISCOSITY][j_1]+f[i].prop[MUT][j_1]);
					//}
			        sum+=2.0*sqr(f[i].potent[GRADXVX][j_1] /* + 0.5 * diag_add_only_Turb*/);
			        sum+=2.0*sqr(f[i].potent[GRADYVY][j_1] /* + 0.5 * diag_add_only_Turb*/);
			        sum+=2.0*sqr(f[i].potent[GRADZVZ][j_1] /* + 0.5 * diag_add_only_Turb*/);
			        sum+=0.5*sqr(f[i].potent[GRADYVX][j_1]+f[i].potent[GRADXVY][j_1]);
			        sum+=0.5*sqr(f[i].potent[GRADZVX][j_1]+f[i].potent[GRADXVZ][j_1]);
			        sum+=0.5*sqr(f[i].potent[GRADYVZ][j_1]+f[i].potent[GRADZVY][j_1]);
					...
					//sum-=(2.0/3.0)*sqr(f[i].potent[GRADXVX][j_1]+f[i].potent[GRADYVY][j_1]+f[i].potent[GRADZVZ][j_1]); // добавок связанный с несжимаемостью/сжимаемостью
		            ...
		            sum=0.0;
		            sum+=0.5*sqr(f[i].potent[GRADYVZ][j_1]-f[i].potent[GRADZVY][j_1]); // проверено !
		            sum+=0.5*sqr(f[i].potent[GRADZVX][j_1]-f[i].potent[GRADXVZ][j_1]);
		            sum+=0.5*sqr(f[i].potent[GRADXVY][j_1]-f[i].potent[GRADYVX][j_1]);
