...
void correct_mf(doublereal** &mfcurrentretune, doublereal** potent,  doublereal** tau,
	...
	integer *ilevel_alice, int* ptr, float** prop, TOCHKA*& volume_loc) {
...
#pragma omp parallel for
	for (integer iP=0; iP<maxelm; ++iP) {
		...
		if (iE > -1) {
			if (!bE) taue = tau[VELOCITY_X_COMPONENT][iE] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - feplus)*tau[VELOCITY_X_COMPONENT][iE] + feplus *tau[VELOCITY_X_COMPONENT][iP]); else taue = tau[VELOCITY_X_COMPONENT][iE];
		}
		if (iW > -1) {
			if (!bW) tauw = tau[VELOCITY_X_COMPONENT][iW] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - fwplus)*tau[VELOCITY_X_COMPONENT][iW] + fwplus *tau[VELOCITY_X_COMPONENT][iP]); else tauw = tau[VELOCITY_X_COMPONENT][iW];
		}
		if (iN > -1) {
			if (!bN) taun = tau[VELOCITY_Y_COMPONENT][iN] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fnplus)*tau[VELOCITY_Y_COMPONENT][iN] + fnplus *tau[VELOCITY_Y_COMPONENT][iP]); else taun = tau[VELOCITY_Y_COMPONENT][iN];
		}
		if (iS > -1) {
			if (!bS) taus = tau[VELOCITY_Y_COMPONENT][iS] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fsplus)*tau[VELOCITY_Y_COMPONENT][iS] + fsplus *tau[VELOCITY_Y_COMPONENT][iP]); else taus = tau[VELOCITY_Y_COMPONENT][iS];
		}
		if (iT > -1) {
			if (!bT) taut = tau[VELOCITY_Z_COMPONENT][iT] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - ftplus)*tau[VELOCITY_Z_COMPONENT][iT] + ftplus *tau[VELOCITY_Z_COMPONENT][iP]); else taut = tau[VELOCITY_Z_COMPONENT][iT];
		}
		if (iB > -1) {
			if (!bB) taub = tau[VELOCITY_Z_COMPONENT][iB] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - fbplus)*tau[VELOCITY_Z_COMPONENT][iB] + fbplus *tau[VELOCITY_Z_COMPONENT][iP]); else taub = tau[VELOCITY_Z_COMPONENT][iB];
		}
		...
		if (b_on_adaptive_local_refinement_mesh) {
			...
			if (iE2 > -1) {
				if (!bE2) taue2 = tau[VELOCITY_X_COMPONENT][iE2] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - feplus2) * tau[VELOCITY_X_COMPONENT][iE2] + feplus2 * tau[VELOCITY_X_COMPONENT][iP]); else taue2 = tau[VELOCITY_X_COMPONENT][iE2];
			}
			if (iW2 > -1) {
				if (!bW2) tauw2 = tau[VELOCITY_X_COMPONENT][iW2] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - fwplus2) * tau[VELOCITY_X_COMPONENT][iW2] + fwplus2 * tau[VELOCITY_X_COMPONENT][iP]); else tauw2 = tau[VELOCITY_X_COMPONENT][iW2];
			}
			if (iN2 > -1) {
				if (!bN2) taun2 = tau[VELOCITY_Y_COMPONENT][iN2] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fnplus2) * tau[VELOCITY_Y_COMPONENT][iN2] + fnplus2 * tau[VELOCITY_Y_COMPONENT][iP]); else taun2 = tau[VELOCITY_Y_COMPONENT][iN2];
			}
			if (iS2 > -1) {
				if (!bS2) taus2 = tau[VELOCITY_Y_COMPONENT][iS2] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fsplus2) * tau[VELOCITY_Y_COMPONENT][iS2] + fsplus2 * tau[VELOCITY_Y_COMPONENT][iP]); else taus2 = tau[VELOCITY_Y_COMPONENT][iS2];
			}
			if (iT2 > -1) {
				if (!bT2) taut2 = tau[VELOCITY_Z_COMPONENT][iT2] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - ftplus2) * tau[VELOCITY_Z_COMPONENT][iT2] + ftplus2 * tau[VELOCITY_Z_COMPONENT][iP]); else taut2 = tau[VELOCITY_Z_COMPONENT][iT2];
			}
			if (iB2 > -1) {
				if (!bB2) taub2 = tau[VELOCITY_Z_COMPONENT][iB2] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - fbplus2) * tau[VELOCITY_Z_COMPONENT][iB2] + fbplus2 * tau[VELOCITY_Z_COMPONENT][iP]); else taub2 = tau[VELOCITY_Z_COMPONENT][iB2];
			}

			if (iE3 > -1) {
				if (!bE3) taue3 = tau[VELOCITY_X_COMPONENT][iE3] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - feplus3) * tau[VELOCITY_X_COMPONENT][iE3] + feplus3 * tau[VELOCITY_X_COMPONENT][iP]); else taue3 = tau[VELOCITY_X_COMPONENT][iE3];
			}
			if (iW3 > -1) {
				if (!bW3) tauw3 = tau[VELOCITY_X_COMPONENT][iW3] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - fwplus3) * tau[VELOCITY_X_COMPONENT][iW3] + fwplus3 * tau[VELOCITY_X_COMPONENT][iP]); else tauw3 = tau[VELOCITY_X_COMPONENT][iW3];
			}
			if (iN3 > -1) {
				if (!bN3) taun3 = tau[VELOCITY_Y_COMPONENT][iN3] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fnplus3) * tau[VELOCITY_Y_COMPONENT][iN3] + fnplus3 * tau[VELOCITY_Y_COMPONENT][iP]); else taun3 = tau[VELOCITY_Y_COMPONENT][iN3];
			}
			if (iS3 > -1) {
				if (!bS3) taus3 = tau[VELOCITY_Y_COMPONENT][iS3] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fsplus3) * tau[VELOCITY_Y_COMPONENT][iS3] + fsplus3 * tau[VELOCITY_Y_COMPONENT][iP]); else taus3 = tau[VELOCITY_Y_COMPONENT][iS3];
			}
			if (iT3 > -1) {
				if (!bT3) taut3 = tau[VELOCITY_Z_COMPONENT][iT3] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - ftplus3) * tau[VELOCITY_Z_COMPONENT][iT3] + ftplus3 * tau[VELOCITY_Z_COMPONENT][iP]); else taut3 = tau[VELOCITY_Z_COMPONENT][iT3];
			}
			if (iB3 > -1) {
				if (!bB3) taub3 = tau[VELOCITY_Z_COMPONENT][iB3] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - fbplus3) * tau[VELOCITY_Z_COMPONENT][iB3] + fbplus3 * tau[VELOCITY_Z_COMPONENT][iP]); else taub3 = tau[VELOCITY_Z_COMPONENT][iB3];
			}

			if (iE4 > -1) {
				if (!bE4) taue4 = tau[VELOCITY_X_COMPONENT][iE4] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - feplus4) * tau[VELOCITY_X_COMPONENT][iE4] + feplus4 * tau[VELOCITY_X_COMPONENT][iP]); else taue4 = tau[VELOCITY_X_COMPONENT][iE4];
			}
			if (iW4 > -1) {
				if (!bW4) tauw4 = tau[VELOCITY_X_COMPONENT][iW4] * tau[VELOCITY_X_COMPONENT][iP] / ((1.0 - fwplus4) * tau[VELOCITY_X_COMPONENT][iW4] + fwplus4 * tau[VELOCITY_X_COMPONENT][iP]); else tauw4 = tau[VELOCITY_X_COMPONENT][iW4];
			}
			if (iN4 > -1) {
				if (!bN4) taun4 = tau[VELOCITY_Y_COMPONENT][iN4] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fnplus4) * tau[VELOCITY_Y_COMPONENT][iN4] + fnplus4 * tau[VELOCITY_Y_COMPONENT][iP]); else taun4 = tau[VELOCITY_Y_COMPONENT][iN4];
			}
			if (iS4 > -1) {
				if (!bS4) taus4 = tau[VELOCITY_Y_COMPONENT][iS4] * tau[VELOCITY_Y_COMPONENT][iP] / ((1.0 - fsplus4) * tau[VELOCITY_Y_COMPONENT][iS4] + fsplus4 * tau[VELOCITY_Y_COMPONENT][iP]); else taus4 = tau[VELOCITY_Y_COMPONENT][iS4];
			}
			if (iT4 > -1) {
				if (!bT4) taut4 = tau[VELOCITY_Z_COMPONENT][iT4] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - ftplus4) * tau[VELOCITY_Z_COMPONENT][iT4] + ftplus4 * tau[VELOCITY_Z_COMPONENT][iP]); else taut4 = tau[VELOCITY_Z_COMPONENT][iT4];
			}
			if (iB4 > -1) {
				if (!bB4) taub4 = tau[VELOCITY_Z_COMPONENT][iB4] * tau[VELOCITY_Z_COMPONENT][iP] / ((1.0 - fbplus4) * tau[VELOCITY_Z_COMPONENT][iB4] + fbplus4 * tau[VELOCITY_Z_COMPONENT][iP]); else taub4 = tau[VELOCITY_Z_COMPONENT][iB4];
			}
			...
			// Наконец вычисление скорректированного массового 
			// потока на грани КО.
			mfloc[iP][E_SIDE] = mfcurrentretune[iP][E_SIDE] - taue * gradpame * dSqe;
			mfloc[iP][W_SIDE] = mfcurrentretune[iP][W_SIDE] - tauw * gradpamw * dSqw;
			mfloc[iP][N_SIDE] = mfcurrentretune[iP][N_SIDE] - taun * gradpamn * dSqn;
			mfloc[iP][S_SIDE] = mfcurrentretune[iP][S_SIDE] - taus * gradpams * dSqs;
			mfloc[iP][T_SIDE] = mfcurrentretune[iP][T_SIDE] - taut * gradpamt * dSqt;
			mfloc[iP][B_SIDE] = mfcurrentretune[iP][B_SIDE] - taub * gradpamb * dSqb;
		...
		if ((iW > -1) && (bW)) {
			if ((w[border_neighbor[iW - maxelm].MCB - ls].Vx * w[border_neighbor[iW - maxelm].MCB - ls].Vx +
				w[border_neighbor[iW - maxelm].MCB - ls].Vy * w[border_neighbor[iW - maxelm].MCB - ls].Vy +
				w[border_neighbor[iW - maxelm].MCB - ls].Vz * w[border_neighbor[iW - maxelm].MCB - ls].Vz) > 1.0e-30) {

				mfloc[iP][W_SIDE] = prop[RHO][iP] * dy * dz * w[border_neighbor[iW - maxelm].MCB - ls].Vx;
			}
		}

		if ((iE > -1) && (bE)) {
			if ((w[border_neighbor[iE - maxelm].MCB - ls].Vx * w[border_neighbor[iE - maxelm].MCB - ls].Vx +
				w[border_neighbor[iE - maxelm].MCB - ls].Vy * w[border_neighbor[iE - maxelm].MCB - ls].Vy +
				w[border_neighbor[iE - maxelm].MCB - ls].Vz * w[border_neighbor[iE - maxelm].MCB - ls].Vz) > 1.0e-30) {

				mfloc[iP][E_SIDE] = prop[RHO][iP] * dy * dz * w[border_neighbor[iE - maxelm].MCB - ls].Vx;
			}
		}

		if ((iS > -1) && (bS)) {
			if ((w[border_neighbor[iS - maxelm].MCB - ls].Vx * w[border_neighbor[iS - maxelm].MCB - ls].Vx +
				w[border_neighbor[iS - maxelm].MCB - ls].Vy * w[border_neighbor[iS - maxelm].MCB - ls].Vy +
				w[border_neighbor[iS - maxelm].MCB - ls].Vz * w[border_neighbor[iS - maxelm].MCB - ls].Vz) > 1.0e-30) {

				mfloc[iP][S_SIDE] = prop[RHO][iP] * dx * dz * w[border_neighbor[iS - maxelm].MCB - ls].Vy;
			}
		}

		if ((iN > -1) && (bN)) {
			if ((w[border_neighbor[iN - maxelm].MCB - ls].Vx * w[border_neighbor[iN - maxelm].MCB - ls].Vx +
				w[border_neighbor[iN - maxelm].MCB - ls].Vy * w[border_neighbor[iN - maxelm].MCB - ls].Vy +
				w[border_neighbor[iN - maxelm].MCB - ls].Vz * w[border_neighbor[iN - maxelm].MCB - ls].Vz) > 1.0e-30) {

				mfloc[iP][N_SIDE] = prop[RHO][iP] * dx * dz * w[border_neighbor[iN - maxelm].MCB - ls].Vy;
			}
		}

		if ((iB > -1) && (bB)) {
			if ((w[border_neighbor[iB - maxelm].MCB - ls].Vx * w[border_neighbor[iB - maxelm].MCB - ls].Vx +
				w[border_neighbor[iB - maxelm].MCB - ls].Vy * w[border_neighbor[iB - maxelm].MCB - ls].Vy +
				w[border_neighbor[iB - maxelm].MCB - ls].Vz * w[border_neighbor[iB - maxelm].MCB - ls].Vz) > 1.0e-30) {

				mfloc[iP][B_SIDE] = prop[RHO][iP] * dx * dy * w[border_neighbor[iB - maxelm].MCB - ls].Vz;
			}
		}

		if ((iT > -1) && (bT)) {
			if ((w[border_neighbor[iT - maxelm].MCB - ls].Vx * w[border_neighbor[iT - maxelm].MCB - ls].Vx +
				w[border_neighbor[iT - maxelm].MCB - ls].Vy * w[border_neighbor[iT - maxelm].MCB - ls].Vy +
				w[border_neighbor[iT - maxelm].MCB - ls].Vz * w[border_neighbor[iT - maxelm].MCB - ls].Vz) > 1.0e-30) {

				mfloc[iP][T_SIDE] = prop[RHO][iP] * dx * dy * w[border_neighbor[iT - maxelm].MCB - ls].Vz;
			}
		}
		...
		//if ((iW >= maxelm) && (pa[iP].x < -0.0356)) {
			//if (fabs(1.0 - mfloc[iP][W_SIDE] / (1.1614 * dy * dz)) > 1.0e-3)
			//{
				//std::cout << "correct mf:\n";
				//std::cout << mfcurrentretune[W_SIDE] / (rP * dy * dz) << std::endl;
				//std::cout << Fw_sum / (rP * dy * dz) << std::endl;
				//std::cout << mfloc[iP][W_SIDE]  / (1.1614 * dy * dz) << std::endl;
				//getchar();
			//}
		//}
...
void correct_boundary_volume(integer iVar, doublereal** &potent,
							 ...
							 int*** neighbors_for_the_internal_node, BOUND* &border_neighbor,
							 integer ls, integer lw, WALL* w, doublereal* &relax_value, integer binterpol, TOCHKA*& volume_loc) {
		...
		if (iS > -1) {
			if (iS >= maxelm) {
				...
				if (((border_neighbor[inumber].MCB >= ls) && (border_neighbor[inumber].MCB < (ls + lw)) && (w[border_neighbor[inumber].MCB - ls].bpressure || w[border_neighbor[inumber].MCB - ls].bopening))) {
					...
					//if (iVar==VY) { printf("Vs==%e, Vp==%e\n",potent[iVar][iS],potent[iVar][iP]); system("pause"); } // debug
				} // pressure outlet
				else if (((border_neighbor[inumber].MCB >= ls) && (border_neighbor[inumber].MCB < (ls + lw)) && w[border_neighbor[inumber].MCB - ls].bsymmetry)) {
					// граница симметрии: по VX и VZ стоит однородное условие Неймана, а для VY==0.0;
					// Значит скорость VX и VZ в граничном узле нужно скорректировать записав в неё значение из ближайшего внутреннего узла,
