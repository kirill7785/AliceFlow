...
void resize_integer_array(integer*& arr, integer& size, integer new_size) {

	if (size > 0) {
		integer* arr_copy = new integer[size];
		for (integer i = 0; i < size; ++i) arr_copy[i] = arr[i];
		delete[] arr;
		integer isize_old = size;
		size = new_size;
		arr = new integer[size];
		for (integer i = 0; i < (size < isize_old ? size : isize_old); ++i) arr[i] = arr_copy[i];
		delete[] arr_copy;
	}
	else if (new_size > 0) {
		size = new_size;
...
void add_integer_in_array(integer*& arr, integer& size, integer &icurent_position, integer iadd) {
	

	if (icurent_position < size) {
		arr[icurent_position] = iadd;
		++icurent_position;
	}
	else {
		if (size > 0) {
			...
			size = 2 * isize_old + 2;
			arr = new integer[size];
			for (integer i = 0; i < isize_old; ++i) arr[i] = arr_copy[i];
			delete[] arr_copy;
		}
		else {
			arr = new integer[16];
...
void visible_CRS_Matrix(integer n, integer nnz, doublereal*& val,
	...
	doublereal*& rthdsd, doublereal*& potent,
	BLOCK*& b, int& lb, integer*& id,
	WALL*& w, int& lw, integer& maxelm,
	...
	int& ls, TPROP* &matlist)
{
	...
	integer maxelm = 0;
	for (integer i = 1; i < static_cast<integer>(lb); ++i) {
		if ((b[i].itype != PHYSICS_TYPE_IN_BODY::HOLLOW) && (block_is_active[i])) maxelm++;
	}
	integer* id = new integer[static_cast<integer>(maxelm) + static_cast<integer>(lw)];
	integer* id_reverse = new integer[static_cast<integer>(lb) + static_cast<integer>(lw)];
	integer* wall2block_link = new integer[static_cast<integer>(lw)];
	integer ic = 0;
	for (integer i = 1; i < static_cast<integer>(lb); ++i) {
	...
	for (integer i = 0; i < static_cast<integer>(lw); ++i) {
	...
	integer** ilink = new integer * [static_cast<integer>(maxelm) + static_cast<integer>(lw)];
	integer** ilink_reverse = new integer * [static_cast<integer>(maxelm) + static_cast<integer>(lw)];
	doublereal** dS = new doublereal * [static_cast<integer>(maxelm) + static_cast<integer>(lw)];
	integer* inumber_neighbour = new integer[static_cast<integer>(maxelm) + static_cast<integer>(lw)];
	integer* inumber_neighbour_only_body = new integer[static_cast<integer>(maxelm) + static_cast<integer>(lw)];
	for (integer i = 0; i < static_cast<integer>(maxelm) + static_cast<integer>(lw); ++i) {
	...
	// 03.07.2021
	integer** inodes_list = new integer*[static_cast<integer>(lb)];
	integer* inodes_list_size = new integer[static_cast<integer>(lb)];
	integer* inodes_list_current_position = new integer[static_cast<integer>(lb)];
	for (integer i = 1; i < static_cast<integer>(lb); ++i) {
		inodes_list_size[i] = 0;
		inodes_list_current_position[i] = 0;
	}
	//for (integer i = 1; i < lb; ++i) {
		// Находим количество соседей блока i, в массиве id он имеет номер ic.
		for (integer j_1 = 0; j_1 < static_cast<integer>(t.maxelm); ++j_1) {
			//if (t.whot_is_block[j_1] == i)
			integer i = t.whot_is_block[j_1];
			{
				add_integer_in_array(inodes_list[i], inodes_list_size[i], inodes_list_current_position[i], j_1);
			}
		}
	//}
	for (integer i = 1; i < static_cast<integer>(lb); ++i) {
		resize_integer_array(inodes_list[i], inodes_list_size[i], inodes_list_current_position[i]);
	}
	...
	{ // делаем у переменной ic локальную область видимости.
		...
		if ((b[i].itype != PHYSICS_TYPE_IN_BODY::HOLLOW) && (block_is_active[i])) {
			...
			for (integer j_84 = 0; j_84 < inodes_list_current_position[i]; j_84++)
			{
				integer j_1 = inodes_list[i][j_84];
				//if (t.whot_is_block[j_1] == i)
				{
	...
	for (integer i = 1; i < lb; ++i) {
		if (inodes_list_current_position[i] > 0) {
			delete[] inodes_list[i];
		}
	}
	delete[] inodes_list;
	
	delete[] inodes_list_size;
	delete[] inodes_list_current_position;
		...
		*/
		...
		if (b_nonlinear_network) {
			...
			if (((b_Newton_Richman)||(b_Stefan_Bolcman))) {
				...
				doublereal alphaA = 0.999;// было 0.9 0.4
				//if (maxelm > 200) {
					//alphaA = 0.9;
				//}
				for (integer i_1 = 0; i_1 < maxelm; ++i_1) {					
