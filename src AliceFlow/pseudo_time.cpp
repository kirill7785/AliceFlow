...
void tau_calc(doublereal* &tau, integer maxelm, integer maxbound,
	...
	if (bVERY_STABILITY_ON==true) {
		...
		tau_avg=tau_avg/(static_cast<doublereal>(maxelm));
		if (btimedep) {
			// нестационарная задача.
			for (integer iP=0; iP<maxelm; iP++) {
...
/*
		...
		*/
		...
		for (integer i9=0; i9<=2; i9++) {
			//if (sumanb[i9][iP] < 1.0e-5) {
				//std::cout << "ap_vel=" << sumanb[i9][iP] << std::endl;
				//system("pause");
			//}
	...
	if ((!boldscheme)) {
	   if (bVERY_STABILITY_ON==true) {
		   ...
		   tau_avg[VELOCITY_X_COMPONENT]=tau_avg[VELOCITY_X_COMPONENT]/(static_cast<doublereal>(maxelm));
		   tau_avg[VELOCITY_Y_COMPONENT]=tau_avg[VELOCITY_Y_COMPONENT]/(static_cast<doublereal>(maxelm));
		   tau_avg[VELOCITY_Z_COMPONENT]=tau_avg[VELOCITY_Z_COMPONENT]/(static_cast<doublereal>(maxelm));
