...
static char* my_itoa(integer value, char* str, int base)
{
	...
	if (base != 10) {
		std::cout << "my_itoa base !=10 nepredusmotrennoe osnovanie stepeni.\n";
		system("pause");
		exit(1);
	}
...
void mingw_input_new(const char* fname, integer& lmatmax, int& lb, int& ls, int& lw, TPROP*& matlist, BLOCK*& b, SOURCE*& s, WALL*& w,
	...
	integer& ltdp, TEMP_DEP_POWER* &gtdps, int& lu, UNION* &my_union)
{
	...
	else
	{
		if (fp != nullptr) {
			...
			if (imakesource("pause_suppression", idin)) {
				// Найдено успешно.
				if ((idin == 0) || (idin == 1)) {

					pause_suppression = idin;

					if (pause_suppression == 1) {
						std::cout << "optimetric running..." << std::endl;
					}
					...
					std::cout << "pause suppression unknown identifier..." << idin << std::endl;
					//system("pause");
			...
			if (imakesource("SchemeTurbulent", idin)) {
				// Найдено успешно.
				// выбор схемы для всех турбулентных уравнений.
				// Внимание эти определения должны полностью соответствовать 
				// определениям в файле my_approx_convective2.c
				switch (idin) {
				case 1: iTURBScheme = UNEVEN_MUSCL; break; // MUSCL 2
				case 2: iTURBScheme = UNEVEN_SOUCUP; break; // SOUCUP [MINMOD] 2
				case 3: iTURBScheme = UNEVEN_HLPA; break; // HLPA 2
				case 4: iTURBScheme = UNEVEN_SMART; break; // SMART 3
				case 5: iTURBScheme = UNEVEN_WACEB; break; // WACEB 3 TVD
				case 6: iTURBScheme = UNEVEN_SMARTER; break; // SMARTER 3
				case 7: iTURBScheme = UNEVEN_LPPA; break; // LPPA 3
				case 8: iTURBScheme = UNEVEN_VONOS; break; // VONOS 3
				case 9: iTURBScheme = UNEVEN_STOIC; break; // STOIC
				case 10: iTURBScheme = UNEVEN_CLAM; break; // CLAM
				case 11: iTURBScheme = UNEVEN_OSHER; break; // OSHER
				case 12: iTURBScheme = UNEVEN_EXPONENTIAL; break; // EXPONENTIAL
				case 13: iTURBScheme = UNEVEN_SUPER_C; break; // SUPER_C
				case 14: iTURBScheme = UNEVEN_ISNAS; break; // ISNAS
				case 15: iTURBScheme = UNEVEN_CUBISTA; break; // CUBISTA
				default: iTURBScheme = 2; break; // UDS самая стабильная схема.
				}
				//printf("iTURBScheme  =%lld\n", iTURBScheme);
			}
			else {
				printf("WARNING!!! SchemeTurbulent not found in file premeshin.txt\n");
				iTURBScheme = 2; // UDS самая стабильная схема.
				printf("iTURBScheme = Upwind \n");
				if (bSTOP_Reading) system("pause");
			}
			...
			if (imakesource("amg_manager_sorting_alg", idin)) {
				...
				//printf("my_amg_manager.imySortAlgorithm =%lld\n", my_amg_manager.imySortAlgorithm);
				std::cout << "my_amg_manager.imySortAlgorithm =" << (static_cast<int>(my_amg_manager.imySortAlgorithm)) << std::endl;
				if (bSTOP_Reading) system("pause");
			}
			...
			if (imakesource("amgcl_iterator", idin)) {
				...
				my_amg_manager.amgcl_iterator = AMGCL_ITERATOR_ALG::BiCGStab; // 0 - BiCGStab; 1 - FGMRes.
				//printf("my_amg_manager.amgcl_iterator =%lld\n", my_amg_manager.amgcl_iterator);
				std::cout << "my_amg_manager.amgcl_iterator =" << (static_cast<int>(my_amg_manager.amgcl_iterator)) << std::endl;
			...
			for (i = 0; i < lmatmax; ++i) {
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].rho = static_cast<doublereal>(fin);
				}
				else {
				...
				for (int i_4 = 0; i_4 < matlist[i].n_cp; i_4++) {
					...
					if (fmakesource(name0, fin)) {
						// Найдено успешно.
						matlist[i].temp_cp[i_4] = static_cast<float>(fin);
					}
					else {
						printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					...
					if (fmakesource(name0, fin)) {
						// Найдено успешно.
						matlist[i].arr_cp[i_4] = static_cast<float>(fin);
					}
					else {
						printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				for (int i_4 = 0; i_4 < matlist[i].n_lam; i_4++) {
					...
					if (fmakesource(name0, fin)) {
						// Найдено успешно.
						matlist[i].temp_lam[i_4] = static_cast<float>(fin);
					}
					else {
						printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					...
					if (fmakesource(name0, fin)) {
						// Найдено успешно.
						matlist[i].arr_lam[i_4] = static_cast<float>(fin);
					}
					else {
						printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_x = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_y = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_z = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_x_beta_t_solid = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_y_beta_t_solid = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_z_beta_t_solid = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_x_Young_Module = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_y_Young_Module = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_z_Young_Module = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_Poisson_ratio_xy = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_Poisson_ratio_xz = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_Poisson_ratio_yz = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_Poisson_ratio_yx = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_Poisson_ratio_zx = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].orthotropy_multiplyer_Poisson_ratio_zy = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].ShearModule_xy = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].ShearModule_yz = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].ShearModule_xz = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				//if (fmakesource(name0, fin)) {
					...
					//if (fabs(fin) < 1.0e-30) {
						...
						//exit(1);
				//	}
					//Poissonratio = static_cast<doublereal>(fin);
				//}
				//else {
					//printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (imakesource(name0, idin)) {
					...
					for (int i_4 = 0; i_4 < matlist[i].n_Poisson_ratio; i_4++) {
						...
						if (fmakesource(name0, fin)) {
							// Найдено успешно.
							matlist[i].temp_Poisson_ratio[i_4] = static_cast<float>(fin);
						}
						else {
						...
						if (fmakesource(name0, fin)) {
							// Найдено успешно.
							matlist[i].arr_Poisson_ratio[i_4] = static_cast<float>(fin);
						}
						else {
				...
				if (imakesource(name0, idin)) {
					...
					for (int i_4 = 0; i_4 < matlist[i].n_YoungModule; i_4++) {
						...
						if (fmakesource(name0, fin)) {
							// Найдено успешно.
							matlist[i].temp_Young_Module[i_4] = static_cast<float>(fin);
						}
						else {
				...
				if (imakesource(name0, idin)) {
					...
					for (int i_4 = 0; i_4 < matlist[i].n_beta_t_solid; i_4++) {
						...
						if (fmakesource(name0, fin)) {
							// Найдено успешно.
							matlist[i].temp_beta_t_solid[i_4] = static_cast<float>(fin);
						}
						else {
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].mu = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].beta_t = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].mumin = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					matlist[i].mumin = 0.0; // .
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].mumax = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					matlist[i].mumax = 1.0; // .
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].Amu = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					matlist[i].Amu = 0.0; // .
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].Bmu = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					matlist[i].Bmu = 0.0; // .
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].Cmu = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
					matlist[i].Cmu = 0.0; // .
				...
				if (fmakesource(name0, fin)) {
					// Найдено успешно.
					matlist[i].degreennmu = static_cast<float>(fin);
				}
				else {
					printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
			...
			for (i = 0; i < lw; ++i) {
				...
				case 4:
					...
					if (fmakesource(name0, fin)) {
						// Найдено успешно.
						w[i].emissivity = static_cast<doublereal>(fin);
					}
					else {
						printf("WARNING!!! %s not found in file premeshin.txt\n", name0);
						w[i].emissivity = 0.0; // .
					...
					if (fmakesource(name0, fin)) {
						// Найдено успешно.
						w[i].ViewFactor = static_cast<doublereal>(fin); // Фактор видимости.
			...
			if (imakesource("iu_number", idin)) {
				...
				for (int iuid = 99; iuid >= 0; iuid--) {
					...
					buffer[0] = '\0'; my_itoa(iuid, buffer, 10); strcat(name0, buffer);
					strcat(name0, "xS");
			...
			else
			{
				...
				if (imakesource("amg1r6_checker", idin)) {
					// Найдено успешно.
					AMG1r5Dat.AMG1R6_LABEL = static_cast<integer>(idin);
					//printf(" AMG1R6_LABEL=%lld\n",AMG1R6_LABEL );
				}
				else {
					printf("WARNING!!! amg1r6_checker not found in file premeshin.txt\n");
					AMG1r5Dat.AMG1R6_LABEL = 0; // default amg1r5.
					printf(" AMG1R6_LABEL=%lld\n", AMG1r5Dat.AMG1R6_LABEL);
					if (bSTOP_Reading) system("pause");
				}
				...
				if (imakesource("nrd", idin)) {
					// Найдено успешно.
					if (idin > 9999) {
						AMG1r5Dat.b_iluk_amg1r5_LABEL_D = true;
						AMG1r5Dat.nrd_LABEL = static_cast<integer>(idin - 10000);
					}
					else {
						AMG1r5Dat.nrd_LABEL = static_cast<integer>(idin);
					}
					//printf(" nrd_LABEL=%lld\n",nrd_LABEL );
				}
				else {
					printf("WARNING!!! nrd not found in file premeshin.txt\n");
					AMG1r5Dat.nrd_LABEL = 1131; // default C/F relaxation.
					printf(" nrd_LABEL=%lld\n", AMG1r5Dat.nrd_LABEL);
					if (bSTOP_Reading) system("pause");
				}
				...
				if (imakesource("nru", idin)) {
					// Найдено успешно.
					if (idin > 9999) {
						AMG1r5Dat.b_iluk_amg1r5_LABEL_U = true;
						AMG1r5Dat.nru_LABEL = static_cast<integer>(idin - 10000);
					}
					else {
						AMG1r5Dat.nru_LABEL = static_cast<integer>(idin);
					}
					//printf(" nru_LABEL=%lld\n",nru_LABEL );
				}
				else {
					printf("WARNING!!! nru not found in file premeshin.txt\n");
					AMG1r5Dat.nru_LABEL = 1131; // default C/F relaxation.
					printf(" nru_LABEL=%lld\n", AMG1r5Dat.nru_LABEL);
					if (bSTOP_Reading) system("pause");
				}
				...
				if (fmakesource("ecg2", fin)) {
					// Найдено успешно.
					if ((fin > 0.0) && (fin < 1.0)) {
						AMG1r5Dat.ecg2_LABEL = static_cast<doublereal>(fin);
					}
					else {
						std::cout << "Error reading AMG1r5Dat.ecg2_LABEL=" << fin << std::endl;
						system("pause");
						AMG1r5Dat.ecg2_LABEL = 0.25;
					}
				}
				else {
					printf("WARNING!!! ecg2 not found in file premeshin.txt\n");
					AMG1r5Dat.ecg2_LABEL = 0.25; // .
					printf("ecg2 or strong threshold amg1r5 =%e\n", AMG1r5Dat.ecg2_LABEL);
					if (bSTOP_Reading) system("pause");
				}
				...
				if (fmakesource("ewt2", fin)) {
					// Найдено успешно.
					if ((fin > 0.0) && (fin < 1.0)) {
						AMG1r5Dat.ewt2_LABEL = static_cast<doublereal>(fin);
					}
					else {
						std::cout << "Error reading AMG1r5Dat.ewt2_LABEL=" << fin << std::endl;
						system("pause");
						AMG1r5Dat.ewt2_LABEL = 0.35;
					}
				}
				else {
					printf("WARNING!!! ewt2 not found in file premeshin.txt\n");
					AMG1r5Dat.ewt2_LABEL = 0.35; // .
					printf("ewt2 or F to F amg1r5 =%e\n", AMG1r5Dat.ewt2_LABEL);
					if (bSTOP_Reading) system("pause");
				}
				...
				if (imakesource("stabilization_amg1r5_algorithm", idin)) {
					...
					switch (idin) {
					case 0: AMG1r5Dat.stabilization_amg1r5_algorithm = AMG1R5_OUT_ITERATOR::NONE_only_amg1r5;
						break;
					case 1: AMG1r5Dat.stabilization_amg1r5_algorithm = AMG1R5_OUT_ITERATOR::BiCGStab_plus_amg1r5;
						break;
					case 2: AMG1r5Dat.stabilization_amg1r5_algorithm = AMG1R5_OUT_ITERATOR::FGMRes_plus_amg1r5;
						break;
					case 3: AMG1r5Dat.stabilization_amg1r5_algorithm = AMG1R5_OUT_ITERATOR::Non_Linear_amg1r5;
						break;
					default:
						AMG1r5Dat.stabilization_amg1r5_algorithm = AMG1R5_OUT_ITERATOR::BiCGStab_plus_amg1r5;
						break;
					}
					...
					AMG1r5Dat.stabilization_amg1r5_algorithm = AMG1R5_OUT_ITERATOR::BiCGStab_plus_amg1r5; // default BiCGStab+amg1r5.
		...
		if (fp != nullptr) {
			...
			*/
			...
			for (i = 0; i < lmatmax; ++i) {
				...
				//matlist[i].arr_beta_t_solid[0] = static_cast<doublereal>(fin)* 1E-6;
		...
		// свободной free границе в механике без приложенной силы.
			...
			//	}
				...
				for (i = 0; i < eqin.imaxflD; ++i) {
					...
					if (din > 9999) {
						AMG1r5Dat.b_iluk_amg1r5_LABEL_D = true;
						AMG1r5Dat.nrd_LABEL = din - 10000;
					}
					else {
						AMG1r5Dat.nrd_LABEL = din;
					}
					...
					if (din > 9999) {
						AMG1r5Dat.b_iluk_amg1r5_LABEL_U = true;
						AMG1r5Dat.nru_LABEL = din - 10000;
					}
					else {
						AMG1r5Dat.nru_LABEL = din;
					}
...
#endif
					if ((fin > 0) && (fin < 1.0)) {
						AMG1r5Dat.ecg2_LABEL = fin;
					}
					else {
						std::cout << "Error! : AMG1r5Dat.ecg2_LABEL ==" << fin << std::endl;
						system("pause");
						exit(1);
					}
...
#endif
					if ((fin > 0) && (fin < 1.0)) {
						AMG1r5Dat.ewt2_LABEL = fin;
					}
					else {
						std::cout << "Error! : AMG1r5Dat.ewt2_LABEL ==" << fin << std::endl;
						system("pause");
						exit(1);
					}
