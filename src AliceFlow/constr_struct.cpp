...
doublerealT get_max_array_elm(doublerealT *& a, const integer n) {
	...
	return  thrust::reduce(a, a + n,	-1,	thrust::maximum<doublerealT>());
...
void center_cord3D_ray_tracing(integer iP, int** nvtx, TOCHKA* pa, TOCHKA& p, integer id) {
	...
	const integer ilink0 = static_cast<integer>(nvtx[0][iP]) - 1;
	p.x = 0.5 * (pa[static_cast<integer>(nvtx[1][iP]) - 1].x + pa[ilink0].x);
	p.y = 0.5 * (pa[static_cast<integer>(nvtx[2][iP]) - 1].y + pa[ilink0].y);
	p.z = 0.5 * (pa[static_cast<integer>(nvtx[4][iP]) - 1].z + pa[ilink0].z);
}
...
doublereal volume3D_ray_tracing(integer iP, int** nvtx, TOCHKA* pa) {
	doublereal dx, dy, dz;
	const integer ilink0 = static_cast<integer>(nvtx[0][iP]) - 1;
	dx = pa[static_cast<integer>(nvtx[1][iP]) - 1].x - pa[ilink0].x;
	dy = pa[static_cast<integer>(nvtx[2][iP]) - 1].y - pa[ilink0].y;
	dz = pa[static_cast<integer>(nvtx[4][iP]) - 1].z - pa[ilink0].z;
...
#pragma omp section
		{
			// X
			QSBid.ix11 = 1;
			QSBid.x11 = new doublereal[static_cast<integer>(QSBid.ix11) + 1]; // число границ по оси x
...
#pragma omp section
		{
			// Y
			QSBid.iy11 = 1;
			QSBid.y11 = new doublereal[static_cast<integer>(QSBid.iy11) + 1]; // число границ по оси y
...
#pragma omp section
		{
			// Z
			QSBid.iz11 = 1;
			QSBid.z11 = new doublereal[static_cast<integer>(QSBid.iz11) + 1]; // число границ по оси z
	...
	const integer isize_ijk_block_id = (static_cast<integer>(QSBid.ix11) + 1) * (static_cast<integer>(QSBid.iy11) + 1) * (static_cast<integer>(QSBid.iz11) + 1);
	if (print_log_message) {
...
#pragma omp parallel for
	...
	integer  i= static_cast<integer>(lb)-1;
	//integer k;
	...
	for (i = static_cast<integer>(lb) - 1; i >= 0; i--) {
...
#pragma omp parallel for
	...
	for (m8 = lb - 1; m8 >= 0; m8--) {
		...
		if (b[m8].g.itypegeom == PRISM)
		{
			...
			{
				integer iP_k1 =k1 * (static_cast<integer>(QSBid.ix11) + 1)*(static_cast<integer>(QSBid.iy11) + 1);
				if ((k1 < 0) || (k1 > static_cast<integer>(QSBid.iz11))) {
					// ERROR
					printf("ERROR PRISM\n");
				...
				{
					integer iP_j1=j1 * (static_cast<integer>(QSBid.ix11) + 1) +iP_k1;
					...
					for (integer i1 = block_indexes[m7].iL; i1 < block_indexes[m7].iR; i1++)
					{
						...
						if ((i1 < 0) || (i1 > static_cast<integer>(QSBid.ix11))) {
							// ERROR
							printf("ERROR PRISM\n");
							printf("inx=%d iny=%d inz=%d \n", QSBid.ix11, QSBid.iy11, QSBid.iz11);
...
int myisblock_id_PRISM_only(int lb, BLOCK* &b, TOCHKA p) {
	...
	if ((ib < 0) || (ib >= lb)) {
		...
		printf("ikey=%lld size=%lld ib=%d\n",ikey, static_cast<integer>((static_cast<integer>(QSBid.ix11) + 1)*(static_cast<integer>(QSBid.iy11) + 1)*(static_cast<integer>(QSBid.iz11) + 1)), ib);
...
void enumerate_volume_improved(int* &evt, int &maxelm, integer iflag,
	...
	evt = nullptr;
	evt = new int[static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz)];
	if (evt == nullptr) {
	...
	whot_is_block = new int[static_cast<integer>(inx) * static_cast<integer>(iny) * static_cast<integer>(inz)];
	...
	*/
const integer isize_xyz= static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz);
...
#pragma omp parallel for
	...
	for (int k = 0; k < inz; ++k)
	{
		integer iP_k = k * static_cast<integer>(inx) * static_cast<integer>(iny);
		for (int j = 0; j < iny; ++j)
		{
			integer iP_j = j * static_cast<integer>(inx) + iP_k;
			for (int i = 0; i < inx; ++i)
			{
				integer iP = i + iP_j;
				if (evt[iP] > -1) {
...
#pragma omp parallel for
	...
	{
		integer iP_k = k_63 * static_cast<integer>(inx) * static_cast<integer>(iny);
		for (int j_63 = 0; j_63 < iny; ++j_63)
		{
			integer iP_j = j_63 * static_cast<integer>(inx) + iP_k;
...
void enumerate_volume(int* &evt, int &maxelm, integer iflag,
	...
	if (lu==0) {
		...
		if (lu > 0) {
			...
			tck_int_list=new TOCKA_SHORT_INT[static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz)];
			if (tck_int_list == nullptr) {
				// недостаточно памяти на данном оборудовании.
				printf("Problem: not enough memory on your equipment for evt tck_int_list...\n");
				printf("Please any key to exit...\n");
...
void init_evt_f_alice_improved(int* &evt, integer iflag,
	...
	// увеличению быстродействия.
	//integer m7;
	const integer size_xyz = static_cast<integer>(inx) * static_cast<integer>(iny) * static_cast<integer>(inz);
...
void init_evt_f_alice_improved_obobshenie(int* &evt, integer iflag, 
	...
	bool* bvisit = nullptr;
	bvisit = new bool[static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz)];
	// оператор new не требует проверки на null.
	//if (bvisit == nullptr) {
		// недостаточно памяти на данном оборудовании.
		//printf("Problem: not enough memory on your equipment for bvisit constr struct...\n");
	...
	// 28.07.2019
	for (integer i_a = static_cast<integer>(lb) - 1; i_a >= 0; i_a--) {
...
#pragma omp parallel for
	for (integer iP = 0; iP < size_xyz; ++iP) {
	 ...
	 integer m7 = static_cast<integer>(lb)-1;
...
void constr_neighbour(int* evt, int* ent, int** &neighbour, integer maxelm,
   ...
   // for (i=0; i<(inx); ++i) for (j=0; j<(iny); ++j) for (k=0; k<(inz); ++k) {
	...
	const integer iend_size = static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz);
	const integer iend_size1 = static_cast<integer>(inx)* static_cast<integer>(iny);
	...
	for (integer iscan = 0; iscan < maxelm; iscan++) {
		{
			...
			ic = static_cast<integer>(i) + static_cast<integer>(j) * static_cast<integer>(inx) + static_cast<integer>(k) * static_cast<integer>(inx)* static_cast<integer>(iny);
				...
				if (l < maxelm) {
					...
					iNN = ic + 2 * static_cast<integer>(inx) + 1;
					...
					iSS = ic - 2 * static_cast<integer>(inx) + 1;
...
void my_fill_Domain(int* &evt_f, int i, int j, int k, 
		...
		iP= static_cast<integer>(i)+ static_cast<integer>(j)* static_cast<integer>(inx)+ static_cast<integer>(k)* static_cast<integer>(inx)* static_cast<integer>(iny);
        //iE=(i+1)+j*inx+k*inx*iny+1;
		//iW=(i-1)+j*inx+k*inx*iny+1;
        //iN=i+(j+1)*inx+k*inx*iny+1;
		//iS=i+(j-1)*inx+k*inx*iny+1;
		...
		if (evt_f[iP]>0) {
			...
			integer isize = static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz);
			iE = iP + 2;
			iW = iP;
			iN = iP + 1 + inx;
			iS = iP + 1 - inx;
			integer isize_loc = static_cast<integer>(inx)* static_cast<integer>(iny);
			iT = iP + 1 + isize_loc;
			iB = iP + 1 - isize_loc;
...
void my_fill_Domain_recursive(int* &evt_f, int i, int j, int k, 
		...
		*/
		...
		while (itop>0) {
			   ...
			   const integer size_xyz= static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz);
               const integer iP_k=k* static_cast<integer>(inx)* static_cast<integer>(iny);
			   const integer iP_j=iP_k+1+ static_cast<integer>(j)* static_cast<integer>(inx);
			   const integer iP_k_p1=(static_cast<integer>(k)+1)* static_cast<integer>(inx)* static_cast<integer>(iny);
			   const integer iP_k_m1=(static_cast<integer>(k)-1)* static_cast<integer>(inx)* static_cast<integer>(iny);

	           iP= static_cast<integer>(i)+ static_cast<integer>(j)* static_cast<integer>(inx)+iP_k;
               iE=(static_cast<integer>(i)+1)+iP_j;
		       iW=(static_cast<integer>(i)-1)+iP_j;
               iN= static_cast<integer>(i)+(static_cast<integer>(j)+1)* static_cast<integer>(inx)+iP_k+1;
		       iS= static_cast<integer>(i)+(static_cast<integer>(j)-1)* static_cast<integer>(inx)+iP_k+1;
		       iT= static_cast<integer>(i)+ static_cast<integer>(j)* static_cast<integer>(inx)+iP_k_p1+1;
		       iB= static_cast<integer>(i)+ static_cast<integer>(j)* static_cast<integer>(inx)+iP_k_m1+1;
...
void constr_ptr_temp_part1(int &flow_interior, 
	...
	if (brecursiv_algorithm) {
		...
		STEK *stek = nullptr;
		stek = new STEK[static_cast<integer>(inx)* static_cast<integer>(iny)* static_cast<integer>(inz)];


		 for ( int k = 0; k<inz; ++k)
		{
			integer  iP_k= static_cast<integer>(k)* static_cast<integer>(inx)* static_cast<integer>(iny);
            integer  iP_k_p1=(static_cast<integer>(k)+1)* static_cast<integer>(inx)* static_cast<integer>(iny);
            integer  iP_k_m1=(static_cast<integer>(k)-1)* static_cast<integer>(inx)* static_cast<integer>(iny);

			for (int  j = 0; j<iny; ++j)
			{
				 integer  iP_j= static_cast<integer>(j)* static_cast<integer>(inx) +iP_k;
				 for (int  i = 0; i<inx; ++i)
				{

					integer  iP= static_cast<integer>(i) + iP_j;
			//if (bfirst_visit)
			if (bfirst_visit||(evt_f[iP]!= evt_f[iPgold])) {
				...
				iT = static_cast<integer>(i)+ static_cast<integer>(j)* static_cast<integer>(inx)+iP_k_p1+1; //i + j*inx + (k + 1)*inx*iny + 1;
				iB = static_cast<integer>(i)+ static_cast<integer>(j)* static_cast<integer>(inx)+iP_k_m1+1;//i + j*inx + (k - 1)*inx*iny + 1;
...
#pragma omp parallel for
			...
			if (bisequal) {
				...
				if (evt_etalon1 != nullptr) {
					...
					const integer isize0 = static_cast<integer>(inx) * static_cast<integer>(iny) * static_cast<integer>(inz);

#pragma omp parallel for
					for (integer i_1 = 0; i_1 < isize0; ++i_1) {
						evt_etalon[i_1] = evt_etalon1[i_1];
						evt_etalon1[i_1] = false;// initialization.
					}
				}
...
#pragma omp parallel for
	...
	if (!bfirst_visit) {
		...
		for (int  k = 0; k < inz; ++k)
		{
			...
			for (int  j = 0; j < iny; ++j)
			{
				integer  iP_j = iP_k + static_cast<integer>(j) * static_cast<integer>(inx);
				for (int  i = 0; i < inx; ++i) 
				{
...
void constr_ptr_temp_part2(int &flow_interior,
	...
	const integer isize = static_cast<integer>(inx) * static_cast<integer>(iny)* static_cast<integer>(inz);
	...
	domain_id = new integer[static_cast<integer>(max_domain)];
	...
	for (integer iscan = 0; iscan<maxelm; iscan++) 
		{
			int i = static_cast<int>(tck_int_list[iscan].i);
			int j = static_cast<int>(tck_int_list[iscan].j);
			int k = static_cast<int>(tck_int_list[iscan].k);
			integer iP = static_cast<integer>(i) + static_cast<integer>(j) * static_cast<integer>(inx) + static_cast<integer>(k) * static_cast<integer>(inx)* static_cast<integer>(iny);
			{
			...
			if (!bfind) {
				...
				if (ic >= static_cast<integer>(max_domain) - 1) {
					std::cout << "error! nado uvelichit max_domain count..." << std::endl;
					std::cout << "icount domain ==" << ic << std::endl;
					system("PAUSE");
					exit(1);
...
void constr_nodes(TOCHKA* &pa, int &maxnode, int* &ent,
	...
	const integer iSIZE = (static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1)*(static_cast<integer>(inz) + 1);
	ent = nullptr;
	ent = new int[iSIZE];
	// Оператор new не требует после себя проверки на null.
	//if (ent == nullptr) {
...
#pragma omp parallel for
	...
	for (integer iscan = 0; iscan<maxelm; iscan++) {
			...
			integer k_1 = static_cast<integer>(k)* static_cast<integer>(inx)* static_cast<integer>(iny);
			integer k_2 = static_cast<integer>(k)*(static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1);
			integer k_3= (static_cast<integer>(k) + 1)*(static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1);
			integer j_2 = k_2 + static_cast<integer>(j)*(static_cast<integer>(inx) + 1);
			integer j_3 = (static_cast<integer>(j) + 1)*(static_cast<integer>(inx) + 1) + k_2;
			integer j_4 = static_cast<integer>(j)*(static_cast<integer>(inx) + 1) + k_3;
			integer j_5 = (static_cast<integer>(j) + 1)*(static_cast<integer>(inx) + 1) + k_3;
			integer j_1 = k_1 + static_cast<integer>(j)* static_cast<integer>(inx);
			integer i_1 = static_cast<integer>(i) + j_1;
		...
		if (inDomain) {
			...
			pnew.y = ypos[j];
			pnew.z = zpos[k];
			node = (static_cast<integer>(i) + 1) + j_2;
			//addpoint(pa, maxnode, pnew, ent, node, bvisit);
			if (!bvisit[node]) {
				// добавление, точка ещё не встречалась
			...
			pnew.y = ypos[j + 1];
			pnew.z = zpos[k];
			node = (static_cast<integer>(i) + 1) + j_3;
			//addpoint(pa, maxnode, pnew, ent, node, bvisit);
			if (!bvisit[node]) {
				// добавление, точка ещё не встречалась
			...
			pnew.y = ypos[j];
			pnew.z = zpos[k + 1];
			node = (static_cast<integer>(i) + 1) + j_4;
			//addpoint(pa, maxnode, pnew, ent, node, bvisit);
			if (!bvisit[node]) {
				// добавление, точка ещё не встречалась
			...
			pnew.y = ypos[j + 1];
			pnew.z = zpos[k + 1];
			node = (static_cast<integer>(i) + 1) + j_5;
			//addpoint(pa, maxnode, pnew, ent, node, bvisit);
			if (!bvisit[node]) {
				// добавление, точка ещё не встречалась
	...
	SetLength_point(pa, (static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1)*(static_cast<integer>(inz) + 1), maxnode);
	...
	if (bvisit != nullptr) {
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j];
				pnew.z = zpos[k];
				node = (static_cast<integer>(i) + 1) + static_cast<integer>(j) * (static_cast<integer>(inx) + 1) + static_cast<integer>(k) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j + 1];
				pnew.z = zpos[k];
				node = static_cast<integer>(i) + (static_cast<integer>(j) + 1) * (static_cast<integer>(inx) + 1) + static_cast<integer>(k) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j + 1];
				pnew.z = zpos[k];
				node = (static_cast<integer>(i) + 1) + (static_cast<integer>(j) + 1) * (static_cast<integer>(inx) + 1) + static_cast<integer>(k) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			integer k_1 = static_cast<integer>(k) * static_cast<integer>(inx) * static_cast<integer>(iny);
			integer j_1 = k_1 + static_cast<integer>(j) * static_cast<integer>(inx);
			integer i_1 = static_cast<integer>(i) + static_cast<integer>(j_1);
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j];
				pnew.z = zpos[k + 1];
				node = static_cast<integer>(i) + static_cast<integer>(j) * (static_cast<integer>(inx) + 1) + (static_cast<integer>(k) + 1) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j];
				pnew.z = zpos[k + 1];
				node = (static_cast<integer>(i) + 1) + static_cast<integer>(j) * (static_cast<integer>(inx) + 1) + (static_cast<integer>(k) + 1) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j + 1];
				pnew.z = zpos[k + 1];
				node = static_cast<integer>(i) + (static_cast<integer>(j) + 1) * (static_cast<integer>(inx) + 1) + (static_cast<integer>(k) + 1) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
		...
		for (integer iscan = 0; iscan < maxelm; ++iscan) {
			...
			if (in_dom[i_1]) {
				...
				pnew.y = ypos[j + 1];
				pnew.z = zpos[k + 1];
				node = (static_cast<integer>(i) + 1) + (static_cast<integer>(j) + 1) * (static_cast<integer>(inx) + 1) + (static_cast<integer>(k) + 1) * (static_cast<integer>(inx) + 1) * (static_cast<integer>(iny) + 1);
				//addpoint(pa, maxnode, pnew, ent, node, bvisit);
				if (!bvisit[node]) {
					// добавление, точка ещё не встречалась
...
void constr_nodes_flow(TOCHKA* &pa, int &maxnode, int* &ent,
	...
	for (k = 0; k<inz; ++k) for (j = 0; j<iny; ++j) for (i = 0; i<inx; ++i) {
		...
		if (inDomain) {
			...
			pnew.y = ypos[j];
			pnew.z = zpos[k];
			node = static_cast<integer>(i) + static_cast<integer>(j)*(static_cast<integer>(inx) + 1) + static_cast<integer>(k)*(static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1);
			addpoint(pa, maxnode, pnew, ent, node, bvisit);
			pnew.x = xpos[i + 1];
			pnew.y = ypos[j];
			pnew.z = zpos[k];
	...
	for (integer iscan=0; iscan<maxelm; iscan++) {
		{
			...
			// integer ic = i + j * inx + k * inx*iny;
			 ...
			 integer iP_k= static_cast<integer>(k)*(static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1);
			 integer iP_j= static_cast<integer>(j)*(static_cast<integer>(inx) + 1);
             integer iP_k_p1=(static_cast<integer>(k)+1)*(static_cast<integer>(inx) + 1)*(static_cast<integer>(iny) + 1);
             integer iP_j_p1=(static_cast<integer>(j)+1)*(static_cast<integer>(inx) + 1);
			 //integer i1, i2, i3, i4, i5, i6, i7, i8;

			integer i1 = static_cast<integer>(i) + iP_j + iP_k;
			integer i2 = (static_cast<integer>(i) + 1) + iP_j + iP_k;
			integer i3 = static_cast<integer>(i) + iP_j_p1 + iP_k;
			integer i4 = (static_cast<integer>(i) + 1) + iP_j_p1 + iP_k;
			integer i5 = static_cast<integer>(i) + iP_j + iP_k_p1;
			integer i6 = (static_cast<integer>(i) + 1) + iP_j + iP_k_p1;
			integer i7 = static_cast<integer>(i) + iP_j_p1 + iP_k_p1;
			integer i8 = (static_cast<integer>(i) + 1) + iP_j_p1 + iP_k_p1;
		...
		if (my_ALICE_STACK[top_ALICE_STACK - 1].link != nullptr) {
			if (my_ALICE_STACK[top_ALICE_STACK - 1].link->dlist  ) {
				...
				if (octree1->inum_FD > 0) {
					integer ielm = static_cast<integer>(octree1->inum_FD) - 1; // номер nvtx.
...
#pragma omp parallel for reduction(+ : im)
	for (int k = 0; k < inz; ++k) 
	{
		integer  ic_k = static_cast<integer>(k) * static_cast<integer>(inx) * static_cast<integer>(iny);
		for (int  j = 0; j < iny; ++j)
		{
			integer  ic_j = static_cast<integer>(j) * static_cast<integer>(inx) +ic_k;
			for (int i = 0; i < inx; ++i)
	        {
				integer ic = static_cast<integer>(i) +  ic_j;
				if ((evt_f2[ENUMERATECONTVOL][ic] > 0) && (evt_f2[MASKDOMAINFLUID][ic] == (iDom + 1))) {
					im++;
				}
			}
...
#pragma omp parallel for
	...
	for (int   k = 0; k<(inz); ++k)
	 {
		integer   ic_k= static_cast<integer>(k)* static_cast<integer>(inx)* static_cast<integer>(iny);
		integer  ic_k1= static_cast<integer>(k)*(static_cast<integer>(inx)+1)*(static_cast<integer>(iny)+1);
		integer  ic_k_p1=(static_cast<integer>(k)+1)*(static_cast<integer>(inx)+1)*(static_cast<integer>(iny)+1);
		for (int  j = 0; j<(iny); ++j)
		{
            integer  ic_j = static_cast<integer>(j)* static_cast<integer>(inx) +ic_k;
			integer  ic_j1 = static_cast<integer>(j)*(static_cast<integer>(inx)+1);
			integer  ic_j_P1 =(static_cast<integer>(j)+1)*(static_cast<integer>(inx)+1);
			for (int   i = 0; i<(inx); ++i)
			{
				...
				if ((evt_f2[ENUMERATECONTVOL][ic]>0) && (evt_f2[MASKDOMAINFLUID][ic] == (iDom + 1))) {
					// контрольный объём принадлежит расчётной области.
					//integer i1, i2, i3, i4, i5, i6, i7, i8;
					integer  i1 = static_cast<integer>(i) + ic_j1 + ic_k1;
					integer  i2 = (static_cast<integer>(i) + 1) + ic_j1 + ic_k1;
					integer  i3 = static_cast<integer>(i) + ic_j_P1 + ic_k1;
					integer  i4 = (static_cast<integer>(i) + 1) + ic_j_P1 + ic_k1;
					integer  i5 = static_cast<integer>(i) + ic_j1 + ic_k_p1;
					integer  i6 = (static_cast<integer>(i) + 1) + ic_j1 + ic_k_p1;
					integer  i7 = static_cast<integer>(i) + ic_j_P1 + ic_k_p1;
					integer  i8 = (static_cast<integer>(i) + 1) + ic_j_P1 + ic_k_p1;
	...
	for (integer iscan = 0; iscan<maxelm; iscan++) {
		{
			...
			int j = static_cast<int>(tck_int_list[iscan].j);
			int k = static_cast<int>(tck_int_list[iscan].k);
			integer ic = static_cast<integer>(i) + static_cast<integer>(j) * static_cast<integer>(inx) + static_cast<integer>(k) * static_cast<integer>(inx)* static_cast<integer>(iny);
				...
				if (neighbour[G][i]==0) { // грань лежит на границе расчётной области.
					...
					if (bfind) {
					   ...
					   if (bfind) {
						   ...
						   border_neighbor[gran[G][i]].MCB= static_cast<integer>(ls)+ static_cast<integer>(lw);
					   }
...
#pragma omp parallel for reduction(+:ncell)
	for (integer iscan = 0; iscan < maxelm; ++iscan) {
		...
		if ((i < (inx - 1)) && (j < (iny - 1)) && (k < (inz - 1)))
		{
			...
			integer iP_k = static_cast<integer>(k) * static_cast<integer>(inx)* static_cast<integer>(iny);
			integer iP_k_p1 = (static_cast<integer>(k) + 1) * static_cast<integer>(inx)* static_cast<integer>(iny);
			integer iP_j = static_cast<integer>(j) * static_cast<integer>(inx);
			integer iP_j_p1 = (static_cast<integer>(j) + 1) * static_cast<integer>(inx);
