...
#pragma omp parallel for schedule (static)
	...
	const integer LIMIT_ANEW_m = static_cast<integer>((1.0/(1.0* iKnumber_thread)) * AccumulqtorA_m_SIZE8 * nnz + 1);
...
#pragma omp parallel for schedule (static) 
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		const integer ii_start = row_ind_SR[istr];
		const integer ii_end = row_ind_ER[istr];

		// Сканируем текущую i-ую строку поэлементно
		for (integer ii = ii_start; ii <= ii_end; ++ii) {
			...
			if (istartAnew_m[tid] + ism < LIMIT_ANEW_m) {
				for (integer i_6 = 1; i_6 <= ism; ++i_6) {
					...
					if (fabs(vs1) > 1.0e-37) {
						...
						Ak1 Atemp;
						Atemp.aij = static_cast<real_mix_precision>(vs1);
						Atemp.i = static_cast<integer_mix_precision>(istr);
						Atemp.j = static_cast<integer_mix_precision>(jstr);
	...
	for (int i_9 = 0; i_9 < iKnumber_thread; ++i_9)
	{
		if (istartAnew_m[i_9] - 1 < LIMIT_ANEW_m) {
...
#pragma omp parallel for schedule (static) 
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		const integer ii1_begin = row_ind_AS[istr];
		const integer ii1_end = row_ind_AE[istr];

		for (integer ii1 = ii1_begin; ii1 <= ii1_end; ++ii1) {

			const integer col_ind = Amat.j[ii1];
			const doublerealT left_operand = Amat.aij[ii1];

			// Сканируем col_ind строку правого операнда накапливая сумму.
			const integer i_1start = row_ind_PS[col_ind];
			const integer i_1end = row_ind_PE[col_ind];
			...
			for (integer ii2 = i_1start; ii2 <= i_1end; ++ii2) {
				...
				const doublerealT right_operand = P[ii2].aij;

				const integer iaddind = P[ii2].i;
		...
		if (my_amg_manager.bdiagonal_dominant > 0) {
			...
			vector_sum_m[tid][istr] += static_cast<doublerealT>(additional_to_diagonal);
		}
		...
		for (integer i_61 = 1; i_61 <= ism; ++i_61) {
			...
			const integer jstr61 = index_visit_m[tid][i_61];

			hash_table_m[tid][jstr61] = false; // initialization hash.

			const  doublerealT vs161 = vector_sum_m[tid][jstr61];
	...
	//#endif
			...
			if ((istr == jstr61) && (vs161 < 1.0e-20)) {
				...
				for (integer i_63 = 1; i_63 <= index_size_m[tid]; ++i_63) {
					...
					const integer jstr63 = index_visit_m[tid][i_63];
					const doublerealT vs163 = vector_sum_m[tid][jstr63];
#if doubleintprecision == 1
					printf("i=%lld j=%lld aij=%e\n", istr, jstr63, vs163);
#else
					printf("i=%d j=%d aij=%e\n", istr, jstr63, vs163);
				...
				*/
				...
				for (integer i_62 = 1; i_62 <= index_size_m[tid]; ++i_62) {
					const integer jstr62 = index_visit_m[tid][i_62];
							...
							if (istr == jstr) {
								...
								//}
									...
									for (integer i_8 = 1; i_8 <= index_size_m[tid]; ++i_8) {
										if (i_8 != i_6) {
											const integer jstr_8 = index_visit_m[tid][i_8];
											const doublerealT vs1_8 = vector_sum_m[tid][jstr_8];
											sum_dia += fabs(vs1_8);
										}
									}
									// принудительное сильнейшее усиление диагонали.
									Atemp.aij = static_cast<real_mix_precision>(sum_dia);
...
#pragma omp parallel for
				for (integer i_92 = 0; i_92 < istartAnew_m[i_9]; ++i_92) {
					const integer iz = i_92 + istartAnew2;
					Amat.aij[iz] = AccumulqtorA_m[i_9][i_92].aij;
					Amat.i[iz] = AccumulqtorA_m[i_9][i_92].i;
					Amat.j[iz] = AccumulqtorA_m[i_9][i_92].j;					
				}
...
void my_sparse_matrix_by_matrix_multiplication_RA_1(Ak2& Amat,
	...
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		for (integer i = 1; i <= index_size; ++i) {
			integer jstr = index_visit[i];
	...
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		if (nsizeA > istartAnew + index_size) {
			for (integer i = 1; i <= index_size; ++i) {
				...
				if (fabs(vs1) > 1.0e-37) {
					// Мы не записываем в матрицу чистый ноль.
					Amat.aij[istartAnew] = static_cast<real_mix_precision>(vs1);
					Amat.i[istartAnew] = static_cast<integer_mix_precision>(istr);
					Amat.j[istartAnew] = static_cast<integer_mix_precision>(jstr);
					istartAnew++;
				}
	...
	BAD_STRING_PATCHING*& bsp)
{
	...
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		if (my_amg_manager.bdiagonal_dominant > 0) {
			...
			vector_sum[jstr_diag] += static_cast<doublerealT>(additional_to_diagonal);
		}
		...
		for (integer i = 1; i <= index_size; ++i) {
			...
			if ((istr == jstr61) && (vs161 < 1.0e-20)) {
				...
				printf("bad string: \n");
				for (integer j = 1; j <= index_size; ++j) {

					integer jstr63 = index_visit[j];
				...
				/*
				for (integer j = 1; j <= index_size; ++j) {
					integer jstr62 = index_visit[j];
					if (istr != jstr62) {
						if (vector_sum[jstr62] > 0.0) {
							index_visit[j] = -1; // не существует такого элемента (игнорирование).
				...
				*/
				...
				// Выход из цикла for по переменной i.
				break;
			}
