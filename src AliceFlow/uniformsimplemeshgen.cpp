...
// 01.11.2021
// Убираем повторяющиеся значения в массиве координат.
// За два линейных прогона.
void unical_patch(doublereal* &xpos, int &inx) {

	doublereal* xpos_copy = new doublereal[static_cast<integer>(inx) + 2];
	int j = 0;
	for (int i = 0; i < inx; ++i) {
		if (fabs(xpos[i] - xpos[i + 1]) > 1.0e-23) {
			xpos_copy[j] = xpos[i];
			j++;
		}
	}
	if (fabs(xpos[inx-1] - xpos[inx]) > 1.0e-23) {
		xpos_copy[j] = xpos[inx];
	}
	else {
		j--;
	}
	inx = j;
	delete[] xpos;
	xpos = new doublereal[static_cast<integer>(inx) + 1];
	for (int i = 0; i <= inx; ++i) xpos[i] = xpos_copy[i];
	delete[] xpos_copy;
}
...
void SetLength(doublereal* &ra, integer isizeold, integer isize) 
{
	...
	*/
	...
	// В самом начале построения мы можем передать и nullptr,
	// а к ней лучше не применять delete[].
	if (ra != nullptr) {
...
START_LABEL_FOR_ADAPT:
	...
	// Более качсественный рандом от Володи Фомина.
	...
	int lu, UNION* &my_union, integer &iunion_id_p1)
{
	...
	if (iunion_id_p1 == 0) {
		...
		for (i = 0; i < lu; ++i) {
			...
			// Только если уровень юниона подходящий.
			if (my_union[i].iunion_parent == -1) {
		...
		for (i = 0; i < lu; ++i) {
			...
			// Только если уровень юниона подходящий.
			if (my_union[i].iunion_parent + 1== iunion_id_p1) {

				if ((fabs(my_union[i].xE - my_union[i].xS) >= fabs(my_union[i].zE - my_union[i].zS)) && (fabs(my_union[i].xE - my_union[i].xS) >= fabs(my_union[i].yE - my_union[i].yS))) {
					// больше всех
					for (integer i_65 = 0; i_65 < 17; i_65++) {
						doublereal x_1 = my_union[i].xS + i_65 * 0.0625 * fabs(my_union[i].xE - my_union[i].xS);
						if ((x_1 >= my_union[iunion_id_p1-1].xS) && (x_1 <= my_union[iunion_id_p1 - 1].xE)) {
							addboundary(rxboundary, inumboundaryx, x_1, YZ_PLANE, b, lb, w, lw, s, ls);
						}
					}
				}
				...
				if ((fabs(my_union[i].yE - my_union[i].yS) >= fabs(my_union[i].zE - my_union[i].zS)) && (fabs(my_union[i].yE - my_union[i].yS) >= fabs(my_union[i].xE - my_union[i].xS))) {
					...
					for (integer i_65 = 0; i_65 < 17; i_65++) {
						doublereal y_1 = my_union[i].yS + i_65 * 0.0625 * fabs(my_union[i].yE - my_union[i].yS);
						if ((y_1 >= my_union[iunion_id_p1 - 1].yS) && (y_1 <= my_union[iunion_id_p1 - 1].yE)) {
							addboundary(ryboundary, inumboundaryy, y_1, XZ_PLANE, b, lb, w, lw, s, ls);
						}
					}
				}
				...
				if ((fabs(my_union[i].zE - my_union[i].zS) >= fabs(my_union[i].yE - my_union[i].yS)) && (fabs(my_union[i].zE - my_union[i].zS) >= fabs(my_union[i].xE - my_union[i].xS))) {
					...
					for (integer i_65 = 0; i_65 < 17; i_65++) {
						doublereal z_1 = my_union[i].zS + i_65 * 0.0625 * fabs(my_union[i].zE - my_union[i].zS);
						if ((z_1 >= my_union[iunion_id_p1 - 1].zS) && (z_1 <= my_union[iunion_id_p1 - 1].zE)) {
							addboundary(rzboundary, inumboundaryz, z_1, XY_PLANE, b, lb, w, lw, s, ls);
						}
					}
				}
	...
	int lu, UNION* &my_union, integer &iunion_id_p1)
{
	...
	if (iunion_id_p1 == 0) {
		...
		for (i = 0; i < lu; ++i) {
			...
			// Мы имеем дело с подходящим уровнем вложенности юнионов.
			if (my_union[i].iunion_parent == -1) {
		...
		for (i = 0; i < lu; ++i) {
			...
			// Мы имеем дело с подходящим уровнем вложенности юнионов.
			if (my_union[i].iunion_parent == iunion_id_p1  -1) {
...
RESTARTX_SNAPTO:
	...
	if (iunion_id_p1 == 0)
	{
		...
		for (i = 0; i < lu; ++i) {
			...
			// Только если текущий уровень подходящий.
			if (my_union[i].iunion_parent==-1) {
	...
	int &inxadd, int &inyadd, int &inzadd, integer &iunion_id_p1)
{
	...
	doublereal q = 1.0002; // 1.05 - очень слабо неравномерная, 1.1, 1.2 - умеренно неравномерная, 1.25, 1.5, 2 - сильно неравномерная. 
	...
	//std::wcout << "ypos-1" << std::endl;
	//for (int i_39 = 0; i_39 <= inumboundaryy; i_39++) {

		//std::cout << ryboundary[i_39] << std::endl;
	//}
	//system("pause");
	...
	if (iunion_id_p1 == 0) {
		...
		for (int i_28 = 0; i_28 < lu; i_28++) {
			if (my_union[i_28].active) {

				// В данном добавлении мы обязательно должны добавить внешние границы асемблесов если они еще не добавлены.

				//int isold = inumboundaryx - 1;
				addboundary_rudiment(rxboundary, inumboundaryx, my_union[i_28].xS, YZ_PLANE, b, lb, w, lw, s, ls);
				//if (isold > inumboundaryx - 1) ++inumboundaryx;
				Sort_method<doublereal>(rxboundary, inumboundaryx);
				// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
				unical_patch(rxboundary, inumboundaryx); // 01.11.2021
				//inumboundaryx = isold + 1;

				//isold = inumboundaryx - 1;
				addboundary_rudiment(rxboundary, inumboundaryx, my_union[i_28].xE, YZ_PLANE, b, lb, w, lw, s, ls);
				//if (isold > inumboundaryx - 1) ++inumboundaryx;
				Sort_method<doublereal>(rxboundary, inumboundaryx);
				// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
				unical_patch(rxboundary, inumboundaryx); // 01.11.2021
		...
		for (int i_28 = 0; i_28 < lu; i_28++) {
			if (my_union[i_28].active) {
				...
				//std::cout << i_28 << " " << my_union[i_28].yS << " " << my_union[i_28].yE << std::endl;

				//int isold = inumboundaryy - 1;
				addboundary_rudiment(ryboundary, inumboundaryy, my_union[i_28].yS, XZ_PLANE, b, lb, w, lw, s, ls);
				//if (isold > inumboundaryy - 1) ++inumboundaryy;
				Sort_method<doublereal>(ryboundary, inumboundaryy);
				// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
				unical_patch(ryboundary, inumboundaryy); // 01.11.2021
				//inumboundaryy = isold + 1;

				//isold = inumboundaryy - 1;
				addboundary_rudiment(ryboundary, inumboundaryy, my_union[i_28].yE, XZ_PLANE, b, lb, w, lw, s, ls);
				//if (isold > inumboundaryy - 1) ++inumboundaryy;
				Sort_method<doublereal>(ryboundary, inumboundaryy);
				// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
				unical_patch(ryboundary, inumboundaryy); // 01.11.2021
		...
		for (int i_28 = 0; i_28 < lu; i_28++) {
			if (my_union[i_28].active) {
				...
				//int isold = inumboundaryz - 1;
				addboundary_rudiment(rzboundary, inumboundaryz, my_union[i_28].zS, XY_PLANE, b, lb, w, lw, s, ls);
				//if (isold > inumboundaryz - 1) ++inumboundaryz;
				Sort_method<doublereal>(rzboundary, inumboundaryz);
				// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
				unical_patch(rzboundary, inumboundaryz); // 01.11.2021
				//inumboundaryz = isold + 1;

				//isold = inumboundaryz - 1;
				addboundary_rudiment(rzboundary, inumboundaryz, my_union[i_28].zE, XY_PLANE, b, lb, w, lw, s, ls);
				//if (isold > inumboundaryz - 1) ++inumboundaryz;
				Sort_method<doublereal>(rzboundary, inumboundaryz);
				// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
				unical_patch(rzboundary, inumboundaryz); // 01.11.2021
	...
	for (i=0; i<(inumboundaryx); ++i) 
	{
		...
		integer n2=static_cast<integer>((ixintervalcount[i]-1)/2); // округление в меньшую сторону
		doublereal qn2=q;
        ...
        for (k=iposmark; k<=iposmark+ixintervalcount[i]-2; ++k)
		{
			if (!bgeom) {
				if (lb < 2 ) {
					...
					doublereal nvolkow = (static_cast<doublereal>(k - iposmark)) / ((doublereal)(ixintervalcount[i] - 1.0));
   ...
   // необходимость их добавления обусловлена требованнем АЛИС сеток.
	...
	// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
	unical_patch(xpos, inx); // 01.11.2021
	...
	// Пластина
		SetLength(ypos, iyoldsize, iposmark + 10);
		iyoldsize = iposmark + 10;
		for (k = iposmark; k < iposmark + 10; ++k)
		{
			dy = 0.06e-3 / (10.0 - 1);
			ypos[k] = b[0].g.yS + (k - iposmark+1) * dy;
		}
		iposmark = iposmark + 10 - 1;

		// Шипчик.
		const int in_shypchik = 20;
		SetLength(ypos, iyoldsize, iposmark + in_shypchik);
		iyoldsize = iposmark + in_shypchik;
		for (k = iposmark; k < iposmark + in_shypchik; ++k)
		{
			dy = 20.0e-6 / (1.0* in_shypchik - 1);
			ypos[k] = b[1].g.yE + (k - iposmark+1) * dy;
		}
		iposmark = iposmark + in_shypchik - 1;

		// Погранслой

		doublereal q63 = 1.1;

		int n2 = (int)((70-2)); // округление в меньшую сторону
		doublereal qn2 = q63;
		for (integer i1 = 1; i1 < n2; i1++) qn2 *= q63; // возведение в степень.
		std::cout << "individual mesh generator.\n";
		std::cout << b[0].g.yE << "  " << b[1].g.yE << std::endl;
		system("PAUSE");
		doublereal b1 = ((b[0].g.yE-b[1].g.yE)) * (q63 - 1.0) / ((q63*qn2 - 1.0));

		SetLength(ypos, iyoldsize, iposmark + 70);
		iyoldsize = iposmark + 70;
		for (k = iposmark; k < iposmark + 70; ++k)
		{
			if (k == iposmark) {
				ypos[k] = b[1].g.yE + 20.0e-6 + b1;
			}
			else {
				ypos[k] = ypos[k-1]  + b1 * pow(q63, (static_cast<doublereal>(1.0*k - 1.0*iposmark )));
			}
		}
		iposmark = iposmark + 70 - 1;
		...
		/*for (i = 0; i < inumboundaryy; ++i)
		{
			...
			int n2 = (int)((iyintervalcount[i] - 1)); // округление в меньшую сторону
			doublereal qn2 = q63;
			for (integer i1 = 1; i1 < n2; i1++) qn2 *= q63; // возведение в степень.
			doublereal b1 = (ryboundary[i + 1] - ryboundary[i]) * (q63 - 1.0) / ((qn2 - 1.0));
			...
			{// 13.01.2022
				...
				// Сгущающаяся снизу геометрическая прогрессия по оси Oy.
				if (k == iposmark) {
					ypos[k] = ryboundary[i];
				}
				else {
					ypos[k] = ypos[k-1] + b1 * pow(q63, (static_cast<doublereal>(k - iposmark - 1.0)));
				}
...
*/
			...
			for (k=iposmark; k<=iposmark+iyintervalcount[i]-2; ++k)
			{
				if (!bgeom) {
					if (lb < 2) {
						...
						doublereal betavolkov = 1.0 / (sqrt(1.0 - deltavolkov));
						doublereal nvolkow = (static_cast<doublereal>(k - iposmark)) / (static_cast<doublereal>(iyintervalcount[i] - 1.0));
	...
	// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
	unical_patch(ypos, iny); // 01.11.2021
	...
	if (b_one_cell_z) {
		...
		for (i = 0; i < (inumboundaryz); ++i)
		{
			...
			integer n2 = static_cast<integer>((izintervalcount[i] - 1) / 2); // округление в меньшую сторону
			doublereal qn2 = q;
			for (integer i1 = 1; i1 < n2; i1++) qn2 *= q; // возведение в степень.
			...
			for (k = iposmark; k <= iposmark + izintervalcount[i] - 2; ++k)
			{
				if (!bgeom) {
					if (lb < 2) {
						...
						doublereal betavolkov = 1.0 / (sqrt(1.0 - deltavolkov));
						doublereal nvolkow = (static_cast<doublereal>(k - iposmark)) / (static_cast<doublereal>(izintervalcount[i] - 1.0));
		...
		// Сохраняет порядок сортировки и удаляет повторяющиеся координаты узлов сетки.
		unical_patch(zpos, inz); // 01.11.2021
	...
	int &inxadd, int &inyadd, int &inzadd, integer &iunion_id_p1)
{
        ...
        for (k=iposmark; k<=iposmark+ixintervalcount[i]-2; ++k)
		{
			if (!bgeomx) {
				...
				if (lb == 1) {
					...
					doublereal betavolkov = 1.0 / (sqrt(1.0 - deltavolkov));
					doublereal nvolkow = (static_cast<doublereal>(k - iposmark)) / (static_cast<doublereal>(ixintervalcount[i] - 1.0));
	...
	for (i=0; i<inumboundaryy; ++i) 
	{
		...
		// чтобы число узлов було всегда чётное
		if ((iyintervalcount[i]-2)%2==0) iyintervalcount[i]++;
		integer n2=static_cast<integer>((iyintervalcount[i]-1)/2); // округление в меньшую сторону
	...
	int &inxadd, int &inyadd, int &inzadd, integer &iunion_id_p1)
{
	...
	if (iunion_id_p1 == 0) {
		...
		// необходимость их добавления обусловлена требованнем АЛИС сеток.
		for (int i_28 = 0; i_28 < lu; i_28++) {
			if ((my_union[i_28].active)&&(my_union[i_28].iunion_parent==-1)) {
...
#pragma omp parallel for reduction(+ : ismarker)
	...
	else if (iunion_id_p1 == 0) {
		// Полностью исключаем все блоки в других юнионах, отличных от кабинета.
		for (integer m7 = 0; m7 < lb; m7++) {
			if (b[m7].iunion_id > 0) {

				// Если блок принадлежит активному юниону а не кабинету.
				if (my_union[b[m7].iunion_id - 1].active)  {
					// инициализация, на случай если блоки не будут распознаны.
					block_indexes[m7].iL = -1;
					block_indexes[m7].iR = -2;
					block_indexes[m7].jL = -1;
					block_indexes[m7].jR = -2;
					block_indexes[m7].kL = -1;
					block_indexes[m7].kR = -2;
				}
			}
		}
	}
	...
	/*for (integer i = 0; i < lb; ++i) {
		// инициализация, на случай если блоки не будут распознаны.
		//<< "  " << (b[i].iunion_id == 0 ? "cabinet" : my_union[b[i].iunion_id-1].name)
		if (block_indexes[i].iL < 0) {
			std::cout << b[i].name << "  body iL error!!!." << b[i].iunion_id  << std::endl;
			system("PAUSE");
		}
		if (block_indexes[i].iR < 0) {
			std::cout << b[i].name << "  body iR error!!!." << b[i].iunion_id << std::endl;
			system("PAUSE");
		}
		if (block_indexes[i].jL < 0) {
			std::cout << b[i].name << "  body jL error!!!." << b[i].iunion_id << std::endl;
			system("PAUSE");
		}
		if (block_indexes[i].jR < 0) {
			std::cout << b[i].name << "  body jR error!!!." << b[i].iunion_id << std::endl;
			system("PAUSE");
		}
		if (block_indexes[i].kL < 0) {
			std::cout << b[i].name << "  body kL error!!!." << b[i].iunion_id << std::endl;
			system("PAUSE");
		}
		if (block_indexes[i].kR < 0) {
			std::cout << b[i].name << "  body kL error!!!." << b[i].iunion_id << std::endl;
			system("PAUSE");
		}
	}
...
//	for (i = 0; i<(inumboundaryx); ++i) {
	//	alphascal = (rxboundary[i + 1] - rxboundary[i]) / (rxboundary[inumboundaryx] - rxboundary[0]);
		//inowintervalcount = static_cast<integer>(alphascal*inx);
	//	if (inowintervalcount < min_elem_in_x_element) inowintervalcount = min_elem_in_x_element;
		//ixintervalcount[i] = inowintervalcount;
	//}
	...
	*/
	...
	if (1) {
		...
		while (1) {
			...
			//std::cout << "max_size_ratio_x=" << max_size_ratio_x << " etalon =" << etalon_max_size_ratio * 1.1 << std::endl;
			//system("PAUSE");
	...
	//for (integer i = 0; i<(inumboundaryy); ++i) {
		//alphascal = (ryboundary[i + 1] - ryboundary[i]) / (ryboundary[inumboundaryy] - rxboundary[0]);
		//inowintervalcount = static_cast<integer>(alphascal*iny);
		//if (inowintervalcount < min_elem_in_y_element) inowintervalcount = min_elem_in_y_element;
		//iyintervalcount[i] = inowintervalcount;
	//}
...
#pragma omp parallel for
	...
	*/
	...
	//for (i = 0; i<(inumboundaryz); ++i) {
		//alphascal = (rzboundary[i + 1] - rzboundary[i]) / (rzboundary[inumboundaryz] - rzboundary[0]);
		//inowintervalcount = static_cast<integer>(alphascal*inz);
		//if (inowintervalcount < min_elem_in_z_element) inowintervalcount = min_elem_in_z_element;
	//	izintervalcount[i] = inowintervalcount;
	//}
	...
	int& inxadd, int& inyadd, int& inzadd, integer& iunion_id_p1)
{
	...
	LINE_DIRECTIONAL side_id = LINE_DIRECTIONAL::X_LINE_DIRECTIONAL;
	bool bvisit = false;
	if (fabs(base_min_side - fabs(b[0].g.xE - b[0].g.xS)) < 1.0e-30) {
		side_id = LINE_DIRECTIONAL::X_LINE_DIRECTIONAL; bvisit = true;
	}
	if (fabs(base_min_side - fabs(b[0].g.yE - b[0].g.yS)) < 1.0e-30) {
		side_id = LINE_DIRECTIONAL::Y_LINE_DIRECTIONAL; bvisit = true;
	}
	if (fabs(base_min_side - fabs(b[0].g.zE - b[0].g.zS)) < 1.0e-30) {
		side_id = LINE_DIRECTIONAL::Z_LINE_DIRECTIONAL;
		bvisit = true;
	}
	...
	if (bvisit) {
		...
		std::cout << "error! line directional unknown in function cad_geometry_octree_meshgen.\n";
		system("PAUSE");
	}
