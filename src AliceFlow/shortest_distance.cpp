...
// поправки давления
void my_elmatr_quad_short_dist(int iP, equation3D* &sl, equation3D_bon* &slb,  
	TOCHKA* pa, int** nvtx, int*** neighbors_for_the_internal_node, int maxelm, doublereal dbeta) {
...
void calcdistwallCFX(FLOW &f, integer ls, integer lw, WALL* w) {
	...
	for (integer i=0; i<4; ++i) potent[i]=new doublereal[static_cast<integer>(f.maxelm)+ static_cast<integer>(f.maxbound)];
	// матрица
	equation3D_bon* slb = NULL;
		slb = new equation3D_bon[static_cast<integer>(f.maxbound)];
	equation3D* sl = NULL;
		sl=new equation3D[static_cast<integer>(f.maxelm)];
	doublereal* rthdsd = NULL;
		rthdsd=new doublereal[static_cast<integer>(f.maxelm) + static_cast<integer>(f.maxbound)];
	// инициализация
	for (integer i=0; i<4; ++i) for (integer j=0; j< static_cast<integer>(f.maxelm)+ static_cast<integer>(f.maxbound); ++j) potent[i][j]=0.0;
	// сборка матрицы

	// Граничные условия Дирихле обязательно 
	// должны собираться в первую очередь
	doublereal dbeta=1.0; // первый порядок точности.
    for (integer i=0; i< static_cast<integer>(f.maxbound); ++i) {
	...
	for (integer i=0; i< static_cast<integer>(f.maxelm); ++i) {
