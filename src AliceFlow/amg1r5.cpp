...
*/
// 28.03.2019 Увеличил максимальное число уровней сетки до 100.

#pragma once
#ifndef AMG1R5_CPP
#define AMG1R5_CPP 1

const bool RELAX_OMP = false;

const doublereal MY_DOPUSK_AMG1R5 = 1.0e-30;
...
// Для iluk сглаживателя. Начало.
bool bflag_visit_amg1r5[100] = { false };

LEVEL_ADDITIONAL_DATA* milu2_amg1r5 = nullptr;

bool bflag_repeat_buffer_ilu = true;
...
// Для iluk сглаживателя. Окончание.

// 31.12.2019 iluk сглаживатель для amg1r5 алгоритма.
...
/* Subroutine */ integer amg1r5_fgmres_version(doublereal *a, integer *ia, integer *ja,
	...
	bool &bOkfgmres_amg1r5, BLOCK* &b, int &lb, SOURCE* &s_loc, int &ls);
...
/* Subroutine */ integer amg1r5_fgmres_version_matrix_Assemble2(
	...
	integer *nwt, integer *ntr, integer *ierr, integer iVar,
	integer n,
	bool &bOkfgmres_amg1r5, bool bprint_message);
	...
	BLOCK* &b, int &lb, SOURCE* &s_loc, int &ls)
{
	...
	// 31 июля 2021 Сделано умножение матрицы на вектор в Hyb =2*Ell формате.
...
#pragma omp parallel for
	...
	data = new doublereal[string_size * maxelm];
	indices = new integer[string_size * maxelm];

	data_bound = new doublereal[string_size_bound * maxbound];
	indices_bound = new integer[string_size_bound * maxbound];


	// Перепаковка в Ell формат.
#pragma omp parallel for
	for (integer i77 = 0; i77 < string_size * maxelm; ++i77) {
		data[i77] = 0.0;
	}

#pragma omp parallel for
	for (integer i77 = 0; i77 < string_size_bound * maxbound; ++i77) {
		data_bound[i77] = 0.0;
	}

#pragma omp parallel for
	for (integer i77 = 0; i77 < maxelm; ++i77) {
		...
		{
			const integer element_offset = i77 + jstart * maxelm;
		...
		{
			const integer element_offset = i77 + j77 * maxelm;
...
#pragma omp parallel for
	for (integer i77 = maxelm; i77 < maxelm + maxbound; ++i77) {
		...
		for (integer j77 = rowbeg; j77 < rowend; ++j77)
		{
			const integer element_offset = i77-maxelm + jstart * maxbound;
		...
		{
			const integer element_offset = i77-maxelm + j77 * maxbound;
	...
	cudaStatus = cudaMalloc((void**)&dev_data, (maxelm* string_size) * sizeof(doublereal));
	if (cudaStatus != cudaSuccess) {
	...
	// Allocate GPU buffers for three vectors (two input, one output)    .
	cudaStatus = cudaMalloc((void**)&dev_indices, (maxelm* string_size) * sizeof(integer));
	if (cudaStatus != cudaSuccess) {
	...
	// Allocate GPU buffers for three vectors (two input, one output)    .
	cudaStatus = cudaMalloc((void**)&dev_data_bound, (maxbound * string_size_bound) * sizeof(doublereal));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc dev_val failed!");
		system("PAUSE");
		exit(1);
	...
	// Allocate GPU buffers for three vectors (two input, one output)    .
	cudaStatus = cudaMalloc((void**)&dev_indices_bound, (maxbound * string_size_bound) * sizeof(integer));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc dev_col_ind failed!");
		system("PAUSE");
		exit(1);
	...
	cudaStatus = cudaMemcpy(dev_data, data, maxelm* string_size * sizeof(doublereal), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
	...
	// Copy input vectors from host memory to GPU buffers.
	cudaStatus = cudaMemcpy(dev_indices, indices, maxelm * string_size * sizeof(integer), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
	...
	// Copy input vectors from host memory to GPU buffers.
	cudaStatus = cudaMemcpy(dev_data_bound, data_bound, maxbound * string_size_bound * sizeof(doublereal), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
	...
	// Copy input vectors from host memory to GPU buffers.
	cudaStatus = cudaMemcpy(dev_indices_bound, indices_bound, maxbound * string_size_bound * sizeof(integer), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
	...
	MatrixEllPackItpackByVectorKernel << <128, 128 >> > (dev_data, dev_indices, string_size, dev_V, dev_tmp, maxelm, 0);
	...
	MatrixEllPackItpackByVectorKernel << <128, 128 >> > (dev_data_bound, dev_indices_bound, string_size_bound, dev_V, dev_tmp, maxbound, maxelm);
	// Check for any errors launching the kernel
	cudaStatus = cudaGetLastError();
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, " MatrixEllPackItpackByVectorKernel launch failed: %s\n", cudaGetErrorString(cudaStatus));
...
#pragma omp parallel for
	...
	if (n75<30000) {
		...
		if (iVar == TEMP) {
			...
			// 31.07.2021
			// Рекомендация Федюшкина А.И. и Аболдуева И.М.
			epsilon75 = 1.0e-8; // 1.0e-10; Ok + 31.07.2021
		}
		if (iVar == PAM) {
...
#pragma omp parallel for 
		for (integer i77 = 0; i77<n75; ++i77) {
			doublereal pibuf75 = ri75[i77] + (pi75[i77] - vi75[i77] * wi75)*bet75;
			pi75[i77] = pibuf75;
		}
...
#pragma omp parallel for
		for (integer i77 = 0; i77 < n75; ++i77) {
			y75[i77] = 0.0; // Если начинать не с нуля то небудет сходимости для PAM !.
			y76[i77+1] = 0.0;//+1
			pi76[i77+1] = pi75[i77];//+1
		}
...
#pragma omp parallel for
	for (integer i77 = 0; i77<n75; ++i77) {
		s75[i77] = ri75[i77] - al75*vi75[i77];
	}
...
#pragma omp parallel for
	for (integer i77 = 0; i77 < n75; ++i77) {
		z75[i77] = 0.0; // Если начинать не с нуля то небудет сходимости для PAM !.
	}
...
#pragma omp parallel for
	for (integer i77 = 0; i77<n75; ++i77) {
		//dx75[i77]+=al75*pi75[i77]+wi75*s75[i77]; // так было без предобуславливателя
		dx75[i77] += al75*y75[i77] + wi75*z75[i77]; // так стало с предобуславливателем
		ri75[i77] = s75[i77] - wi75*t75[i77];
	}
	...
	bool bprint_mesage_diagnostic = yes_print_amg;
...
/* Subroutine */ integer amg1r5_Vorst_modification_matrix_Assemble2(doublereal *a, integer *ia, integer *ja,
	...
	doublereal *eps, integer *madapt, integer *nrd, integer *nsolco,
	integer *nru, doublereal *ecg1, doublereal *ecg2, doublereal *ewt2,
	integer *nwt, integer *ntr, integer *ierr,  integer n, bool bprint_message)
{
...
#pragma omp parallel for
		...
		// печать невязки на консоль
		bool bprint_mesage_diagnostic = bprint_message;
		if (bprint_mesage_diagnostic) {
			if ((icount75 % 10) == 0) {
				std::cout << "iter  residual" << std::endl;
			}
...
/* ===> WARNINGS */
    ...
    if ((*isym == 1 && asym != 0.) || (*isym > 1 && asym == 0.) || 
		(*irow0 == 1 && nazer != nnu) || (*irow0 > 1 && nazer == nnu)) {
	 if (yes_print_amg) {
...
L20:
	...
	integer* a_i = new integer[ndjtr + 1];// JTR
	...
	integer iws_loc = 0;
	if (i__2 != 1)  iws_loc = imaxw[i__2 - 1] + 2 - imin[i__2];
	integer iend2 = iw[imaxw[i__2] + myi_max(0, iws_loc) + 1] - 1;
	for (integer ii_1 = 1; ii_1 <= iend2; ii_1++) a_i[ii_1] = static_cast<integer>(a[jtrst + ii_1 - 1]);
	integer i__2memo = i__2;
	...
	rwsrt_(&i__2, &ecg1, &ecg2, ierr, &a[1], &ia[1], &ja[1], &iw[1], &
		imin[1], &imax[1], &imaxw[1], &ifg[1], &a_i[1], &time[1], &
		ndjtr, ium, &mdjtr);
...
/*       NDIR: AVAILABLE WORK SPACE */
	...
	for (integer ii_1 = 1; ii_1 <= imax[i__2]-imin[i__2]+ 1; ii_1++) u_i[ii_1] = static_cast<integer>(u[irst + ii_1-1]);
	...
	pcol_(&i__2, ierr, &ia[1], &ja[1], &iw[1], &imin[1], &imax[1], &imaxw[
		1], &icg[1], &ifg[1], &u_i[1], &a_i[1], &time[1], ndicg, &
			ndir, ium, &mdicg, &mdir, &iias);

	
	
	for (integer ii_1 = 1; ii_1 <= iend2; ii_1++) a[jtrst+ii_1-1] = static_cast<doublereal>(a_i[ii_1]);
	delete[] a_i;
	...
	//pwint_(&i__2, &ewt2, &ichk, &mmax, &a[1], &ia[1], &ja[1], &iw[1], &
		...
		//ium, mda, mdja, &iajas);
	pwint_(&i__2, &ewt2, &ichk, &mmax, &a[1], &ia[1], &ja[1], &iw[1], &
		imin[1], &imax[1], &iminw[1], &imaxw[1], &ifg[1], &icg[1], &u_i[1],
		&time[1], ierr, irow0, &ncolx, nda, ndja, ndicg, ndia,
		ium, mda, mdja, &iajas);
	...
	opdfn_(&k, ierr, &mmax, &a[1], &ia[1], &ja[1], &iw[1], &imin[1], &
		imax[1], &iminw[1], &imaxw[1], &icg[1], &ifg[1], &u_i[1], &
		nstcol[1], &ncolx, &time[1], nda, ndja, ium, mda, mdja, &
		iajas);

	
	// Сохраняем результаты в глобальной переменной.
	for (integer ii_1 = 1; ii_1 <= imax[i__2memo] - imin[i__2memo] + 1; ii_1++) u[irst + ii_1-1] = static_cast<doublereal>(u_i[ii_1]);
	delete[] u_i;
...
/* ===> CREATE F-POINTS AROUND ABOVE C-TOCHKA */
	...
	i__ = jtr[j];
    //i__ = static_cast<integer>(jtr[j]); // 24.03.2019 PVS-Studio
	if (icg[i__] != -2) {
	    goto L77;
	}
	icg[i__] = -1;
...
/*       YET THERE) */
	...
	for (jj = ia[i__] + 1; jj <= i__2; ++jj) {
	    ...
	    ++ifg[iii];
	    iiii = iii - ilo1;
	    if (ir[iiii] != 0) { // 24.03.2019 PVS-Studio
			goto L76;
	    }
		//if ((static_cast<integer>(ir[iiii])) != 0) {
		//goto L76;
	    //}
	     ir[iiii] = itop; // 24.03.2019 PVS-Studio
...
L78:
	...
	--ifg[i__];
	ii = i__ - ilo1;
	if (ir[ii] != 0) { // 24.03.2019 PVS-Studio
	    goto L87;
	}
	//if ((static_cast<integer>(ir[ii])) != 0) {
	  //  goto L87;
	//}
	ir[ii] = itop; // 24.03.2019 PVS-Studio
...
L95:
    iip = in;
    in = ir[in]; // 24.03.2019 PVS-Studio
	//in =static_cast<integer>(ir[in]);
    ir[iip] = 0; // 24.03.2019 PVS-Studio
	//ir[iip] = 0.0;
    goto L90;
	...
	*fac, doublereal *resi, integer *levels)
{
    ...
    extern /* Subroutine */ integer relx_(integer *, integer *, doublereal *, 
	    doublereal *, doublereal *, integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, /*real*/ unsigned int *, char ch_direct);
	unsigned int tnew=0;
...
/*             COARSE GRID DETERMINATION. */
	...
	nptsf = 0;
			...
			mink = 100000000;
#endif
	//mink = 1000000; // старое оригинальное значение.
	ifi = 1;
...
/*         COMPONENT TO ZERO */
	...
	if (nsp <= 0) {
		printf("error!!! nsp=%lld = nda - jhi; nda=%lld jhi=%lld\n",nsp,*nda,jhi);
		system("PAUSE");
		exit(1);
	}
	
	integer* a_i = new integer[nsp+1];
	for (integer ii_1 = 1; ii_1 <= nsp; ii_1++) a_i[ii_1] = static_cast<integer>(a[jhi + ii_1]);
...
/* ===>   FACTORIZATION ALLREADY DONE */
	...
	if (nsp <= 0) {
		printf("FACTORIZATION ALLREADY DONE\n");
		printf("error!!! nsp=%lld = nda - jhi; nda=%lld jhi=%lld\n", nsp, *nda, jhi);
		system("PAUSE");
		exit(1);
	}
	integer* a_i = new integer[nsp + 1];
	//for (integer ii_1 = 0; ii_1 <= nsp; ii_1++) a_i[ii_1] = static_cast<integer>(a[jhi + 1 + ii_1]);
	for (integer ii_1 = 1; ii_1 <= nsp; ii_1++) a_i[ii_1] = static_cast<integer>(a[jhi + ii_1]);
	...
	ndrv_(&npoint, &ja[jhi + 1], &ja[jhi + np + 1], &ja[jhi + (np << 1) +
		1], &ia[ilo], &ja[jlo], &a[jlo], &f[ilo], &u[ilo], &nsp, &a_i[0],
		&a[jhi + 1], &esp, &path, &flag__);
...
/*     RELX_FROZEN                                              SUBROUTINE */

/* ....................................................................... */

/* Subroutine */ integer relx_frozen(integer* k, integer* irel, doublereal* a,
...
/*          = 4:   FULL SWEEP: FF -- C -- COLORS (HIGHEST FIRST) */
	...
	//printf("getchar %lld",*irel); system("pause");
...
L100:
	...
	if (AMG1r5Dat.b_iluk_amg1r5_LABEL_D || AMG1r5Dat.b_iluk_amg1r5_LABEL_U) {
...
L200:
	...
	// Здесь будет добавлено iluk разложение, сглаживатель.
	if (AMG1r5Dat.b_iluk_amg1r5_LABEL_D || AMG1r5Dat.b_iluk_amg1r5_LABEL_U) {
	...
	if ((ch_direct=='U' && AMG1r5Dat.b_iluk_amg1r5_LABEL_U)||
		(ch_direct == 'D' && AMG1r5Dat.b_iluk_amg1r5_LABEL_D)) {

		integer iunknown_size = imax[*k] - imin[*k] + 1;
		...
		for (integer i__ = imin[*k]; i__ <= i__1; ++i__) {
			...
			//u[i__] = s / a[ia[i__]];
			milu2_amg1r5[*k - 1].zbuf[i__ - imin[*k]+1] = s - u[i__] * a[ia[i__]];
			/* L220: */
		}
		
		lusol_1patchforRUMBA(iunknown_size, milu2_amg1r5[*k - 1].zbuf, milu2_amg1r5[*k - 1].zbuf2, milu2_amg1r5[*k - 1]);
	...
	{
		// Внимание !!! нумерация начинается с единицы.
		//printf("%lld \n",*k);
		//system("pause");
...
if (yes_print_amg) {
	...
	printf("( -----------------------------------------)\n");
	printf("( 1 RWSRT   ,%u,   11 INTADD  \n", time[1]);
	printf(" ,%u 2 PRE-COL ,%u,   12 RESCAL   ,%u\n", t[1 - 1], time[2], t[2 - 1]);
	printf(" 3 CHK-COL ,%u,   13 RELAX    ,%u 4 INTERPOL\n", time[3], t[3 - 1]);
	printf(",%u,   14 V-*      ,%u 5 RESTRICT,\n", time[4], t[4 - 1]);
	printf("%u,   15 OTHERS   ,%u 6 OPDFN   ,%u, \n", time[5], t[5 - 1], time[6]);
	printf("  16 CONJ-GRAD ,%u 7 TRUNC   ,%u,   17 YALE-\n", t[6 - 1], time[7]);
	printf("SMP ,%u 8 OTHERS  ,%u,   18 ------   ,%u)\n", t[7 - 1], time[8], t[8 - 1]);
...
/*              S      - S=RANDOM */
	// rundom работает именно на ошибке округления.
	// Здесь нельзя менять преобразование типов. 24.01.2019
    ret_val = exp(*s) * 100.;
...
/*  ******  MOVE JU NEXT TO JL  ***************************************** */
    jlmax = isp[il + *n] - 1;  // PVS-Studio
	//jlmax = (static_cast<integer>(isp[il + *n])) - 1;
    ju = jl + jlmax;
    jumax = isp[iu + *n] - 1; // PVS-Studio
...
/* L1: */
	...
	rsp[ju + j - 1] = rsp[jutmp + j - 1];// Это же один и тот же массив
    }
...
void amg_loc_memory_networkT(doublereal* &val, integer* &col_ind, integer* &row_ptr, integer n, integer nnz,
	...
	BLOCK* &b, int &lb, int &ls,  integer maxelm, bool bnonlinear)
{

	// Графовый метод создан и функционирует только для теплопередачи в твёрдом теле.
	//const integer iVar = TEMP;
	...
	if (res_sum>res_sum_previos) // защита от повторного холостого запуска экономит время конечного пользователя.
	{
		...
		*/
		...
		integer nrd = 0;
		//nrd=1131;
		nrd = AMG1r5Dat.nrd_LABEL;
		integer nsolco = 0;
		nsolco = 110;
		integer nru = 0;
		//nru=1131;
		nru = AMG1r5Dat.nru_LABEL;
		doublereal ecg1 = 0.0;
		ecg1 = 0.0;
		doublereal ecg2 = 0.0;
		//ecg2=0.25;
		ecg2 = AMG1r5Dat.ecg2_LABEL;
		doublereal ewt2 = 0.0;
		//ewt2=0.35;
		ewt2 = AMG1r5Dat.ewt2_LABEL;
		integer nwt = 0;
		nwt = 2;
		integer ntr = 0;
		ntr = 0;
		...
		*/
		...
		//for (integer k=0; k<(maxelm+maxbound); ++k) QuickSortCSIR_amg(ja, a, ia[k+1], ia[k+2]-2); // первый элемент всегда диагональный.
		//for (integer k=0; k<(maxelm+maxbound); ++k) QuickSortCSIR_amg(ja, a, ia[k+1]-1, ia[k+2]-2); 

		for (integer k = 1; k <= nnu; ++k) ig[k - 1] = ia[k]; // инициализация.
	...
	// см. equation3DtoCRS.
        ...
        for (integer k=0; k<maxelm; ++k) {
			...
			if (fabs(sl[k].ap) > nonzeroEPS) {
                a[ik]=sl[k].ap/alpharelax;
				ja[ik]= static_cast<integer>(sl[k].iP)+1;
                ia[k]=my_imin(ik+1,ia[k]);
				ik++;
			}
			if ((sl[k].iE>-1) && (fabs(sl[k].ae) > nonzeroEPS)) {
                a[ik]=-sl[k].ae;
				ja[ik]= static_cast<integer>(sl[k].iE)+1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iN>-1) && (fabs(sl[k].an) > nonzeroEPS)) {
                a[ik]=-sl[k].an;
				ja[ik]= static_cast<integer>(sl[k].iN)+1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iT>-1) && (fabs(sl[k].at) > nonzeroEPS)) {
                a[ik]=-sl[k].at;
				ja[ik]= static_cast<integer>(sl[k].iT)+1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}		
			if ((sl[k].iS>-1) && (fabs(sl[k].as) > nonzeroEPS)) {
                a[ik]=-sl[k].as;
				ja[ik]= static_cast<integer>(sl[k].iS)+1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iW>-1) && (fabs(sl[k].aw) > nonzeroEPS)) {
				a[ik]=-sl[k].aw;
				ja[ik]= static_cast<integer>(sl[k].iW)+1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iB>-1) && (fabs(sl[k].ab) > nonzeroEPS)) {
				a[ik]=-sl[k].ab;
				ja[ik]= static_cast<integer>(sl[k].iB)+1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			...
			if ((sl[k].iE2>-1) && (fabs(sl[k].ae2) > nonzeroEPS)) {
				a[ik] = -sl[k].ae2;
				ja[ik] = static_cast<integer>(sl[k].iE2) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iN2>-1) && (fabs(sl[k].an2) > nonzeroEPS)) {
				a[ik] = -sl[k].an2;
				ja[ik] = static_cast<integer>(sl[k].iN2) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iT2>-1) && (fabs(sl[k].at2) > nonzeroEPS)) {
				a[ik] = -sl[k].at2;
				ja[ik] = static_cast<integer>(sl[k].iT2) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iS2>-1) && (fabs(sl[k].as2) > nonzeroEPS)) {
				a[ik] = -sl[k].as2;
				ja[ik] = static_cast<integer>(sl[k].iS2) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iW2>-1) && (fabs(sl[k].aw2) > nonzeroEPS)) {
				a[ik] = -sl[k].aw2;
				ja[ik] = static_cast<integer>(sl[k].iW2) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iB2>-1) && (fabs(sl[k].ab2) > nonzeroEPS)) {
				a[ik] = -sl[k].ab2;
				ja[ik] = static_cast<integer>(sl[k].iB2) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			...
			if ((sl[k].iE3>-1) && (fabs(sl[k].ae3) > nonzeroEPS)) {
				a[ik] = -sl[k].ae3;
				ja[ik] = static_cast<integer>(sl[k].iE3) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iN3>-1) && (fabs(sl[k].an3) > nonzeroEPS)) {
				a[ik] = -sl[k].an3;
				ja[ik] = static_cast<integer>(sl[k].iN3) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iT3>-1) && (fabs(sl[k].at3) > nonzeroEPS)) {
				a[ik] = -sl[k].at3;
				ja[ik] = static_cast<integer>(sl[k].iT3) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iS3>-1) && (fabs(sl[k].as3) > nonzeroEPS)) {
				a[ik] = -sl[k].as3;
				ja[ik] = static_cast<integer>(sl[k].iS3) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iW3>-1) && (fabs(sl[k].aw3) > nonzeroEPS)) {
				a[ik] = -sl[k].aw3;
				ja[ik] = static_cast<integer>(sl[k].iW3) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iB3>-1) && (fabs(sl[k].ab3) > nonzeroEPS)) {
				a[ik] = -sl[k].ab3;
				ja[ik] = static_cast<integer>(sl[k].iB3) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			...
			if ((sl[k].iE4>-1) && (fabs(sl[k].ae4) > nonzeroEPS)) {
				a[ik] = -sl[k].ae4;
				ja[ik] = static_cast<integer>(sl[k].iE4) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iN4>-1) && (fabs(sl[k].an4) > nonzeroEPS)) {
				a[ik] = -sl[k].an4;
				ja[ik] = static_cast<integer>(sl[k].iN4) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iT4>-1) && (fabs(sl[k].at4) > nonzeroEPS)) {
				a[ik] = -sl[k].at4;
				ja[ik] = static_cast<integer>(sl[k].iT4) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iS4>-1) && (fabs(sl[k].as4) > nonzeroEPS)) {
				a[ik] = -sl[k].as4;
				ja[ik] = static_cast<integer>(sl[k].iS4) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iW4>-1) && (fabs(sl[k].aw4) > nonzeroEPS)) {
				a[ik] = -sl[k].aw4;
				ja[ik] = static_cast<integer>(sl[k].iW4) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
			if ((sl[k].iB4>-1) && (fabs(sl[k].ab4) > nonzeroEPS)) {
				a[ik] = -sl[k].ab4;
				ja[ik] = static_cast<integer>(sl[k].iB4) + 1;
				ia[k] = my_imin(ik + 1, ia[k]);
				ik++;
			}
        ...
        for (integer k=0; k<maxbound; ++k) {
			if (fabs(slb[k].aw) > nonzeroEPS) {
               // val[ik]=slb[k].aw/alpharelax;
				a[ik]=slb[k].aw; // релаксация для граничных узлов не применяется.
				...
				ja[ik]= static_cast<integer>(slb[k].iW)+1;
				ia[maxelm + k] = my_imin(ik + 1, ia[maxelm + k]);
				ik++;
			}
			if ((slb[k].iI>-1) && (fabs(slb[k].ai) > nonzeroEPS)) {
				a[ik]=-slb[k].ai;
				ja[ik]= static_cast<integer>(slb[k].iI)+1;
				ia[maxelm + k] = my_imin(ik + 1, ia[maxelm + k]);
	...
	WALL* &w, int &lw, bool* &boundary)
{
	...
	/*{
		if (w[0].ifamily == WALL_BOUNDARY_CONDITION::DIRICHLET_FAMILY) {
			//if ((fabs(w[jw].g.yS - y) < 1.0e-20) && ((x > w[jw].g.xS) && (x < w[jw].g.xE) && (z < w[jw].g.zE) && (z > w[jw].g.zS))) {
				...
				std::cout << m.row_ptr[i] << "  " << m.row_ptr[i + 1] << std::endl;
				for (integer k = m.row_ptr[i]; k < m.row_ptr[i + 1]; ++k) {

					std::cout << "val = " << m.val[k] << "  " << m.col_ind[k] << std::endl;
				}
	...
	const doublereal nonzeroEPS = 1e-37; // для отделения вещественного нуля
	doublereal res_sum = 1.0;// Защита от повторного срабатывания Невязка большая нуля большая тепкущей стартовой невязки.
	//res_sum = 0.0;
	//res_sum = 1.0;
	...
	if (res_sum > res_sum_previos) // защита от повторного холостого запуска экономит время конечного пользователя.
	{
		...
		*/
		...
		// обязателная инициализация.
		for (integer k = 0; k <= nnu + 1; ++k) ia[k] = nna + 1; // инициализация.//<=
...

		{
			...
			if (steady_or_unsteady_global_determinant == PHYSICAL_MODEL_SWITCH::UNSTEADY_TEMPERATURE) {
				...
				// alpharelax = 0.99999; - сильнейшее искажение нестационарного решения.
					...
					if (m.col_ind[k1] == k) {
						...
						if (fabs(m.val[k1]) > nonzeroEPS) {
							...
							if (((boundary != nullptr)&&(!boundary[k])) && (m.row_ptr[k + 1] - 1 > m.row_ptr[k])) {
								...
								a[ik] = m.val[k1]/ alpharelax;
								f[k] += (1.0 - alpharelax)* m.val[k1]* u[k] / alpharelax;
							}
							else {
								a[ik] = m.val[k1];
							}
			...
			ia[nnu] = static_cast<integer>(nnz_1) + 1;
			if (m.row_ptr[n] != nna) {
				if (bprintmessage) {
					printf("nnz_1=%d nna=%lld nnz input=%lld\n", nnz_1, nna, m.row_ptr[n]);
				}

				// По видимому мы храним в матрице некоторое количество нулевых элементов,
				// возможно для сохранения симметричности портрета матрицы.
				//system("pause");// Убрал 05.11.2021
			}
		...
		*/
		...
		if ((SECOND_T_SOLVER_ID_SWITCH::AMG1R5_SECOND_T_SOLVER == iswitchsolveramg_vs_BiCGstab_plus_ILU6)&&
		    (AMG1R5_OUT_ITERATOR::NONE_only_amg1r5 == AMG1r5Dat.stabilization_amg1r5_algorithm)){
			// amg - особенно хорош для поправки давления в SIMPLE алгоритме.
			// алгоритм 1985 года.
			...
			amg1r5_(a, ia, ja,
				...
				&nwt, &ntr, &ierr);
		         ...
		         (AMG1R5_OUT_ITERATOR::BiCGStab_plus_amg1r5 == AMG1r5Dat.stabilization_amg1r5_algorithm)){
			// 23-24 декабря 2017.
			// 13.10.2018
			// BiCGStab + amg1r5.
			...
			amg1r5_Vorst_modification_matrix_Assemble2(a, ia, ja,
				...
				&eps, &madapt, &nrd, &nsolco,
				&nru, &ecg1, &ecg2, &ewt2,
				&nwt, &ntr, &ierr, n, bprintmessage);
		}
		else if ((SECOND_T_SOLVER_ID_SWITCH::AMG1R5_SECOND_T_SOLVER==iswitchsolveramg_vs_BiCGstab_plus_ILU6)&&
		(AMG1R5_OUT_ITERATOR::FGMRes_plus_amg1r5 == AMG1r5Dat.stabilization_amg1r5_algorithm)){
			// FGMres + amg1r5.
			//31 декабря 2017.
			...
			// В качестве внешнего итерационного процесса используется 
			// алгоритм Юсефа Саада и Мартина Г. Шульца FGMRes. 
			// amg1r5 используется только как
			// многосеточный предобуславливатель.
			...
			amg1r5_fgmres_version_matrix_Assemble2(a, ia, ja,
				...
				&eps, &madapt, &nrd, &nsolco,
				&nru, &ecg1, &ecg2, &ewt2,
				&nwt, &ntr, &ierr, iVar,  n, bOkfgmres_amg1r5, bprintmessage);
		}
...
void Bi_CGStab_internal3(equation3D* &sl, equation3D_bon* &slb,
	...
	BLOCK* &b, int &lb, SOURCE* &s_loc, int &ls, integer inumber_iteration_SIMPLE,
	integer*& color, integer dist_max, bool breordering_for_parallel, WALL* &w, int &lw);
	...
	int * &whot_is_block, bool bprint_message)
{
	...
	const integer isize0 = static_cast<integer>(maxelm) + static_cast<integer>(maxbound);

	 // На случай если память не была выделена.
	 if (dX0==nullptr) {
		 
	    dX0=new doublereal[isize0];
...
/*       IG              5.4*NNU */
	...
	integer nnu=0; // число неизвестных.
	nnu=static_cast<integer>(maxelm)+ static_cast<integer>(maxbound);
	...
	*/
	...
	if ((iVar == TEMP) && (breakRUMBAcalc_for_nonlinear_boundary_condition  )) {
		ncyc = 1012; // 2 - V цикла.
		eps = res_sum*sqrt(static_cast<doublereal>(maxelm) + static_cast<doublereal>(maxbound))*0.0002;
	}
	if (bSIMPLErun_now_for_temperature) {
		//ncyc = 102; // 2 - V цикла.
	...
	for (integer k=0; k<=nnu+1; ++k) amgGM.ia[k]=nna+1; // инициализация.
	...
	//std::cout << "alpharelax1=" << alpharelax1 << "alpharelax=" << alpharelax << std::endl;
	//printf("%e %e\n", alpharelax1, alpharelax);
	//system("pause");
	...
	// см. equation3DtoCRS.
        ...
        for (integer k=0; k<maxelm; ++k) {
			...
			if (fabs(sl[k].ap) > nonzeroEPS) {
				...
				if (bSIMPLErun_now_for_natural_convection) {
						...
						(iVar == VELOCITY_Z_COMPONENT)) 
					{
						...
						amgGM.a[ik] = sl[k].ap / alpharelax;
						amgGM.f[k] += (1.0 - alpharelax) * sl[k].ap * amgGM.u[k] / alpharelax;
						sl[k].ap = sl[k].ap / alpharelax;
					}
				}
				amgGM.ja[ik]=static_cast<integer>(sl[k].iP)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}
			if ((sl[k].iE>-1) && (fabs(sl[k].ae) > nonzeroEPS)) {
                amgGM.a[ik]=-sl[k].ae;
				amgGM.ja[ik]= static_cast<integer>(sl[k].iE)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}
			if ((sl[k].iN>-1) && (fabs(sl[k].an) > nonzeroEPS)) {
                amgGM.a[ik]=-sl[k].an;
				amgGM.ja[ik]= static_cast<integer>(sl[k].iN)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}
			if ((sl[k].iT>-1) && (fabs(sl[k].at) > nonzeroEPS)) {
                amgGM.a[ik]=-sl[k].at;
				amgGM.ja[ik]= static_cast<integer>(sl[k].iT)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}		
			if ((sl[k].iS>-1) && (fabs(sl[k].as) > nonzeroEPS)) {
                amgGM.a[ik]=-sl[k].as;
				amgGM.ja[ik]= static_cast<integer>(sl[k].iS)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}
			if ((sl[k].iW>-1) && (fabs(sl[k].aw) > nonzeroEPS)) {
				amgGM.a[ik]=-sl[k].aw;
				amgGM.ja[ik]= static_cast<integer>(sl[k].iW)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}
			if ((sl[k].iB>-1) && (fabs(sl[k].ab) > nonzeroEPS)) {
				amgGM.a[ik]=-sl[k].ab;
				amgGM.ja[ik]= static_cast<integer>(sl[k].iB)+1;
				amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
				ik++;
			}
			...
			if (sl[k].bE2) {
				if ((sl[k].iE2 > -1) && (fabs(sl[k].ae2) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].ae2;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iE2) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bN2) {
				if ((sl[k].iN2 > -1) && (fabs(sl[k].an2) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].an2;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iN2) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bT2) {
				if ((sl[k].iT2 > -1) && (fabs(sl[k].at2) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].at2;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iT2) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bS2) {
				if ((sl[k].iS2 > -1) && (fabs(sl[k].as2) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].as2;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iS2) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bW2) {
				if ((sl[k].iW2 > -1) && (fabs(sl[k].aw2) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].aw2;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iW2) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bB2) {
				if ((sl[k].iB2 > -1) && (fabs(sl[k].ab2) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].ab2;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iB2) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			...
			if (sl[k].bE3) {
				if ((sl[k].iE3 > -1) && (fabs(sl[k].ae3) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].ae3;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iE3) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bN3) {
				if ((sl[k].iN3 > -1) && (fabs(sl[k].an3) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].an3;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iN3) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bT3) {
				if ((sl[k].iT3 > -1) && (fabs(sl[k].at3) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].at3;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iT3) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bS3) {
				if ((sl[k].iS3 > -1) && (fabs(sl[k].as3) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].as3;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iS3) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bW3) {
				if ((sl[k].iW3 > -1) && (fabs(sl[k].aw3) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].aw3;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iW3) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bB3) {
				if ((sl[k].iB3 > -1) && (fabs(sl[k].ab3) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].ab3;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iB3) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			...
			if (sl[k].bE4) {
				if ((sl[k].iE4 > -1) && (fabs(sl[k].ae4) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].ae4;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iE4) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bN4) {
				if ((sl[k].iN4 > -1) && (fabs(sl[k].an4) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].an4;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iN4) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bT4) {
				if ((sl[k].iT4 > -1) && (fabs(sl[k].at4) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].at4;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iT4) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bS4) {
				if ((sl[k].iS4 > -1) && (fabs(sl[k].as4) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].as4;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iS4) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bW4) {
				if ((sl[k].iW4 > -1) && (fabs(sl[k].aw4) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].aw4;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iW4) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
			if (sl[k].bB4) {
				if ((sl[k].iB4 > -1) && (fabs(sl[k].ab4) > nonzeroEPS)) {
					amgGM.a[ik] = -sl[k].ab4;
					amgGM.ja[ik] = static_cast<integer>(sl[k].iB4) + 1;
					amgGM.ia[k] = my_imin(ik + 1, amgGM.ia[k]);
					ik++;
				}
			}
        ...
        for (integer k=0; k<maxbound; ++k) {
			//if (slb[k].iI == -1) 
			{
				if (fabs(slb[k].aw) > nonzeroEPS) {
					// val[ik]=slb[k].aw/alpharelax;
					amgGM.a[ik] = slb[k].aw; // релаксация для граничных узлов не применяется.
					/*if ((slb[k].iI>-1) && (fabs(slb[k].ai) > nonzeroEPS)) {
						 // Внимание !!! было произведено тестирование: один вариант был с нижней релаксацией для граничных узлов,
						 // а второй вариант был без нижней релаксации на граничных узлах. Было выяснено, что для сходимости
						 // более благоприятен вариант без нижней релаксации на граничных узлах.
					...
					amgGM.ja[ik] = static_cast<integer>(slb[k].iW) + 1;
					amgGM.ia[maxelm + k] = my_imin(ik + 1, amgGM.ia[maxelm + k]);
					ik++;
				}
			}/*
			else {
				if (fabs(slb[k].aw) > nonzeroEPS) {
					// val[ik]=slb[k].aw/alpharelax;
					amgGM.a[ik] = slb[k].aw / alpharelax; // релаксация для граничных узлов не применяется.
					amgGM.f[maxelm + k] += (1.0 - alpharelax)*slb[k].aw*amgGM.u[maxelm + k] / alpharelax;
			...
			if ((slb[k].iI>-1) && (fabs(slb[k].ai) > nonzeroEPS)) {
				amgGM.a[ik]=-slb[k].ai;
				amgGM.ja[ik]= static_cast<integer>(slb[k].iI)+1;
				amgGM.ia[maxelm + k] = my_imin(ik + 1, amgGM.ia[maxelm + k]);
				// Это очень важный вопрос и он требует проверки !
       ...
       //for (integer k=0; k<(maxelm+maxbound); ++k) QuickSortCSIR_amg(ja, a, ia[k+1], ia[k+2]-2); // первый элемент всегда диагональный.
		...
		for (integer k=1; k<=nnu; ++k) amgGM.ig[k-1]=amgGM.ia[k]; // инициализация.
		...
		for (integer i23 = 0; i23 < isize0; i23++) {
		...
		if (iVorst_version == 0) {
			...
			amg1r5_non_linear_(amgGM.a, amgGM.ia, amgGM.ja,
				...
				&eps, &madapt, &nrd, &nsolco,
				&nru, &ecg1, &ecg2, &ewt2,
				&nwt, &ntr, &ierr,b,lb,maxelm, isize0, whot_is_block);
		}
	...
	int * &whot_is_block, bool bprint_message)
{
	              ...
	              // iVorst_version == 1 - amg1r5 алгоритм является предобуславливателем к алгоритму Хенка Ван дер Ворста BiCGStab.
				   ...
				   if (bmemory_local) {
					   ...
					   amg_global_memory(sl, slb, maxelm,  maxbound, dV, dX0, alpharelax, iVar, bLRfree,m, ifrontregulationgl, ibackregulationgl, iVorst_version, b, lb, s_loc, ls,w,lw, whot_is_block, bprint_message);
				   }
}
