...
// 25,09,2021 Еще более дальнобойная интерполяция.
#include "my_long_range_amg_interpolation_FFFC.cpp"
								...
								if (this_is_C_node[Amat[is0].j]  ) {
									...
									if (fabs(Amat[is0].aij) > maxelem_threshold_theta) {
										if (fabs(sumP) < RealZERO) {
											...
											//if (fabs(Amat[is0].aij) > maxelem_threshold*theta) {
											if (fabs(Amat[is0].aij) > maxelem_threshold_theta) {
												P[icount1].j = static_cast<integer_mix_precision>(i8);
												P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat[is0].j]);
												...
												if (fabs(Amat[is0_loc].aij) > maxelem_threshold_loc_magic) {
													...
													if (this_is_C_node[Amat[is0_loc].j]  ) {
														if (Amat[is0_loc].j != istr_etalon) {
															...
															P[icount1].j = static_cast<integer_mix_precision>(i8);
															P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat[is0_loc].j]);
															//P[icount1].aij = fabs(Amat[is0].aij) / sumP;
								...
								if (this_is_C_node[Amat.j[is0]]  ) {
									...
									if (fabs(Amat.aij[is0]) > maxelem_threshold_theta) {
										if (fabs(sumP) < RealZERO) {
											...
											if (fabs(Amat.aij[is0]) > maxelem_threshold_theta) {
												P[icount1].j = static_cast<integer_mix_precision>(i8);
												P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat.j[is0]]);
												...
												P[icount1].aij = static_cast<real_mix_precision>(fabs(Amat.aij[is0]) / SumPall);
												++icount1;
												if (icount1 >= nsizePR * n) {
													printf("memory error!!!\n");
													printf("not enough memory for the interpolation operator.\n");
											...
											if (fabs(Amat.aij[is0_loc]) > maxelem_threshold_loc_magic) {
												...
												if (this_is_C_node[Amat.j[is0_loc]]  ) {
													if (Amat.j[is0_loc] != istr_etalon) {
														...
														P[icount1].j = static_cast<integer_mix_precision>(i8);
														P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat.j[is0_loc]]);
														//P[icount1].aij = fabs(Amat.aij[is0]) / sumP;
														if (fabs(sumP_loc) < RealZERO) {
															printf("error 3.0 ! division by zero. sumP_loc =%e\n", sumP_loc);
															//system("pause");
														...
														P[icount1].aij = static_cast<real_mix_precision>(multiplyer_nu * fabs(Amat.aij[is0_loc]) / sumP_loc);
														++icount1;
														if (icount1 >= nsizePR * n) {
															printf("memory error!!!\n");
															printf("not enough memory for the interpolation operator.\n");
...
void my_interpolation_procedure_number3A_PMIS_parallel8(integer& the_number_of_neighbors_that_are_not_C_nodes_gl,
	...
	doublerealT& theta83, bool& btreshold_on_new_vetv, integer& ifrom_re_operation_protection,
	bool& from_re_operation_protection0, doublerealT& magic82, doublerealT*& threshold_quick_all,
	doublerealT*& threshold_quick_only_negative, bool b_aggressive_coarsening)
{

	// Не добавляем новые С узлы 18.09.2021
	//bool b_aggressive_coarsening = true;

	static int inum_agg_pass = -1;
	inum_agg_pass++;
			...
			for (integer i8 = 1; i8 <= n_a[ilevel - 1]; ++i8) {
				...
				// оператора интерполяции а значит и оператора Галёркина.
					...
					*/
					...
					if (ii1 == iend_marker_position) baddC = false; // Условие Дирихле мы не добавляем в С узлы. 01.10.2021
					if (ii1 + 1 == iend_marker_position) baddC = false; // однородное условие Неймана 01.10.2021

					if (b_aggressive_coarsening) {
						// Все эти включения дополнительного разрежения приводят к полной деградации сходимости.
						// Можно получить Acomp 1.59 на 7pt Laplasian но сходимость сильнейшим образом деградирует так что решение не сойдется ир за 2000 итераций.

						// Огрубление аггрессивное. Мы не добавляем новые С узлы.

						
#ifdef MINGW_COMPILLER
						const int iran = rand() % 100;						
#else
						const int iran = uid(gen);
#endif

						if (iran > 10) {

							// Только в 10% случяаев мы добавляем узел.
...
#pragma omp critical
...
#else
			printf("number (fabs(sumP) < RealZERO) = %d; n_a=%d.\n", iadditionalCstatistic_gl, n_a[ilevel - 1]);
#endif
		}
		//system("pause");
		//exit(1);
	...
	*/
	...
	if (!byes_add) {
		...
		const int ntid = number_cores();

		integer LOC_P_LIM_SIZE = -1;
		double dsize = 2.2*(1.0/ ntid)*nsizePR*n_a[0];
		LOC_P_LIM_SIZE = static_cast<integer>(dsize);

		Ak1** P_loc = new Ak1*[ntid];
		integer* isize_P_loc = new integer[ntid];
		for (int itid = 0; itid < ntid; ++itid) {
			isize_P_loc[itid] = 0;
		...
		bool** this_is_Strong_C_node = new bool*[ntid];
		for (int itid = 0; itid < ntid; ++itid) {
...
#pragma omp parallel for
		...
		integer** iStrongC_nodes_ind = new integer*[ntid];
		for (int itid = 0; itid < ntid; ++itid) {
										...
										if (aij_abs > maxelem_threshold_theta) {
											...
											else */
											{
												...
												//if (aij_abs > maxelem_threshold_theta) {
													...
													Ptmp.aij = static_cast<real_mix_precision>(aij_abs / SumPall);
																...
																if (Amat.j[is0_loc] != istr_etalon) {
																	...
																	if (this_is_Strong_C_node[itid][Amat.j[is0_loc]]) {
																		...
																		//if (sumP_loc > RealZERO) {
																			...
																			Ptmp.aij = static_cast<real_mix_precision>(multiplyer_nu * aij_loc_abs / sumP_loc);
						...
						for (integer is0 = ii1; (is0 <= iend_1); ++is0) {
								...
								if (this_is_C_node[Amat.j[is0]]  ) {
									...
									if (Amat.abs_aij[is0] > maxelem_threshold_theta) {
										...
										//if (fabs(Amat.aij[is0]) > maxelem_threshold*theta) {
										//if (Amat.abs_aij[is0] > maxelem_threshold_theta) {
											P[icount1].j = static_cast<integer_mix_precision>(inode);
											P[icount1].i = static_cast<integer_mix_precision>(C_numerate[Amat.j[is0]]);
											//P[icount1].aij = fabs(Amat.aij[is0]) / sumP;
											if (fabs(SumPall) < RealZERO) {
												printf("error 1.0 ! division by zero. SumPall =%e\n", SumPall);
											...
											P[icount1].aij = static_cast<real_mix_precision>(Amat.abs_aij[is0] / SumPall);
											++icount1;
											if (icount1 >= nsizePR * n) {
												printf("memory error!!!\n");
												printf("not enough memory for the interpolation operator.\n");
													...
													{ // Обязательная проверка
														...
														{// можно убирать - сходимость НЕ портится
															//if (Amat.j[is0_loc] != iFpoint) {
																...
																P[icount1].aij = static_cast<real_mix_precision>(multiplyer_nu * Amat.abs_aij[is0_loc] / sumP_loc);
																++icount1;
																if (icount1 >= nsizePR * n) {
																	printf("memory error!!!\n");
																	printf("not enough memory for the interpolation operator.\n");
