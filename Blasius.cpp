...
void boundarylayer_info(FLOW* &f, TEMPER &t, integer flow_interior_count, WALL* w, integer lw) {
	...
	iP=iPC;
	while (f[0].neighbors_for_the_internal_node[S_SIDE][0][iP]<f[0].maxelm) iP=f[0].neighbors_for_the_internal_node[S_SIDE][0][iP]; // перемотка в начало.
	...
	while (iP<f[0].maxelm) {
		...
		if ((p.y>0.0) && (p.y<length_plate)) {
			...
			integer iBT=iP;
			while (f[0].neighbors_for_the_internal_node[B_SIDE][0][iBT]<f[0].maxelm) iBT=f[0].neighbors_for_the_internal_node[B_SIDE][0][iBT]; // перемотка в начало пластины.
			...
			while ((f[0].neighbors_for_the_internal_node[T_SIDE][0][iBT]<f[0].maxelm) && (VYT<deltascal*U_inf)) {
				iBT=f[0].neighbors_for_the_internal_node[T_SIDE][0][iBT]; // удаляемся от пластины перпендикулярно её плоскости.
				VYB=f[0].potent[VELOCITY_Y_COMPONENT][f[0].neighbors_for_the_internal_node[B_SIDE][0][iBT]];
			    VYT=f[0].potent[VELOCITY_Y_COMPONENT][iBT];
				zB=zT;
			...
			while ((t.neighbors_for_the_internal_node[T_SIDE][0][iBT]<t.maxelm)&&(t.potent[iBT]>Temp_critical)) {
				iBT=t.neighbors_for_the_internal_node[T_SIDE][0][iBT]; // удаляемся от пластины перпендикулярно её плоскости.
				TempB=t.potent[t.neighbors_for_the_internal_node[B_SIDE][0][iBT]];
			    TempT=t.potent[iBT];
				zB=zT;
