						...
						if (fabs(sumP) < RealZERO) {
							...
							if (bpositive_connections) {
								...
								if (fabs(Amat.aij[is0]) > maxelem_threshold*theta) {
									...
									P[icount1].i = (integer_mix_precision)(C_numerate[Amat.j[is0]]);
									P[icount1].aij = (real_mix_precision)(fabs(Amat.aij[is0]) / sumP);
									icount1++;
										...
										for (integer is0_loc = ii1_loc; (is0_loc <= nnz_a[ilevel - 1] + iadd) && (Amat.i[is0_loc] == Amat.i[ii1_loc]); is0_loc++) {
											if (Amat.j[is0_loc] != Amat.i[ii1_loc]) {
												if (this_is_C_node[Amat.j[is0_loc]]  ) {
													...
													if (fabs(Amat.aij[is0_loc]) > maxelem_threshold_loc*theta) {
														P[icount1].j = (integer_mix_precision)(i8);
														P[icount1].i = (integer_mix_precision)(C_numerate[Amat.j[is0_loc]]);
														//P[icount1].aij = fabs(Amat.aij[is0]) / sumP;
														P[icount1].aij = (real_mix_precision)(multiplyer_nu*fabs(Amat.aij[is0_loc]) / sumP_loc);
														icount1++;
														if (icount1 >= nsizePR*n) {
															printf("memory error!!!\n");
															printf("not enough memory for the interpolation operator.\n");
									...
									if (fabs(Amat.aij[is0]) > maxelem_threshold_theta) {
										if (fabs(sumP) < RealZERO) {
											...
											if (fabs(Amat.aij[is0]) > maxelem_threshold_theta) {
												...
												P[icount1].aij = (real_mix_precision)(fabs(Amat.aij[is0]) / SumPall);
												icount1++;
												if (icount1 >= nsizePR*n) {
													printf("memory error!!!\n");
													printf("not enough memory for the interpolation operator.\n");
							...
							if (Amat.j[is0] != Amat.i[ii1]) {
								if (this_is_C_node[Amat.j[is0]]  ) {
									...
									if (bmodify_interpol4_amg && ((icsos == 1) && (icsosF == 0))) {
										...
										if (is0_candidate != -1) {
											P[icount1].j = (integer_mix_precision)(i8);
											P[icount1].i = (integer_mix_precision)(C_numerate[Amat.j[is0_candidate]]);
											P[icount1].aij = (real_mix_precision)(fabs(Amat.aij[is0_candidate]) / (sumP + sum1));
											icount1++;
											if (icount1 >= nsizePR*n) {
												printf("memory error!!!\n");
												printf("not enough memory for the interpolation operator.\n");
										...
										if (fabs(Amat.aij[is0]) > maxelem_threshold_theta) {
											P[icount1].j = (integer_mix_precision)(i8);
											P[icount1].i = (integer_mix_precision)(C_numerate[Amat.j[is0]]);
											P[icount1].aij = (real_mix_precision)(fabs(Amat.aij[is0]) / (sumP + sum1));
											icount1++;
											if (icount1 >= nsizePR*n) {
												printf("memory error!!!\n");
												printf("not enough memory for the interpolation operator.\n");
			...
			for (integer i8 = 1; i8 <= n_a[ilevel - 1]; i8++) if (this_is_F_node[i8]  ) {
				...
				if ((false) && (icsos == 1)) {
					...
					if (((icsos == 1) || (icsos == 2) || (icsos == 3)) && (icsosF != 0)) {
													...
													if (Amat.j[is0_loc] != Amat.i[ii1_loc]) {
														if (this_is_C_node[Amat.j[is0_loc]]  ) {
															...
															if (fabs(Amat.aij[is0_loc]) > maxelem_threshold_loc*magic) {
																...
																P[icount1].aij = (real_mix_precision)(mult1*fabs(Amat.aij[is0_loc]) / sumP_loc1);
						...
						for (integer i_95 = icount1_mem; i_95 < icount1; i_95++) {
							P[i_95].aij = (real_mix_precision)(P[i_95].aij / sum_ok);
						}
													...
													if (this_is_C_node[Amat.j[is0_loc]]  ) {
														for (i_97 = 0; i_97 < ilength_n; i_97++) {
															if (Amat.j[is0_loc] == jposition_in_P[i_97]) {
																...
																P[icount1_frozen + i_97].aij += (real_mix_precision)((my_mult * fabs(Amat.aij[is0_loc])) / (sumP * sum23));
																break;
															}
														}
													}
