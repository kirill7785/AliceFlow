...
// BiCGStab и FGMRes(m)  и V-цикл amg в качестве предобуславливателя.
...
bool solution_phase(Ak2& Amat, // Матрица СЛАУ в CRS формате.
	...
	INIT_SELECTOR_CASE_CAMG_RUMBAv_0_14 imyinit
	 ...
	 doublerealT dr_grid_complexity = (doublerealT)(amg_pp.dr_grid_complexity);
	 bool debug_reshime = amg_pp.debug_reshime;
	 doublerealT dapply_ilu_max_pattern_size = (doublerealT)(amg_pp.dapply_ilu_max_pattern_size);
	 doublerealT RealZERO = (doublerealT)(amg_pp.RealZERO);
	...
	if (ilevel == 0) {
		...
		printf("ERROR: number of level=0;\n");
...
#pragma omp parallel for
	...
	for (integer ii = 1; ii <= nnz_a[0]; ii++) {
		if (flag[Amat.i[ii]] == false) {
			...
			Amat.aij[ii] = (real_mix_precision)(1.0 / ap); // умножение быстрей деления.
		}
	}
	...
	for (integer ilevel_detector = 1; ilevel_detector <= maxlevel-1; ilevel_detector++) {
		...
		if (ilevel > ilevel_detector) {
			...
			const doublerealT theta7 = (doublerealT)(theta(ilevel)); // передаётся в функцию извне.
...
#pragma omp parallel for
	...
	doublerealT dres_initial = (doublerealT)(norma(residual_fine[0], n_a[0]));
...
#pragma omp parallel for
							for (integer i23 = 0; i23 < n_a[0]; i23++) {
								x_old[i23 + 1] = (doublerealT)(x_temper[i23]);
								//x_old[i23 + 1] = x[i23 + 1];
							}
#pragma omp parallel for
							for (integer i23 = 0; i23 < n_a[0]; i23++) {
...
#pragma omp parallel for
							for (integer i23 = 0; i23 < n_a[0]; i23++) {
								...
								x_temper[i23] = (doublerealT)(x_old[i23 + 1] + alpha_relax_nonlinear_boundary_condition * (x[i23 + 1] - x_old[i23 + 1]));
								if (x_temper[i23] < -272.15) x_temper[i23] = -272.15;
								x[i23 + 1] = x_temper[i23];
							}
							...
							for (integer i23 = 0; i23 < n_a[0]; i23++) {
								rthdsd_loc123[i23] = (doublerealT)(rthdsd_no_radiosity_patch[i23]);
								if ((i23 >= iadd_qnbc_maxelm) && (qnbc[i23 - iadd_qnbc_maxelm].bactive) && (qnbc[i23 - iadd_qnbc_maxelm].bStefanBolcman_q_on) && (qnbc[i23 - iadd_qnbc_maxelm].bNewtonRichman_q_on == false)) {
									...
									rthdsd_loc123[i23] = (doublerealT)(alpha_relax142 * (-qnbc[i23 - iadd_qnbc_maxelm].emissivity * STEFAN_BOLCMAN_CONST * ((273.15 + x_temper[i23]) * (273.15 + x_temper[i23]) * (273.15 + x_temper[i23]) * (273.15 + x_temper[i23]) - (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb) * (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb) * (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb) * (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb))) +
									...
									rthdsd_loc123[i23] *= (doublerealT)(qnbc[i23 - iadd_qnbc_maxelm].dS);
								}
								if ((i23 >= iadd_qnbc_maxelm) && (qnbc[i23 - iadd_qnbc_maxelm].bactive) && (qnbc[i23 - iadd_qnbc_maxelm].bNewtonRichman_q_on) && (qnbc[i23 - iadd_qnbc_maxelm].bStefanBolcman_q_on == false)) {
									// Ньютон-Рихман.
									//doublerealT alpha_relax142 = 0.25;
									rthdsd_loc123[i23] = (doublerealT)(-qnbc[i23 - iadd_qnbc_maxelm].film_coefficient * (x_temper[i23] - qnbc[i23 - iadd_qnbc_maxelm].Tamb));
									rthdsd_loc123[i23] *= (doublerealT)(qnbc[i23 - iadd_qnbc_maxelm].dS);
								}
								if ((i23 >= iadd_qnbc_maxelm) && (qnbc[i23 - iadd_qnbc_maxelm].bactive) && (qnbc[i23 - iadd_qnbc_maxelm].bNewtonRichman_q_on) && (qnbc[i23 - iadd_qnbc_maxelm].bStefanBolcman_q_on)) {
									// Условие смешанного типа.
									//doublerealT alpha_relax142 = 0.25;
									rthdsd_loc123[i23] = (doublerealT)((-qnbc[i23 - iadd_qnbc_maxelm].emissivity * STEFAN_BOLCMAN_CONST * ((273.15 + x_temper[i23]) * (273.15 + x_temper[i23]) * (273.15 + x_temper[i23]) * (273.15 + x_temper[i23]) - (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb) * (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb) * (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb) * (273.15 + qnbc[i23 - iadd_qnbc_maxelm].Tamb))));
									rthdsd_loc123[i23] += (doublerealT)(-qnbc[i23 - iadd_qnbc_maxelm].film_coefficient * (x_temper[i23] - qnbc[i23 - iadd_qnbc_maxelm].Tamb));
									rthdsd_loc123[i23] *= (doublerealT)(qnbc[i23 - iadd_qnbc_maxelm].dS);
								}
							}
...
#pragma omp parallel for
			...
			dres = (doublerealT)(norma(residual_fine[0], n_a[0]));
			ret74 += (real_mix_precision)(fabs(dres));
			if (bprint_mesage_diagnostic) {
...
#pragma omp parallel for
				...
				doublerealT dresfinish_probably = (doublerealT)(0.1 * norma(residual_fine[0], n_a[0]));
				if ((iVar == VELOCITY_X_COMPONENT) || (iVar == VELOCITY_Y_COMPONENT) || (iVar == VELOCITY_Z_COMPONENT) ||
					...
					dresfinish_probably = (doublerealT)(1.0e-3 * norma(residual_fine[0], n_a[0]));
				}
				if (bSIMPLErun_now_for_temperature) {
					if (iVar == TEMP) {
				...
				if (bonly_solid_calculation) {
					dresfinish_probably = (doublerealT)(1.0e-5 * norma(residual_fine[0], n_a[0]));
				}
				integer i943 = 0;
...
#pragma omp parallel for
				for (integer i47 = 1; i47 <= n_a[0]; i47++) {
					x_best_search[i47] = (doublerealT)(x[i47]);
					x_copy[i47] = (doublerealT)(x[i47]); // 4 ноября 2016.
				}
				system("PAUSE");
...
#pragma omp parallel for private(i75)
		for (i75 = 0; i75 < n75; i75++) {
			x[i75 + 1] = dx75[i75];
			x_best_search[i75 + 1] = (doublerealT)(dx75[i75]);
		}
...
#pragma omp parallel for
					for (integer i_1 = 0; i_1 < n75; i_1++) {
						x[i_1 + 1] = dx[i_1];
						x_best_search[i_1 + 1] = (doublerealT)(dx[i_1]);
					}
...
void setup_phase_classic_aglomerative_amg6(Ak2& Amat,
	...
	doublerealT magic82 = (doublerealT)(my_amg_manager.magic);
	doublerealT magic83 = (doublerealT)(my_amg_manager.magic);
	...
	// Были обнаружены задачи теплопередачи с очень сложной пространственной формой для 
		...
		//doublereal AccumulqtorA_m_SIZE8 = (doublereal)(1.3*my_amg_manager.memory_size); // было значение 2.0.
	...
	// iKnumber_thread=8 - Это число потоков.
		...
		for (int i_9 = 0; i_9 < iKnumber_thread; i_9++) {
			...
			AccumulqtorA_m[i_9] = new Ak1[(integer)((1.0 / (1.0 * iKnumber_thread)) * AccumulqtorA_m_SIZE8 * nnz + 1)];
			if (bprint_mesage_diagnostic) {
	...
	while ((ilevel < maxlevel - 1) && (n_a[ilevel - 1] > MIN_COARSE_MESH_NODES) && (bcontinue_global)) {
		...
		if (ilevel > 1) {
			doublerealT procent_n = (doublerealT)((100.0*(abs(n_a[ilevel - 1] - n_a[ilevel - 2]))) / (1.0*n_a[ilevel - 2]));
...
#pragma omp parallel for
			...
			for (integer ii = 1 + iadd; ii <= ii_end1; ii++) {
				...
				doublerealT theta_threshold3 = (doublerealT)(theta(ilevel) * threshold_quick_all[Amat.i[ii]]);
				for (integer is0 = ii; (is0 <= is0_end); is0++) {
						...
						while ((ii_65 <= iend_marker_position) && (P[ii_65].j == istr_65)) {
							if ((P[ii_65].aij > 0) && (fabs(P[ii_65].aij) > alpha_truncation*dmax_plus)) {
								if (bstrong_trunc) {
									if (icount_strong_trunc < IMAX_STRONG_TRUNC) {
										...
										P[icounter_truncation].aij = (real_mix_precision)(fabs(dsum_plus / dsum_plus_new)*P[ii_65].aij);
										icounter_truncation++;
									}
									icount_strong_trunc++;
								}
							...
							if ((P[ii_65].aij < 0) && (fabs(P[ii_65].aij) > alpha_truncation*dmax_minus)) {
								if (bstrong_trunc) {
									if (icount_strong_trunc < IMAX_STRONG_TRUNC) {
										P[icounter_truncation] = P[ii_65];
										P[icounter_truncation].aij = (real_mix_precision)(fabs(dsum_minus / dsum_minus_new)*P[ii_65].aij);
										icounter_truncation++;
									}
									icount_strong_trunc++;
								}
...
#pragma omp parallel for 
	...
	if (bprint_mesage_diagnostic) {
		...
		doublerealT sizegb = (doublerealT)(16 * iadd / 1.0e9);
	...
	for (integer i_78 = 0; i_78 < n; i_78++) {
		...
		for (integer i_79 = row_ptr[i_78]; i_79 < row_ptr[i_78 + 1]; i_79++) {
			if (col_ind[i_79] == i_78) {
				...
				Amat.aij[diagonal] = (real_mix_precision)(val[i_79]);
				if (val[i_79] <= 0.0) {
					std::cout << "diag aij=" << val[i_79] << " i=" << i_79 << std::endl;
				}
				Amat.abs_aij[diagonal] = (real_mix_precision)(fabs(val[i_79]));
				Amat.j[diagonal] = (integer_mix_precision)(col_ind[i_79] + 1); //  индексация столбца начинается с единицы
			}
		}
		...
		for (integer i_79 = row_ptr[i_78]; i_79 < row_ptr[i_78 + 1]; i_79++) {
			if (col_ind[i_79] != i_78) {
				Amat.aij[post] = (real_mix_precision)(val[i_79]);
				if (val[i_79] >= 0.0) {
					std::cout << "aij=" << val[i_79] << " i=" << i_79 << std::endl;
				}
				Amat.abs_aij[post] = (real_mix_precision)(fabs(val[i_79]));
				Amat.j[post] = (integer_mix_precision)(col_ind[i_79] + 1); //  индексация столбца начинается с единицы
				post++;
			}
		}

		for (integer i_79 = row_ptr[i_78]; i_79 < row_ptr[i_78 + 1]; i_79++) {
			Amat.i[i_79 + 1] = (integer_mix_precision)(i_78 + 1); // номер строки который начинается с единиуцы.
		}
	}
