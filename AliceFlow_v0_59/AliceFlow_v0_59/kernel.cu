...
#pragma omp parallel for
	...
	*/
	...
	if (number_cores() == 2) {
		...
		for (int i = 0; i < my_global_temperature_struct.maxelm; i++) if (inumerate[i] == -1) { std::cout << "nested desection error" << i << "\n"; getchar(); }
		//getchar();
    }
...
#pragma omp parallel for
		...
		if (1 || bcleantemp) {
			...
			solve_Thermal(my_global_temperature_struct, f, matlist, w, lw, lu, b, lb, ls, m, false,
				...
				maxelm_global_ret, 1.0, 1.0, bAVLrealesation, t_for_Mechanical, inumerate);
