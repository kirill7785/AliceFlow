			...
			if (istartAnew_m[tid] + ism < LIMIT_ANEW_m) {
				for (integer i_6 = 1; i_6 <= ism; ++i_6) {
					...
					if (fabs(vs1) > 1.0e-37) {
						...
						Ak1 Atemp;
						Atemp.aij = (real_mix_precision)(vs1);
...
#pragma omp parallel for schedule (static) 
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		if (my_amg_manager.bdiagonal_dominant > 0) {
			...
			vector_sum_m[tid][istr] += (doublerealT)(additional_to_diagonal);
		}
						...
						if (fabs(vs1) > 1.0e-37) {
							...
							if (istr == jstr) {
								...
								//}
									...
									Atemp.aij = (real_mix_precision)(sum_dia);
	...
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		if (nsizeA > istartAnew + index_size) {
			for (integer i_6 = 1; i_6 <= index_size; ++i_6) {
				...
				if (fabs(vs1) > 1.0e-37) {
					// Мы не записываем в матрицу чистый ноль.
					Amat.aij[istartAnew] = (real_mix_precision)(vs1);
					Amat.i[istartAnew] = (integer_mix_precision)(istr);
					Amat.j[istartAnew] = (integer_mix_precision)(jstr);
					istartAnew++;
				}
	...
	BAD_STRING_PATCHING*& bsp)
{
	...
	for (integer istr = 1; istr <= numberofcoarcenodes; ++istr) {
		...
		if (my_amg_manager.bdiagonal_dominant > 0) {
			...
			vector_sum[jstr_diag] += (doublerealT)(additional_to_diagonal);
		}
