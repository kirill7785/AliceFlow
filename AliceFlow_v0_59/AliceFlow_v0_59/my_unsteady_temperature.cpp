	...
	if (EndTime > 0.0) {
		...
		while (time < EndTime) {
				...
				if (!bost) {
					...
					integer kmod2 = (i-1) % (glTSL.n_cycle * inumber_step_size + (integer)(div_t_ostatok_sutok));
					...
					if ((i != 1) && ((i - 1) % (glTSL.n_cycle * inumber_step_size + (integer)(div_t_ostatok_sutok)) == 0)) {
                		ig++;
						bost = false;
						j = 1;
						if (glTSL.tau1 > 1.0e-30) {
			...
			for (integer j = 0; j < iN; j++) {
				...
				if (bTemperature) {
					...
					if (bsecond_T_solver) {
							...
							poweron_multiplier_sequence_loc, poweron_multiplier_sequence_loc0, bAVLrealesation, t_for_Mechanical, inumerate);
					}
					else {
						integer dist_max = 3;
...
//#endif
				...
				*/
				...
				{
					report_temperature_for_unsteady_modeling(0, fglobal, t, b, lb, s, ls, w, lw, 0, phisicaltime, poweron_multiplier_sequence0[j], t.operatingtemperature);
				}
					  ...
					  if (eqin.fluidinfo[iflow].iflow == 1) {
						  ...
						  for (int i = inumber_iteration_SIMPLE[iflow] + 1; i < iend; i++) {
							  ...
							  if (i == (inumber_iteration_SIMPLE[iflow] + 1)) {
								  ...
								  if (!b_on_adaptive_local_refinement_mesh) {
									  fglobal[iflow].alpha[VELOCITY_X_COMPONENT] = 0.7; // 0.8 0.5 0.7
									  fglobal[iflow].alpha[VELOCITY_Y_COMPONENT] = 0.7; // 0.8 0.5 0.7
									  fglobal[iflow].alpha[VELOCITY_Z_COMPONENT] = 0.7; // 0.8 0.5 0.7
									  fglobal[iflow].alpha[PRESS] = 0.3;// 0.3; // 0.2 0.8
									  fglobal[iflow].alpha[NUSHA_SL] = 0.8;// 1.0;// 0.7;
									  // Без нижней релаксации сходимости нет.
									  fglobal[iflow].alpha[TURBULENT_KINETIK_ENERGY_SL] = 0.1;// сходится и при 0.1 время расчёта сокращается вдвое по сравнению с 0.8.
