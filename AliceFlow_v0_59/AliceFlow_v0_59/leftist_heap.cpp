...
void LeftistHeapSort(Ak2& Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		...
		Amat.i[i] = (integer_mix_precision)(get_min(heap));
		Amat.j[i] = j_a[i_];
		Amat.aij[i] = a[i_];
		i++;
...
void LeftistHeapSort_j(Ak2& Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		integer i_ = get_min_link(heap);
		Amat.j[i] = (integer_mix_precision)(get_min(heap));
		Amat.i[i] = i_a[i_];
		Amat.aij[i] = a[i_];
		i++;
		heap = delete_min(heap);
...
void LeftistHeapSort(Ak1* &Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		integer i_ = get_min_link(heap);
		Amat[i].i = (integer_mix_precision)(get_min(heap));
		Amat[i].j = (integer_mix_precision)(j_a[i_]);
		Amat[i].aij = a[i_];
		i++;
		heap = delete_min(heap);
	}
...
void LeftistHeapSort_j(Ak1* &Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		integer i_ = get_min_link(heap);
		Amat[i].j = (integer_mix_precision)(get_min(heap));
		Amat[i].i = (integer_mix_precision)(i_a[i_]);
		Amat[i].aij = a[i_];
		i++;
		heap = delete_min(heap);
	}
