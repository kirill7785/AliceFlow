...
L20:
	...
	for (integer ii_1 = 1; ii_1 <= iend2; ii_1++) a_i[ii_1] = (integer)(a[jtrst + ii_1 - 1]);
...
/*       NDIR: AVAILABLE WORK SPACE */
	...
	for (integer ii_1 = 1; ii_1 <= iend2; ii_1++) a[jtrst+ii_1-1] = (doublereal)(a_i[ii_1]);
	delete[] a_i;
	...
	for (integer ii_1 = 1; ii_1 <= imax[i__2memo] - imin[i__2memo] + 1; ii_1++) u[irst + ii_1-1] = (doublereal)(u_i[ii_1]);
	delete[] u_i;
...
/* ===> CREATE F-POINTS AROUND ABOVE C-TOCHKA */
	...
	i__ = jtr[j];
    //i__ = (integer)(jtr[j]); // 24.03.2019 PVS-Studio
	if (icg[i__] != -2) {
	    goto L77;
	}
	icg[i__] = -1;
...
/*         COMPONENT TO ZERO */
	...
	for (integer ii_1 = 1; ii_1 <= nsp; ii_1++) a_i[ii_1] = (integer)(a[jhi + ii_1]);
...
/* ===>   FACTORIZATION ALLREADY DONE */
	...
	integer* a_i = new integer[nsp + 1];
	//for (integer ii_1 = 0; ii_1 <= nsp; ii_1++) a_i[ii_1] = (integer)(a[jhi + 1 + ii_1]);
	for (integer ii_1 = 1; ii_1 <= nsp; ii_1++) a_i[ii_1] = (integer)(a[jhi + ii_1]);
