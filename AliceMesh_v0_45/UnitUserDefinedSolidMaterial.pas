...
type
  TFormUserDefinedSolidMat = class(TForm)
    ...
    Label15: TLabel;
    EditEz: TEdit;
    CheckBox_lambda_is_file: TCheckBox;
    ...
    procedure CheckBox_lambda_is_fileClick(Sender: TObject);
...
begin
    ...
    begin
       ...
       begin
          ...
          if (CheckBox_lambda_is_file.Checked) then
          begin
              // Теплопроводность тектолитовой платы считанная из специального текстового файла.
              Laplas.workmat[imatid].lambda_is_file:=1;
          end
            else
          begin
              Laplas.workmat[imatid].lambda_is_file:=0;
          end;
...
// Если тепловые свойства кубика определяются платой то мы скрываем пользовательский
// ввод.
procedure TFormUserDefinedSolidMat.CheckBox_lambda_is_fileClick(
  Sender: TObject);
begin
   if (CheckBox_lambda_is_file.Checked) then
   begin
     GBuserproperties.Visible:=false;
   end
   else
   begin
     GBuserproperties.Visible:=true;
   end;
end;
