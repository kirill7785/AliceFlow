...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            stl:=Trim(copy(stl,2,length(stl)-2)); // убираем скобки
      ...
      begin
          stri:=Trim(copy(stri,2,length(stri)-2)); // убираем скобки
    ...
    begin
       ...
       if (FormatSettings.DecimalSeparator='.') then
         begin
            // Разделитель целой и дробной части точка.
            // Меняем все запятые на точки.
            val(StringReplace(Trim(st),',','.',[rfReplaceAll]),c,code);  // пытаемся получить число
         end
         else
         begin
            // Разделитель целой и дробной части запятая.
          ...
          begin
             ...
             begin
                ...
                begin
                   // Разделитель целой и дробной части точка.
                   // Меняем все запятые на точки.
                   val(StringReplace(AddVariableForm.edtvalue.Text,',','.',[rfReplaceAll]),c,code);  // пытаемся получить число
                end
                 else
                begin
                   // Разделитель целой и дробной части запятая.
...
begin
    // Считаем число открывающих и зарывающих скобок.
    ileft:=0;
    iright:=0;
    ...
    begin
       // Число открывающих скобок равняется числу закрывающих скобок.
    ...
    begin
       // Количество открывающих скобок не равно числу закрывающих скобок.
       bOk:=false;
       my_real_convert:=-10000.0; // Явно ошибочное значение.
    end;
end;
