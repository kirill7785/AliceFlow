...
type
  TEGDForm = class(TForm)
    ...
    procedure ComboBoxFlowRegimeClick(Sender: TObject);
...
begin
   ...
   begin
     ...
     // if (Laplas.egddata.myflmod[CBIdCurFLzone.ItemIndex].iflow=1) then
      ...
      // режим течения
      //ComboBoxFlowRegime.ItemIndex:=Laplas.egddata.myflmod[CBIdCurFLzone.ItemIndex].iflowregime;
      // модель турбулентности
      //ComboBoxturbulentmodel.ItemIndex:=Laplas.egddata.myflmod[CBIdCurFLzone.ItemIndex].iturbmodel;
      // режим течения
      ComboBoxFlowRegime.ItemIndex:=Laplas.egddata.myflmod[0].iflowregime;
      // модель турбулентности
...
begin
   ...
   begin
      ...
      ComboBoxFlowRegime.Visible:=true;
      GBGravity.Visible:=true;
      // ламинарный или турбулентный.
      if ((ComboBoxFlowRegime.ItemIndex=0) or
          (ComboBoxFlowRegime.ItemIndex=2)) then
      begin
         // LAMINAR or Inviscid
   ...
   begin
      ...
      ComboBoxFlowRegime.Visible:=false;
      GroupBoxTurbulentModel.Visible:=false;
...
begin
  ...
  sforval:=StringReplace(Egx.Text,',','.',[rfReplaceAll]);
   ...
   if (ComboBoxFlowRegime.ItemIndex=0) then
   begin
   ...
   *}
   ...
   (ComboBoxFlowRegime.ItemIndex=2)) then
   begin
      Laplas.egddata.myflmod[0].iflowregime:=ComboBoxFlowRegime.ItemIndex;
   end
    else
   begin
...
procedure TEGDForm.ComboBoxFlowRegimeClick(Sender: TObject);
begin
   // смена режима течения:
   // ламинарный или турбулентный.
   if ((ComboBoxFlowRegime.ItemIndex=0)or(ComboBoxFlowRegime.ItemIndex=2)) then
   begin
      // LAMINAR or Inviscid
      GroupBoxTurbulentModel.Visible:=false;
   end
