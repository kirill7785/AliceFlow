...
type
  TForm_amg_manager = class(TForm)
    ...
    ButtonRumbaApply: TButton;
    CheckBoxCFReorderTemperature: TCheckBox;
    CheckBoxCFReorderSpeed: TCheckBox;
    CheckBoxCFReorderPressure: TCheckBox;
    CheckBoxCFReorderStress: TCheckBox;
    Edit_truncation_Stress: TEdit;
    CheckBoxbautoThresholdTemperature: TCheckBox;
    CheckBoxbautoThresholdSpeed: TCheckBox;
    CheckBoxbthresholdPressureAuto: TCheckBox;
    CheckBoxbthresholdautoStress: TCheckBox;
    ...
    procedure ButtonRumbaApplyClick(Sender: TObject);
    procedure CheckBoxbautoThresholdTemperatureClick(Sender: TObject);
    procedure CheckBoxbautoThresholdSpeedClick(Sender: TObject);
    procedure CheckBoxbthresholdPressureAutoClick(Sender: TObject);
    procedure CheckBoxbthresholdautoStressClick(Sender: TObject);
...
begin
    ...
    ComboBoxnumberpresmothers.ItemIndex:=1; // nu = 1
    ...
    ComboBoxnumberpostsweeps.ItemIndex:=2; //nu = 2
    ...
    ComboBoxmemorysize.ItemIndex:=7;  // memory number A size
    ComboBoxmemorysizeSpeed.ItemIndex:=7;
    ComboBoxmemorysizePressure.ItemIndex:=7;
    ComboBoxmemorysizeStress.ItemIndex:=7;
   ...
   // none or linear version none (0).
   // Stabilization BiCGStab + amg (РУМБА) (1).
   // Stabilization FGMRes + amg (РУМБА) (2).
   // nonlinear version none (3).
   // 8.01.2017
   ...
   CheckBoxCFReorderTemperature.Checked:=false;
   CheckBoxCFReorderSpeed.Checked:=false;
   CheckBoxCFReorderPressure.Checked:=false;
   CheckBoxCFReorderStress.Checked:=false;

   CheckBoxbautoThresholdTemperature.Checked:=false;
   CheckBoxbautoThresholdSpeed.Checked:=false;
   CheckBoxbthresholdPressureAuto.Checked:=false;
   CheckBoxbthresholdautoStress.Checked:=false;

end;

// Применение настроек пользователя.
procedure TForm_amg_manager.ButtonRumbaApplyClick(Sender: TObject);
begin
    Close;
end;


procedure TForm_amg_manager.CheckBoxbautoThresholdSpeedClick(Sender: TObject);
begin
   if (CheckBoxbautoThresholdSpeed.Checked) then
   begin
      EditthresholdSpeed.Visible:=false; // creator depend
   end
    else
   begin
      EditthresholdSpeed.Visible:=true; // user depend
   end;
end;

procedure TForm_amg_manager.CheckBoxbautoThresholdTemperatureClick(
  Sender: TObject);
begin
   if (CheckBoxbautoThresholdTemperature.Checked) then
   begin
      Editthreshold.Visible:=false; // creator depend
   end
    else
   begin
      Editthreshold.Visible:=true; // user depend
   end;
end;

procedure TForm_amg_manager.CheckBoxbthresholdautoStressClick(Sender: TObject);
begin
   if (CheckBoxbthresholdautoStress.Checked) then
   begin
      EditthresholdStress.Visible:=false; // creator depend
   end
   else
   begin
      EditthresholdStress.Visible:=true; // user depend
   end;
end;

procedure TForm_amg_manager.CheckBoxbthresholdPressureAutoClick(
  Sender: TObject);
begin
   if (CheckBoxbthresholdPressureAuto.Checked) then
   begin
       EditthresholdPressure.Visible:=false; // creator depend
   end
   else
   begin
      EditthresholdPressure.Visible:=true; // user depend
   end;
end;
...
begin
    if ((ComboBoxcoarseningPressure.ItemIndex=8)or(ComboBoxcoarseningPressure.ItemIndex=9)or
    (ComboBoxcoarseningPressure.ItemIndex=10)or(ComboBoxcoarseningPressure.ItemIndex=11)) then
   begin
      // PMIS, HMIS
      // PMIS2, PMIS2_full
...
begin
    if ((ComboBoxcoarseningSpeed.ItemIndex=8) or (ComboBoxcoarseningSpeed.ItemIndex=9) or
   (ComboBoxcoarseningSpeed.ItemIndex=10) or (ComboBoxcoarseningSpeed.ItemIndex=11)) then
   begin
      // PMIS, HMIS
      // PMIS2, PMIS2_full
...
begin
   if ((ComboBoxcoarseningStress.ItemIndex=8) or (ComboBoxcoarseningStress.ItemIndex=9) or
       (ComboBoxcoarseningStress.ItemIndex=10) or (ComboBoxcoarseningStress.ItemIndex=11)) then
   begin
      // PMIS, HMIS
      // PMIS2, PMIS2_full
...
begin
   // Temperature
   if ((ComboBoxcoarseningTemp.ItemIndex=8) or (ComboBoxcoarseningTemp.ItemIndex=9) or
       (ComboBoxcoarseningTemp.ItemIndex=10) or (ComboBoxcoarseningTemp.ItemIndex=11)) then
   begin
      // PMIS, HMIS
      // PMIS2, PMIS2_full
      ComboBoxmemorysize.ItemIndex:=2;// size ierarhion 4A
