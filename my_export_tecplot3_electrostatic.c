...
void exporttecplotxy360T_3D_part1and3(integer maxelm, integer maxbound, bool bextendedprint, integer ncell, 
									  ...
									  BOUND* sosedb, integer ivarexport)
{
	// расширенная печать
		...
		*/
		...
		if (bextendedprint) {
			for (i=0; i<maxbound; i++) {
				...
				case E : fprintf(fp, "%+.16f ", pa[nvtx[0][sosedb[i].iI]-1].x*m2cm);
					     break;
				case W : fprintf(fp, "%+.16f ", pa[nvtx[1][sosedb[i].iI]-1].x*m2cm);
					     break;
				case N : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].x+pa[nvtx[1][sosedb[i].iI]-1].x)*m2cm);
					     break;
				case S : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].x+pa[nvtx[1][sosedb[i].iI]-1].x)*m2cm);
					     break;
				case T : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].x+pa[nvtx[1][sosedb[i].iI]-1].x)*m2cm);
					     break;
				case B : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].x+pa[nvtx[1][sosedb[i].iI]-1].x)*m2cm);
					     break;
				}
			    if (i%10==0) fprintf(fp, "\n");
			}
		...
		if (bextendedprint) {
			for (i=0; i<maxbound; i++) {
				switch (sosedb[i].Norm) {// определим внутреннюю нормаль к границе
				case E : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].y+pa[nvtx[2][sosedb[i].iI]-1].y)*m2cm);
					     break;
				case W : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].y+pa[nvtx[2][sosedb[i].iI]-1].y)*m2cm);
					     break;
				case N : fprintf(fp, "%+.16f ", pa[nvtx[0][sosedb[i].iI]-1].y*m2cm);
					     break;
				case S : fprintf(fp, "%+.16f ", pa[nvtx[2][sosedb[i].iI]-1].y*m2cm);
					     break;
				case T : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].y+pa[nvtx[2][sosedb[i].iI]-1].y)*m2cm);
					     break;
				case B : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].y+pa[nvtx[2][sosedb[i].iI]-1].y)*m2cm);
					     break;
				}
			    if (i%10==0) fprintf(fp, "\n");
			}
		...
		if (bextendedprint) {
			for (i=0; i<maxbound; i++) {
				switch (sosedb[i].Norm) {// определим внутреннюю нормаль к границе
				case E : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].z+pa[nvtx[4][sosedb[i].iI]-1].z)*m2cm);
					     break;
				case W : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].z+pa[nvtx[4][sosedb[i].iI]-1].z)*m2cm);
					     break;
				case N : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].z+pa[nvtx[4][sosedb[i].iI]-1].z)*m2cm);
					     break;
				case S : fprintf(fp, "%+.16f ", 0.5*(pa[nvtx[0][sosedb[i].iI]-1].z+pa[nvtx[4][sosedb[i].iI]-1].z)*m2cm);
					     break;
				case T : fprintf(fp, "%+.16f ", pa[nvtx[0][sosedb[i].iI]-1].z*m2cm);
					     break;
				case B : fprintf(fp, "%+.16f ", pa[nvtx[4][sosedb[i].iI]-1].z*m2cm);
					     break;
				}
			    if (i%10==0) fprintf(fp, "\n");
			}
...
void xyplot_temp(TEMPER &t, doublereal* tempfiltr) {
	...
	if ((err = fopen_s( &fp, "xyplotT.txt",  "w")) != 0) {
		...
		// перемотка в начало 
		switch (iplane) {
		  case XY : while (t.sosedi[B][iPf].iNODE1<t.maxelm) iPf=t.sosedi[B][iPf].iNODE1; break;
		  case XZ : while (t.sosedi[S][iPf].iNODE1<t.maxelm) iPf=t.sosedi[S][iPf].iNODE1; break;
		  case YZ : while (t.sosedi[W][iPf].iNODE1<t.maxelm) iPf=t.sosedi[W][iPf].iNODE1; break;
		}
		...
		switch (iplane) {
		  case XY : fprintf(fp, "%+.16f %+.16f %+.16f\n",
			  p.z - 0.5*dz, t.potent[t.sosedi[B][iPf].iNODE1],
			  tempfiltr[t.sosedi[B][iPf].iNODE1]);
			              break;
		  case XZ :  fprintf(fp,"%+.16f %+.16f %+.16f\n",
			  p.y - 0.5*dy, t.potent[t.sosedi[S][iPf].iNODE1],
			  tempfiltr[t.sosedi[S][iPf].iNODE1]);
			              break;
		  case YZ :  fprintf(fp, "%+.16f %+.16f %+.16f\n",
			  p.x - 0.5*dx, t.potent[t.sosedi[W][iPf].iNODE1],
			  tempfiltr[t.sosedi[W][iPf].iNODE1]);
			              break;
		}
		switch (iplane) {
		  case XY : while (iPf<t.maxelm) {
			        ...
			        fprintf(fp, "%+.16f %+.16f %+.16f\n", 
						  p.z, t.potent[iPf],
					      tempfiltr[iPf]);
					if (t.sosedi[T][iPf].iNODE1 >= t.maxelm) {
						  volume3D(iPf, t.nvtx, t.pa, dx, dy, dz);
                          fprintf(fp, "%+.16f %+.16f %+.16f\n",
							  p.z + 0.5*dz, t.potent[t.sosedi[T][iPf].iNODE1],
							  tempfiltr[t.sosedi[T][iPf].iNODE1]);
					}
					iPf = t.sosedi[T][iPf].iNODE1;
					} break;
		  case XZ : while (iPf<t.maxelm) {
			        ...
			        fprintf(fp, "%+.16f %+.16f %+.16f\n", 
						  p.y, t.potent[iPf],
					      tempfiltr[iPf]);
					if (t.sosedi[N][iPf].iNODE1 >= t.maxelm) {
						  volume3D(iPf, t.nvtx, t.pa, dx, dy, dz);
                          fprintf(fp, "%+.16f %+.16f %+.16f\n",
							  p.y + 0.5*dy, t.potent[t.sosedi[N][iPf].iNODE1],
							  tempfiltr[t.sosedi[N][iPf].iNODE1]);
					}
					/*
					// Узнаём последовательность узлов для отладки.
					printf("iPf=%d\n",iPf);
					if (fglobal[ifi].sosedi[N][iPf].iNODE1>=fglobal[ifi].maxelm) {
						printf("iPffinish=%d\n",fglobal[ifi].sosedi[N][iPf].iNODE1);
						getchar();
					}
					*/
					iPf = t.sosedi[N][iPf].iNODE1;
					} break;
		  case YZ : while (iPf<t.maxelm) {
			        ...
			        fprintf(fp, "%+.16f %+.16f %+.16f\n", 
						  p.y, t.potent[iPf],
					      tempfiltr[iPf]);
					if (t.sosedi[E][iPf].iNODE1 >= t.maxelm) {
						  volume3D(iPf, t.nvtx, t.pa, dx, dy, dz);
                          fprintf(fp, "%+.16f %+.16f %+.16f\n",
							  p.x + 0.5*dx, t.potent[t.sosedi[E][iPf].iNODE1],
							  tempfiltr[t.sosedi[E][iPf].iNODE1]);
					}
					iPf = t.sosedi[E][iPf].iNODE1;
					} break;
		}
		fclose(fp);
	}
...
void xyplot( FLOW* &fglobal, integer flow_interior, TEMPER &t) {
	...
	if ((err = fopen_s( &fp, "xyplot.txt",  "w")) != 0) {
		...
		// перемотка в начало 
		switch (iplane) {
		case XY: while (fglobal[ifi].sosedi[B][iPf].iNODE1<fglobal[ifi].maxelm) iPf = fglobal[ifi].sosedi[B][iPf].iNODE1; break;
		case XZ: while (fglobal[ifi].sosedi[S][iPf].iNODE1<fglobal[ifi].maxelm) iPf = fglobal[ifi].sosedi[S][iPf].iNODE1; break;
		case YZ: while (fglobal[ifi].sosedi[W][iPf].iNODE1<fglobal[ifi].maxelm) iPf = fglobal[ifi].sosedi[W][iPf].iNODE1; break;
		}
		...
		switch (iplane) {
		  case XY : fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n",
			  p.z - 0.5*dz, fglobal[ifi].potent[VX][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[VY][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[VZ][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[PAM][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[PRESS][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[FBUF][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[GRADZPRESS][fglobal[ifi].sosedi[B][iPf].iNODE1],
			  fglobal[ifi].potent[GRADZPAM][fglobal[ifi].sosedi[B][iPf].iNODE1],
						  fglobal[ifi].mf[iPf][G]);
			              break;
		  case XZ :  fprintf(fp,"%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n",
			  p.y - 0.5*dy, fglobal[ifi].potent[VX][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[VY][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[VZ][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[PAM][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[PRESS][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[FBUF][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[GRADYPRESS][fglobal[ifi].sosedi[S][iPf].iNODE1],
			  fglobal[ifi].potent[GRADYPAM][fglobal[ifi].sosedi[S][iPf].iNODE1],
						  fglobal[ifi].mf[iPf][G]);
			              break;
		  case YZ :  fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n",
			  p.x - 0.5*dx, fglobal[ifi].potent[VX][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[VY][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[VZ][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[PAM][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[PRESS][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[FBUF][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[GRADXPRESS][fglobal[ifi].sosedi[W][iPf].iNODE1],
			  fglobal[ifi].potent[GRADXPAM][fglobal[ifi].sosedi[W][iPf].iNODE1],
						  fglobal[ifi].mf[iPf][G]);
			              break;
		}
		switch (iplane) {
		  case XY : while (iPf<fglobal[ifi].maxelm) {
			        ...
			        fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n", 
						  ...
						  fglobal[ifi].mf[iPf][G]);
					if (fglobal[ifi].sosedi[T][iPf].iNODE1 >= fglobal[ifi].maxelm) {
						  volume3D(iPf, fglobal[ifi].nvtx, fglobal[ifi].pa, dx, dy, dz);
                          fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n",
							  p.z + 0.5*dz, fglobal[ifi].potent[VX][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[VY][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[VZ][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[PAM][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[PRESS][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[FBUF][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[GRADZPRESS][fglobal[ifi].sosedi[T][iPf].iNODE1],
							  fglobal[ifi].potent[GRADZPAM][fglobal[ifi].sosedi[T][iPf].iNODE1],
						  fglobal[ifi].mf[iPf][G]);
					}
					iPf = fglobal[ifi].sosedi[T][iPf].iNODE1;
					} break;
		  case XZ : while (iPf<fglobal[ifi].maxelm) {
					...
					if (fglobal[ifi].sosedi[N][iPf].iNODE1 >= fglobal[ifi].maxelm) {
						  volume3D(iPf, fglobal[ifi].nvtx, fglobal[ifi].pa, dx, dy, dz);
                          fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n",
							  p.y + 0.5*dy, fglobal[ifi].potent[VX][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[VY][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[VZ][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[PAM][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[PRESS][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[FBUF][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[GRADYPRESS][fglobal[ifi].sosedi[N][iPf].iNODE1],
							  fglobal[ifi].potent[GRADYPAM][fglobal[ifi].sosedi[N][iPf].iNODE1],
						  fglobal[ifi].mf[iPf][G]);
					}
					/*
					// Узнаём последовательность узлов для отладки.
					...
					*/
					iPf = fglobal[ifi].sosedi[N][iPf].iNODE1;
					} break;
		  case YZ : while (iPf<fglobal[ifi].maxelm) {
			        ...
			        fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n", 
						  ...
						  fglobal[ifi].mf[iPf][G]);
					if (fglobal[ifi].sosedi[E][iPf].iNODE1 >= fglobal[ifi].maxelm) {
						  volume3D(iPf, fglobal[ifi].nvtx, fglobal[ifi].pa, dx, dy, dz);
                          fprintf(fp, "%+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f %+.16f\n",
							  p.x + 0.5*dx, fglobal[ifi].potent[VX][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[VY][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[VZ][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[PAM][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[PRESS][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[FBUF][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[GRADXPRESS][fglobal[ifi].sosedi[E][iPf].iNODE1],
							  fglobal[ifi].potent[GRADXPAM][fglobal[ifi].sosedi[E][iPf].iNODE1],
						  fglobal[ifi].mf[iPf][G]);
					}
					iPf = fglobal[ifi].sosedi[E][iPf].iNODE1;
					} break;
		}
		fclose(fp);
	}
