...
integer _tmain(integer argc, _TCHAR* argv[])
{
	...
	if (dim_2D) {
		...
		A = new Ak[static_cast<integer>(6 * nnz) + 1]; // 6
		R = new Ak[static_cast<integer>(3 * n) + 1]; // 3*nnz 2.4
		P = new Ak[static_cast<integer>(3 * n) + 1]; // 3*nnz 2.4
		integer ic = 1;
		for (integer i = 1; i <= m; ++i) {
			for (integer j = 1; j <= m; ++j) {
	...
	doublereal alpharelax, integer iVar, bool bLRfree, QuickMemVorst& m)
{
	...
	if (res_sum>1.05*finish_residual) // защита от повторного холостого запуска экономит время конечного пользователя.
	{
		...
		*/
		...
		A = new Ak[static_cast<integer>(6 * nna) + 1]; // 6
		if (A == NULL) {
		...
		R = new Ak[static_cast<integer>(3 * nnu) + 1]; // 3*nnz 2.4
		if (R == NULL) {
			// недостаточно памяти на данном оборудовании.
		...
		P = new Ak[static_cast<integer>(3 * nnu) + 1]; // 3*nnz 2.4
		if (P == NULL) {
			// недостаточно памяти на данном оборудовании.
