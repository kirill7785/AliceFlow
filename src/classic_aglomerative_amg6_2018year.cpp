...
bool classic_aglomerative_amg6(Ak2 &Amat,
	...
	BAD_STRING_PATCHING  bsp[i_bsp_LIMIT];
	...
	const integer nullptr_SOSED = -1;
	integer vacant = nullptr_SOSED;
	bool bcontinue = true;
...
#pragma omp parallel for 
		...
		if (row_ind_SA != nullptr) {
			free(row_ind_SA);
			row_ind_SA = nullptr;
		}
		if (row_ind_EA != nullptr) {
			free(row_ind_EA);
			row_ind_EA = nullptr;
		}
