...
void my_elmatr_quad_T3D_bound(integer inumber, integer maxbound, integer maxelm,
	...
	*/
	...
	if (((sosedb[inumber].MCB < (ls + lw)) &&
		(sosedb[inumber].MCB >= ls) &&
		(w[sosedb[inumber].MCB - ls].ifamily == 1))
		&& (w[sosedb[inumber].MCB - ls].bopening)) {
	...
	*/
	...
	// inumber - номер граничного КО.
	// inumber изменяется от 0..maxbound-1
	if (bDirichlet && (((sosedb[inumber].MCB < (ls + lw)) &&
		(sosedb[inumber].MCB >= ls) && 
		(w[sosedb[inumber].MCB - ls].ifamily == 1)) ||
		(bBlockStefanBolcman&&(
		(((sosedb[inumber].MCB < (ls + lw))
			&& (sosedb[inumber].MCB >= ls) && 
			(w[sosedb[inumber].MCB - ls].ifamily == 4))))))) {

		

		if ((bBlockStefanBolcman && (
			(((sosedb[inumber].MCB < (ls + lw)) && (sosedb[inumber].MCB >= ls) 
				&& (w[sosedb[inumber].MCB - ls].ifamily == 4)))))) {
			breakRUMBAcalc_for_nonlinear_boundary_condition = true;
			// Граничное условие Дирихле:
	...
	else if (!bDirichlet && ((sosedb[inumber].MCB == (ls + lw)) 
	|| (sosedb[inumber].MCB < ls) || bopening_Neiman_on ||
	((sosedb[inumber].MCB < (ls + lw)) && (sosedb[inumber].MCB >= ls) && 
	(w[sosedb[inumber].MCB - ls].ifamily != 1)))) {
			...
			if (((sosedb[inumber].MCB < (ls + lw)) && 
				...
				if (bBlockStefanBolcman) {
					...
					*/
					...
					if ((potent[sosedb[inumber].iB] < -271.0) && (potent[sosedb[inumber].iB] <= w[sosedb[inumber].MCB - ls].Tamb))
					{
						// 07.12.2019
						...
						bsc1 = true;
						//alpha_relax1 = 1.0;
						qb = alpha_relax1 *(-w[sosedb[inumber].MCB - ls].emissivity*5.670367e-8*
							((273.15 + potent[sosedb[inumber].iB])*
							(273.15 + potent[sosedb[inumber].iB])*
							(273.15 + potent[sosedb[inumber].iB])*
							(273.15 + potent[sosedb[inumber].iB]) - 
							(273.15 + w[sosedb[inumber].MCB - ls].Tamb)*
							(273.15 + w[sosedb[inumber].MCB - ls].Tamb)*
							(273.15 + w[sosedb[inumber].MCB - ls].Tamb)*
							(273.15 + w[sosedb[inumber].MCB - ls].Tamb))) +
							(1.0 - alpha_relax1)*(-w[sosedb[inumber].MCB - ls].emissivity*5.670367e-8*
							((273.15 + potent_old[sosedb[inumber].iB])*
							(273.15 + potent_old[sosedb[inumber].iB])*
							(273.15 + potent_old[sosedb[inumber].iB])*
							(273.15 + potent_old[sosedb[inumber].iB]) -
							(273.15 + w[sosedb[inumber].MCB - ls].Tamb)*
		...
		if (bcontinue) {
			...
			if (!bopening_Neiman_on) {
				...
				if (((sosedb[inumber].MCB < (ls + lw)) && 
					(sosedb[inumber].MCB >= ls) &&
					(w[sosedb[inumber].MCB - ls].ifamily == 3))) {
					breakRUMBAcalc_for_nonlinear_boundary_condition = true;
				...
				if ((sosedb[inumber].MCB == (ls + lw)) && (adiabatic_vs_heat_transfer_coeff == 2)) {
					...
					if (blocker_Newton_Richman) {
						...
						// Конкретно разница рпавна 20градусов что даст корректную постановку задачи и правильное решении при 
						// условии что процесс нахождения поля температур носит нелинейный характер.
						if ((potent[sosedb[inumber].iB] < -271.0) && (potent[sosedb[inumber].iB] <= operating_temperature_for_film_coeff))
						{
							// 07.12.2019
						    // Защита от опускания температуры ниже абсолютного нуля.
							qb = 0.0;
							...
							qnbc[inumber].Tamb = operating_temperature_for_film_coeff;
							bsc1 = true;
							qb = alpha_relax1 *(-sosedb[inumber].emissivity*5.670367e-8*
								((273.15 + potent[sosedb[inumber].iB])*
								(273.15 + potent[sosedb[inumber].iB])*
									(273.15 + potent[sosedb[inumber].iB])*
									(273.15 + potent[sosedb[inumber].iB]) - 
									...
									(273.15 + operating_temperature_for_film_coeff)*
									(273.15 + operating_temperature_for_film_coeff))) +
								(1.0 - alpha_relax1)*(-sosedb[inumber].emissivity*5.670367e-8*
								((273.15 + potent_old[sosedb[inumber].iB])*
									(273.15 + potent_old[sosedb[inumber].iB])*
									(273.15 + potent_old[sosedb[inumber].iB])*
									(273.15 + potent_old[sosedb[inumber].iB]) - 
				...
				if ((sosedb[inumber].MCB == (ls + lw)) && (adiabatic_vs_heat_transfer_coeff == 3)) {
					...
					*/
					...
					bsc1 = true;
					qb = -film_coefficient*(potent[sosedb[inumber].iB] - operating_temperature_for_film_coeff);
					qb = qb - sosedb[inumber].emissivity*5.670367e-8*
						((273.15 + potent[sosedb[inumber].iB])*
						(273.15 + potent[sosedb[inumber].iB])*
							(273.15 + potent[sosedb[inumber].iB])*
							(273.15 + potent[sosedb[inumber].iB]) - 
			...
			case ESIDE:
				...
				if (sosedb[inumber].iII >= maxelm) {
					...
					if (sosedb[inumber].iII < 0) {
						//printf("error acsess %d\n ", sosedb[inumber].iII);
						//system("PAUSE");
						lamII = prop[LAM][sosedb[inumber].iI];
					}
					else {
