...
// Авторы агрегатора PMIS Янг и де Стерк 2004 год.
...
void set_color_for_render_icolor(int icol)
{
	...
	if ((0 <= icol) and (icol <= 255)) {
		// Синий голубой
		glColor3f(0.0, (icol) / 255.0, 1.0);
	}
	else if ((256 <= icol) and (icol <= 510)) {
		//голубой - зелёный
		glColor3f(0, 1.0, (255.0 - (icol - 255.0)) / 255.0);
	}
	else if ((511 <= icol) and (icol <= 765)) {

		// зелёный - желтый
		glColor3f((icol - 510) / 255.0, 1.0, 0.0);
	}
	else if ((766 <= icol) and (icol <= 1020)) {
...
void set_color_for_render(int id) {
	...
	int icol = 0;
	if (iCFD_animation == 1) {
		icol = min(1020, myi_max(0,round(1020 * ((animation_sequence_functions_openGL[iCURENT_ANIMATION_FUNCTION][iCURENT_ANIMATION_CADER][id] - minimum_val_for_render_pic) / (maximum_val_for_render_pic - minimum_val_for_render_pic)))));
	}
	else {
		icol = min(1020, myi_max(0,round(1020 * ((potent_array_opengl[iCURENT_FUNC_openGL][id] - minimum_val_for_render_pic) / (maximum_val_for_render_pic - minimum_val_for_render_pic)))));
	}
	 //if ((0 <= icol) && (icol <= 1020)) {
...
void set_color_for_render(doublereal potent_val) {
	...
	int icol  = min(1020, myi_max(0, round(1020 * ((potent_val - minimum_val_for_render_pic) / (maximum_val_for_render_pic - minimum_val_for_render_pic)))));
	//if ((0 <= icol) && (icol <= 1020)) {
		//icol = mask_color_for_openGL[icol];// небольшое число различных цветов.
	//}
	set_color_for_render_icolor(icol);
