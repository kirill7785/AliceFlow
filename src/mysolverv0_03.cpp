...
#pragma omp parallel for
		...
		else if (1) {// На основе температуры хранящейся в центре ячейки (Метод Контрольного Объёма).
			// 22.08.2020 
...
#pragma omp parallel for
					   for (integer i = 0; i < f.maxbound; i++) {
						   if (f.slau_bon[GAMMA_LANGTRY_MENTER_SL][i].aw < 0.0) {
							   printf("maxbound=%lld i=%lld problem GAMMA_LANGTRY_MENTER boundary\n", f.maxbound, i);
							   system("pause");
...
#pragma omp parallel for
					   for (integer i = 0; i < f.maxelm; i++) {
						   if (f.slau[GAMMA_LANGTRY_MENTER_SL][i].ap < 0.0) {
							   printf("maxelm=%lld i=%lld problem GAMMA_LANGTRY_MENTER_SL ap=%e\n", f.maxelm, i, f.slau[GAMMA_LANGTRY_MENTER_SL][i].ap);
							   system("pause");
							   //f.slau_bon[iVar][i].aw
						   }
					   }
...
#pragma omp parallel for
							for (integer i = 0; i < f.maxbound; i++) {
								if (f.slau_bon[RE_THETA_LANGTRY_MENTER_SL][i].aw < 0.0) {
									printf("maxbound=%lld i=%lld problem RE_THETA_LANGTRY_MENTER boundary\n", f.maxbound, i);
									system("pause");
...
#pragma omp parallel for
							for (integer i = 0; i < f.maxelm; i++) {
								if (f.slau[RE_THETA_LANGTRY_MENTER_SL][i].ap < 0.0) {
									printf("maxelm=%lld i=%lld problem RE_THETA_LANGTRY_MENTER_SL ap=%e\n", f.maxelm, i, f.slau[RE_THETA_LANGTRY_MENTER_SL][i].ap);
									system("pause");
									//f.slau_bon[iVar][i].aw
								}
							}
	                               ...
	                               integer* &color, integer dist_max_fluid, integer* &color_solid, integer dist_max_solid) 
{
	...
	if (lw == 2) {
		...
		if (w[0].bpressure) {
			if ((w[1].Vx * w[1].Vx + w[1].Vy * w[1].Vy + w[1].Vz * w[1].Vz) > 1.0e-30) {
				b_mass_balance = false;
			}
		}
		else if (w[1].bpressure) {
			if ((w[0].Vx * w[0].Vx + w[0].Vy * w[0].Vy + w[0].Vz * w[0].Vz) > 1.0e-30) {
				b_mass_balance = false;
			}
		}
	}
...
#pragma omp parallel for
			...
			/*Gamma begin*/
			doublereal* turb_Gamma_Langtry_Mentor_SST_aprior = new doublereal[f.maxelm + f.maxbound];
...
#pragma omp parallel for
			...
			doublereal* turb_Re_Theta_Langtry_Mentor_SST_aprior = new doublereal[f.maxelm + f.maxbound];
