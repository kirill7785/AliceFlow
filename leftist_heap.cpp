...
void LeftistHeapSort(Ak2& Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		...
		Amat.i[i] = static_cast<integer_mix_precision>(get_min(heap));
		Amat.j[i] = j_a[i_];
		Amat.aij[i] = a[i_];
		i++;
...
void LeftistHeapSort_j(Ak2& Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		integer i_ = get_min_link(heap);
		Amat.j[i] = static_cast<integer_mix_precision>(get_min(heap));
		Amat.i[i] = i_a[i_];
		Amat.aij[i] = a[i_];
		i++;
		heap = delete_min(heap);
...
void LeftistHeapSort(Ak1* &Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		integer i_ = get_min_link(heap);
		Amat[i].i = static_cast<integer_mix_precision>(get_min(heap));
		Amat[i].j = static_cast<integer_mix_precision>(j_a[i_]);
		Amat[i].aij = a[i_];
		i++;
		heap = delete_min(heap);
	}
...
void LeftistHeapSort_j(Ak1* &Amat, integer first, integer last) {
	...
	while (heap != nullptr) {
		integer i_ = get_min_link(heap);
		Amat[i].j = static_cast<integer_mix_precision>(get_min(heap));
		Amat[i].i = static_cast<integer_mix_precision>(i_a[i_]);
		Amat[i].aij = a[i_];
		i++;
		heap = delete_min(heap);
	}
...
void FibonacciHeapSort(Ak1*& Amat, integer first, integer last) {
	...
	while (!fibo_heap.isEmpty()) {
		integer i_ = abs(static_cast<integer>(fibo_heap.getMinimum() % (n+1)))-1;
		if (i_a[i_] > i_prev) {
		...
		i_prev = i_a[i_];
		Amat[i].i = i_a[i_]; //static_cast<integer>(fibo_heap.getMinimum() % (n+1));
		Amat[i].j = j_a[i_];
		Amat[i].aij = a[i_];
		i++;
		data_BalTree ddel;
		ddel.i = abs(static_cast<integer>(fibo_heap.getMinimum() % (n+1)));
		ddel.count_neighbour = abs(static_cast<integer>(fibo_heap.getMinimum() / (n+1)));
