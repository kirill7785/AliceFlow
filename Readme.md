...
1. Если включить асемблес в модели BSK_Dmitrii_modify.txt то решение нефично низкую температуру показывает в этом асемблесе. 03.01.2023.
2. Нельзя использовать внутрипрограммные библиотечные материалы со свойствами зависящими от температуры в графовом методе. В графовом методе поддерживаются только материалы заданные пользователем. 05.02.2023
