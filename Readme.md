...
Я привёл все модели поставляемые с программой AliceFlow в актуальное состояние. Каждая модель была запущена заново на моём оборудовании. Рекомендую начинать знакомство с простых моделей, например, с модели диода EvdokimovaDiod2D715.txt. Далее можно посмотреть модель CMPA, сначала её гидродинамическую часть CMPA_onlyCFD.txt а затем и тепловую CMPA.txt. Далее остальные модели по вкусу. Модели PIONER_FINISH_90mm_geom.txt, PIONER_FINISH_90mm_geom_x8.txt, Modul_orebrenie.txt относятся к большим моделям напоминающим реальные. Выкладывая большие модели я стремился более полно раскрыть возможности программы. Права на большие модели принадлежат не мне, выкладывается только для демонстрации. Надеюсь ничьи права не ущемлены тем более что ничего конкретного в этих моделях нет.

## Известные проблемы работоспособности (bugs...)
1. Данная программа получает на вход геометрическую информацию самого разнообразного характера. Это и произвольные пользовательские модели из родных для программы обектов (Призмы, цилиндры, полигоны) и импортированные CAD stl объекты. Если говорить только о геометрии то пользователь программы слабо ограничен в черчении моделей. Ограничения конечно есть. Внутри кода программы существует богатый набор констант управляющих обработкой пользовательских входных данных. Я стараюсь настроить управляющие константы универсальным образом и многие мои модели отрабатываются штатно. Однако есть модели приводящие к сбою. Когда такая модель возникает я ввожу правки и тестирую всё по новой. Модели со сбоями не перестают появлятся особенно для CAD stl геометрии которую я использую в данный момент. Надежды вселяют модели которые отрабатываются штатно.
2. Алгебраический многосеточный метод тестируемый на этом многообразии задач сам представляет из себя подсистему с большим количеством настроек. Управляющие константы многосеточного метода настраиваются универсальным образом с запасом чтобы удовлетворять всем моделям подаваемым на вход. Надежды вселяю модели которые отрабатываются штатно. Практикуется подход наличия нескольких взаимозаменяемых решателей страхующих один другой и использующихся для проверки. В итоге проектируется система с дублированием-резервированием возможностей которая должна отработать штатно при любом вводе.
